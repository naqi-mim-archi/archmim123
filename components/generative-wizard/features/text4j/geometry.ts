...
import { auditText4AccessGraph } from '../../../../services/text4jSpatialPolicy';

export const completeText4jGeometry = (input: GeneratedData, designSummary = ''): GeneratedData => {
  const data: GeneratedData = {
    ...
    walls: input.walls?.map(w => ({
      ...
      p1: [...w.p1],
      p2: [...w.p2],
      ...(w.controlPoint ? { controlPoint: [...w.controlPoint] } : {}),
      ...(w.arcCenter ? { arcCenter: [...w.arcCenter] } : {}),
    })),
  ...
  // Local Image-to-JSON results already carry a completed extraction audit.
  // Their raster/OCR evidence is authoritative: semantic design-repair rules
  // must not relocate a detected balcony, replace its slab/railings, convert a
  // visible window into an assumed access door, or invent circulation. This
  // applies identically to Gemini-created and manually uploaded rasters because
  // both enter through the same extractor and expose extractionDiagnostics.
  const isCompletedRasterExtraction = !!input.extractionDiagnostics;
  type GeneratedDoor = NonNullable<GeneratedData['doors']>[number];
  type GeneratedWall = NonNullable<GeneratedData['walls']>[number];
  let mandatoryExteriorDoor: GeneratedDoor | undefined;
  data.doors?.forEach(door => {
    if (!door.mandatoryExteriorEntry) return;
    if (!mandatoryExteriorDoor) mandatoryExteriorDoor = door;
    else delete door.mandatoryExteriorEntry;
  });
  ...
  // Preserve the image orientation exactly. Earlier revisions mirrored plans
  // merely to move the entry facade to the bottom of the preview.
  ...
  const normalizeDuplicateEnvelope = () => {
    ...
    const isAxisWall = (wall: GeneratedWall, axis: EnvelopeSide['axis']) => {
      if (wall.isCurved || wall.wallSource === 'arc') return false;
  ...
  const closeShortWallGaps = () => {
    ...
    data.walls!.forEach(wall => {
      if (wall.isCurved || wall.wallSource === 'arc') return;
      (['p1', 'p2'] as const).forEach(key => {
        ...
        data.walls!.forEach(target => {
          if (target === wall || target.isCurved || target.wallSource === 'arc') return;
  ...
  // The two normalization passes above are conservative wall cleanups. All
  // remaining code in this function is architectural design completion, which
  // is appropriate for abstract generated data but not for faithful raster
  // digitization.
  if (isCompletedRasterExtraction) return data;
  ...
  if (directEntryRoom) {
    ...
    if (entryHost) {
      ...
      if (existingFoyerExteriorDoor && usableWidth >= 0.55) {
        if (!mandatoryExteriorDoor || mandatoryExteriorDoor === existingFoyerExteriorDoor) {
          existingFoyerExteriorDoor.mandatoryExteriorEntry = true;
          mandatoryExteriorDoor = existingFoyerExteriorDoor;
        }
      ...
      } else if (!existingFoyerExteriorDoor && !mandatoryExteriorDoor && usableWidth >= 0.55) {
        ...
        mandatoryExteriorDoor = entryDoor;
  ...
  if (balcony) {
    ...
    if (facadeWall && accessRoomBounds) {
      ...
      const existingFacadeDoor = data.doors?.find(door =>
        distanceToSegment(door.pos, facadeWall) < 0.2 &&
        door.pos[0] >= segmentStart - (door.width || 0) / 2 &&
        door.pos[0] <= segmentEnd + (door.width || 0) / 2
      );
      ...
      const maxAccessWidth = Math.min(2.4, Math.max(0.7, segmentLength - 0.1));
      const evidencedAccessWidth = [
        existingFacadeDoor?.measuredWidth,
        existingFacadeDoor?.width,
        existingFacadeWindow?.measuredWidth,
        existingFacadeWindow?.width,
      ].find(candidate => typeof candidate === 'number' && candidate >= 0.65 && candidate <= maxAccessWidth + 0.05);
      const accessWidth = Math.min(
        maxAccessWidth,
        evidencedAccessWidth ?? Math.max(0.8, Math.min(1.8, segmentLength * 0.45)),
      );
      if (existingFacadeDoor) {
        existingFacadeDoor.pos = [accessX, accessY];
        existingFacadeDoor.width = accessWidth;
        existingFacadeDoor.type = existingFacadeDoor.width >= 1.35 ? 'sliding' : 'glass';
        existingFacadeDoor.assumedProperties = Array.from(new Set([...(existingFacadeDoor.assumedProperties || []), 'glazed-balcony-access']));
      } else {
        // Balcony access is a circulation element. Convert the strongest
        // facade-window candidate into an explicit glazed door instead of
        // representing access as a low-sill/full-height window.
        if (existingFacadeWindow) data.windows = data.windows?.filter(window => window !== existingFacadeWindow);
        (data.doors ||= []).push({
          levelIndex: balcony.levelIndex,
          pos: [accessX, accessY], rotation: 0,
          type: accessWidth >= 1.35 ? 'sliding' : 'glass', width: accessWidth,
          measuredWidth: existingFacadeWindow?.measuredWidth,
          provenance: 'repair-generated',
          evidence: {
            source: 'geometry-repair', confidence: existingFacadeWindow ? 0.68 : 0.5,
            notes: [existingFacadeWindow
              ? 'A facade glazing candidate was reclassified as required balcony access.'
              : 'Balcony access was required by the confirmed room program but had no reliable door symbol.'],
          },
          assumedProperties: ['glazed-balcony-access'],
        });
      }
    }
  } else {
  ...
  // entry/balcony facade. Balcony circulation itself is represented by a door.
  ...
  const ensureExteriorWindow = (room: NonNullable<GeneratedData['rooms']>[number]) => {
    const roomBox = roomBounds(room);
    const alreadyHasAnyExteriorWindow = data.windows?.some(window =>
      (Math.abs(window.pos[0] - box.minX) < 0.2 && Math.abs(roomBox.left - box.minX) < 0.35 && window.pos[1] >= roomBox.bottom - 0.2 && window.pos[1] <= roomBox.top + 0.2) ||
      (Math.abs(window.pos[0] - box.maxX) < 0.2 && Math.abs(roomBox.right - box.maxX) < 0.35 && window.pos[1] >= roomBox.bottom - 0.2 && window.pos[1] <= roomBox.top + 0.2) ||
      (Math.abs(window.pos[1] - box.minY) < 0.2 && Math.abs(roomBox.bottom - box.minY) < 0.35 && window.pos[0] >= roomBox.left - 0.2 && window.pos[0] <= roomBox.right + 0.2) ||
      (Math.abs(window.pos[1] - box.maxY) < 0.2 && Math.abs(roomBox.top - box.maxY) < 0.35 && window.pos[0] >= roomBox.left - 0.2 && window.pos[0] <= roomBox.right + 0.2)
    );
    if (alreadyHasAnyExteriorWindow) return;
  ...
  // OCR labels are direct image evidence and must not be swapped merely because
  // a fragmented wall network produced an unreliable inferred room area.
  if (!data.extractionDiagnostics?.detectedRoomLabels) {
  ...
  // Preserve an element-level audit trail without changing any geometry. Items
  // already carrying raster/OCR evidence keep it; only locally completed items
  // are tagged here so downstream imports can distinguish observation from
  // repair-generated design assumptions.
  const markRepairGenerated = (
    items: Array<{
      provenance?: 'observed' | 'brief-derived' | 'code-default' | 'repair-generated';
      evidence?: { source: 'raster' | 'ocr' | 'design-brief' | 'catalog-default' | 'geometry-repair'; confidence: number; notes?: string[] };
    ...
    items?.forEach(item => {
      ...
      item.provenance = 'repair-generated';
      item.evidence = {
        source: 'geometry-repair',
        confidence: 0.45,
        notes: [`${elementKind} was added or completed by deterministic local geometry rules.`],
      };
      item.assumedProperties = Array.from(new Set([...(item.assumedProperties || []), 'locally-completed']));
    });
  };
  markRepairGenerated(data.walls, 'Wall');
  markRepairGenerated(data.doors, 'Door');
  markRepairGenerated(data.windows, 'Window');
  markRepairGenerated(data.openings, 'Wall opening');
  markRepairGenerated(data.rooms, 'Room marker');
  markRepairGenerated(data.columns, 'Column');
  markRepairGenerated(data.stairs, 'Stair');
  markRepairGenerated(data.railings, 'Railing');
  markRepairGenerated(data.furniture, 'Furniture');
  markRepairGenerated(data.fixtures, 'Fixture');
  ...
  const graphIssues = auditText4AccessGraph(graphRooms, graphEdges, entryIndex >= 0 ? `room-${entryIndex}` : undefined);
  if (graphIssues.length) console.warn('[Text 4.0 spatial audit]', graphIssues);
  console.debug(`[Text 4.0] local spatial audit ${(performance.now() - auditStarted).toFixed(1)} ms`);
  return data;
};
