...
interface ProceduralWizardProps {
  isOpen: boolean;
  onClose: () => void;
...
const ProceduralWizard: React.FC<ProceduralWizardProps> = ({ isOpen, onClose, onApply, initialConfig }) => {
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col h-[70vh] animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-2 text-blue-600">
            <Sparkles size={20} />
            <h2 className="font-bold text-lg text-slate-900">Procedural Architect</h2>
          </div>
...
const ReqCounter = ({ label, value, onChange, min = 0, max = 50, step = 1 }: any) => (
  <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
    <div className="font-bold text-xs text-slate-600 tracking-tight">{label}</div>
    <div className="flex items-center gap-3">
      <button onClick={() => onChange(Math.max(min, value - step))} className="w-8 h-8 rounded-full bg-white shadow-sm flex items-center justify-center text-slate-600 hover:bg-red-50 hover:text-red-600 transition-colors border border-slate-100">-</button>
      <span className="font-mono font-bold text-slate-800 w-6 text-center">{value}</span>
      <button onClick={() => onChange(Math.min(max, value + step))} className="w-8 h-8 rounded-full bg-white shadow-sm flex items-center justify-center text-slate-600 hover:bg-green-50 hover:text-green-600 transition-colors border border-slate-100">+</button>
    </div>
  </div>
);
...
export default ProceduralWizard;
