...
const root = path.resolve(process.argv[2] || process.env.TEXT4J_DS12_ROOT || DEFAULT_ROOT);
...
if (!existsSync(root)) {
  console.log(`[Text 4.0 J DS12] Fixture folder not available; skipped: ${root}`);
  process.exit(0);
}
...
console.log(JSON.stringify({
  fixtureRoot: root,
  scope: 'Text 4.0 J local-fallback orthogonal conversion',
  behaviorChanged: false,
  cases: reports,
}, null, 2));
console.log(`[Text 4.0 J DS12] Baseline harness passed for ${reports.length} orthogonal fixtures.`);
