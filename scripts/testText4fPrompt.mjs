...
const briefModule = await buildModule('services/text4fBrief.ts');
const promptModule = await buildModule('services/text4fPromptBuilder.ts');
const chatVariationModule = await buildModule('services/text4fConversationVariation.ts');
const fallbackModule = await buildModule('services/text4fFallbackGeometry.ts');
const imageConfigModule = await buildModule('services/text4fImageConfig.ts');
const imageAspectGuardModule = await buildModule('services/text4fGeneratedImageAspectGuard.ts');
const roomLabelModule = await buildModule('services/text4fGeneratedImageRoomLabels.ts');
const directUploadModule = await buildModule('services/text4fDirectUpload.ts');
const importHandoffModule = await buildModule('services/text4fImportHandoff.ts');
const text4fFeatureSource = await readFile('components/generative-wizard/features/text4f/index.ts', 'utf8');
const {
  ...
  text4fHasExplicitRectangularRequest,
  text4fBriefBoundaryMeters,
  text4fBriefToDesignSummary,
  updateText4fBriefFromSummary,
  validateText4fBrief,
} = briefModule;
const { buildText4fImagePrompt } = promptModule;
const { getText4fConversationProfile } = chatVariationModule;
const { buildText4fFallbackGeometry } = fallbackModule;
const {
  TEXT4F_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION,
  TEXT4F_LOW_LATENCY_GENERATION_CONFIG,
} = imageConfigModule;
const {
  detectText4fLockedStructuralFrame,
  getText4fLockedImageAspectTarget,
  planText4fLockedAspectCorrection,
} = imageAspectGuardModule;
const {
  buildText4fRoomCrossDimensionLabels,
  formatText4fRoomCrossDimensions,
} = roomLabelModule;
const {
...
const {
  finalizeText4fImportHandoff,
  isText4fAuthoritativePreview,
  markText4fAuthoritativePreview,
} = importHandoffModule;
...
const authoritativePreview = markText4fAuthoritativePreview(previewHandoffFixture);
assert.equal(isText4fAuthoritativePreview(authoritativePreview), true);
let handoffNormalizerCalls = 0;
const handedOffPreview = finalizeText4fImportHandoff(authoritativePreview, element => {
...
const legacyPreview = finalizeText4fImportHandoff(previewHandoffFixture, element => ({
  ...element,
  pos: { x: 999, y: 999 },
}));
...
assert.match(appImportSource, /finalizeText4fImportHandoff\(\s*remappedElements/,
  'Main-canvas placement must honor Text 4.0 F authoritative preview geometry before any legacy host normalization.');

const chatProfiles = [0, 1, 2].map(getText4fConversationProfile);
...
const imperialBrief = legacySummaryToText4fBrief(imperialSummary);
...
assert.equal(validateText4fBrief(imperialBrief).valid, true);
assert.equal(getText4fRectangularBoundaryEligibility(imperialBrief).coverageRatio, 1);
assert.equal(getText4fRectangularBoundaryEligibility(imperialBrief).eligible, true);
assert.deepEqual(imperialBrief.dimensions.rectangularBoundary, { locked: true, source: 'automatic_area_match' });

const imperialPrompt = buildText4fImagePrompt(imperialBrief);
const orderedHeadings = [
  ...
  'STYLE text4f-digitizable-floorplan-v1:',
  'DIMENSIONS:',
];
let previousIndex = -1;
...
assert.match(imperialPrompt, /STYLE text4f-digitizable-floorplan-v1/);
...
const narrowDeepPrompt = buildText4fImagePrompt(narrowDeepBrief);
...
const requestedCasePrompt = buildText4fImagePrompt(requestedCaseBrief);
...
const creativeBrief = legacySummaryToText4fBrief(creativeSummary);
const creativeEligibility = getText4fRectangularBoundaryEligibility(creativeBrief);
...
assert.deepEqual(
  setText4fRectangularBoundaryLock(creativeBrief, true).dimensions.rectangularBoundary,
  { locked: false, source: 'ineligible' },
  'An incompatible area must not be manually rectangle-locked.',
);
const creativePrompt = buildText4fImagePrompt(creativeBrief);
...
const thresholdBrief = legacySummaryToText4fBrief(imperialSummary.replace("600 sq ft (20' x 30')", "950 sq ft (25' x 40')"));
assert.equal(getText4fRectangularBoundaryEligibility(thresholdBrief).eligible, true, '95% coverage must be rectangle-eligible.');
assert.equal(thresholdBrief.dimensions.rectangularBoundary.locked, true);
const belowThresholdBrief = legacySummaryToText4fBrief(imperialSummary.replace("600 sq ft (20' x 30')", "949 sq ft (25' x 40')"));
assert.equal(getText4fRectangularBoundaryEligibility(belowThresholdBrief).eligible, false);

const manualUnlock = setText4fRectangularBoundaryLock(requestedCaseBrief, false);
assert.deepEqual(manualUnlock.dimensions.rectangularBoundary, { locked: false, source: 'user_confirmed' });
const explicitLock = applyText4fRectangularBoundaryPolicy(manualUnlock, 'Use a rectangular boundary.');
assert.deepEqual(explicitLock.dimensions.rectangularBoundary, { locked: true, source: 'explicit_user_request' });
assert.equal(text4fHasExplicitRectangularRequest('Please make it rectangular.'), true);
assert.equal(text4fHasExplicitRectangularRequest('Use a non-rectangular creative plan.'), false);

const landscapePrompt = buildText4fImagePrompt({
  ...requestedCaseBrief,
  dimensions: {
...
const multiFloorPrompt = buildText4fImagePrompt({
  ...requestedCaseBrief,
...
for (const example of reportedExamples) {
  const prompt = buildText4fImagePrompt({
    ...requestedCaseBrief,
    dimensions: {
      ...requestedCaseBrief.dimensions,
...
assert.deepEqual(TEXT4F_LOW_LATENCY_GENERATION_CONFIG.responseModalities, ['IMAGE']);
assert.equal(TEXT4F_LOW_LATENCY_GENERATION_CONFIG.thinkingConfig.thinkingLevel, 'minimal');
assert.equal(TEXT4F_LOW_LATENCY_GENERATION_CONFIG.thinkingConfig.includeThoughts, false);
assert.equal(TEXT4F_LOW_LATENCY_GENERATION_CONFIG.imageConfig.imageSize, '1K');
assert.equal(TEXT4F_LOW_LATENCY_GENERATION_CONFIG.imageConfig.aspectRatio, '1:1');
assert.equal(TEXT4F_LOW_LATENCY_GENERATION_CONFIG.candidateCount, 1);
assert.match(TEXT4F_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /hard rendering constraint/i);
assert.match(TEXT4F_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /within 2%/i);
assert.match(TEXT4F_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /Count balconies, terraces, porches, decks, steps, and railings/i);
assert.match(TEXT4F_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /Exclude annotations and every door leaf or swing arc/i);
assert.match(TEXT4F_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /RECTANGLE LOCKED/i);
assert.match(TEXT4F_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /CREATIVE UNLOCKED/i);
assert.match(TEXT4F_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /never force a rectangle by default/i);
assert.match(TEXT4F_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /Never render prompt instructions, pixel coordinates/i);
assert.match(TEXT4F_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /reserving clear white label zones/i);
assert.match(TEXT4F_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /one full Design Brief room name/i);
assert.match(TEXT4F_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /0\.33 times normal room-label height/i);
assert.match(TEXT4F_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /never abbreviate and never render room dimensions, room areas, label backgrounds, legends, or duplicate labels/i);
assert.match(TEXT4F_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /complete glyph box fits inside clear white floor space/i);
assert.match(TEXT4F_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /No label glyph may touch, cross, obscure, or replace any wall, door leaf, swing arc, window, wall opening, column, stair, railing, fixture, or other architectural mark/i);
assert.match(text4fFeatureSource, /annotateGeneratedPreview:\s*false/, 'The normal D-generated preview must not re-add room dimensions after OCR.');

assert.equal(formatText4fRoomCrossDimensions(3.77, 2.77, 'ft'), `12'-4\" x 9'-1\"`);
assert.equal(formatText4fRoomCrossDimensions(3.77, 2.77, 'm'), '3.77 m x 2.77 m');
const deterministicRoomLabels = buildText4fRoomCrossDimensionLabels({
  walls: [
...
assert.equal(buildText4fRoomCrossDimensionLabels({
  walls: [
    { p1: [0, 0], p2: [4, 0], type: 'exterior' },
    { p1: [4, 0], p2: [4, 3], type: 'exterior' },
    { p1: [4, 3], p2: [0, 3], type: 'exterior' },
...
const detectedLockedFrame = detectText4fLockedStructuralFrame({
...
const lockedTarget = getText4fLockedImageAspectTarget(requestedCaseBrief);
assert.equal(lockedTarget, 0.625);
const lockedCorrection = planText4fLockedAspectCorrection(detectedLockedFrame, lockedTarget);
...
assert.equal(getText4fLockedImageAspectTarget(creativeBrief), undefined, 'Creative-unlocked plans must bypass deterministic axis correction.');
assert.equal(planText4fLockedAspectCorrection({ ratio: 0.63 }, 0.625).applied, false);
assert.equal(getText4fLockedImageAspectTarget({
  ...requestedCaseBrief,
  project: { ...requestedCaseBrief.project, floors: 2 },
...
const metricBrief = legacySummaryToText4fBrief(metricSummary);
const metricPrompt = buildText4fImagePrompt(metricBrief);
...
const editedSummary = text4fBriefToDesignSummary(imperialBrief).replace("20'-0\" x 30'-0\"", "24'-0\" x 25'-0\"");
const editedBrief = updateText4fBriefFromSummary(editedSummary, imperialBrief, 'dimensions');
...
assert.match(buildText4fImagePrompt(editedBrief), /width 24'-0" above; depth 25'-0" left/);

const boundary = text4fBriefBoundaryMeters(imperialBrief);
assert.ok(boundary);
assert.ok(Math.abs(boundary[1].x - 6.096) < 0.000001);
assert.ok(Math.abs(boundary[2].y - 9.144) < 0.000001);

const blankDirectUpload = validateText4fDirectUploadScale({ unitSystem: 'imperial', width: '', depth: '', area: '' });
assert.equal(blankDirectUpload.valid, false, 'A direct upload must provide at least one dimension.');
const partialDirectUpload = prepareText4fDirectUpload(imperialBrief, { unitSystem: 'imperial', width: '25', depth: '', area: '' });
...
const verticalOnlyDirectUpload = prepareText4fDirectUpload(imperialBrief, { unitSystem: 'imperial', width: '', depth: '40', area: '' });
...
const areaOnlyDirectUpload = prepareText4fDirectUpload(imperialBrief, { unitSystem: 'imperial', width: '', depth: '', area: '1000' });
...
const dimensionsOnlyDirectUpload = prepareText4fDirectUpload(imperialBrief, { unitSystem: 'imperial', width: '25', depth: '40', area: '' });
...
const architecturalDirectUpload = prepareText4fDirectUpload(imperialBrief, { unitSystem: 'imperial', width: `25'-6"`, depth: '40 ft 0 in', area: '1,000 sq ft' });
...
const shorthandDirectUpload = validateText4fDirectUploadScale({ unitSystem: 'imperial', width: '36-6', depth: `26'-6"`, area: '1000 sft' });
...
const metricDirectUpload = prepareText4fDirectUpload(imperialBrief, { unitSystem: 'metric', width: '7.5', depth: '12', area: '90' });
...
const formattedMetricDirectUpload = validateText4fDirectUploadScale({ unitSystem: 'metric', width: '762 cm', depth: '12,192 mm', area: '92.9 m²' });
...
const wrongUnitDirectUpload = validateText4fDirectUploadScale({ unitSystem: 'imperial', width: '7.62 m', depth: '12.19 m', area: '' });
...
const fallback = buildText4fFallbackGeometry(imperialBrief, boundary, new Error('synthetic trace failure'));
...
const invalidBrief = legacySummaryToText4fBrief('Purpose: House\nRooms Included:\n- 1 Living Room\nFloors: Single-story');
assert.equal(validateText4fBrief(invalidBrief).valid, false);
assert.ok(validateText4fBrief(invalidBrief).errors.some(error => /width and depth/i.test(error)));

console.log(`Text 4.0 F canonical brief and prompt regression tests passed (${imperialPromptWords} prompt words).`);
