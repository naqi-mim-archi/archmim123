...
const resolver = await buildModule('services/text4cPresetResolver.ts');
const constants = await buildModule('constants.tsx');
const {
  TEXT4C_DOOR_PRESETS,
  TEXT4C_WALL_PRESETS,
  TEXT4C_WINDOW_PRESETS,
...
assert.deepEqual(
  TEXT4C_WALL_PRESETS.map(({ id, label, thicknessM }) => ({ id, label, thickness: thicknessM })),
  constants.WALL_PRESETS,
);
assert.deepEqual(
  TEXT4C_DOOR_PRESETS.map(({ id, label, widthM, subtype }) => ({ id, label, width: widthM, subType: subtype })),
  constants.DOOR_PRESETS,
);
assert.deepEqual(
  TEXT4C_WINDOW_PRESETS.map(({ id, label, widthM, heightM, subtype }) => ({
    id,
    label,
    width: widthM,
...
const residentialExterior = resolveText4cWallPreset({
...
const residentialInterior = resolveText4cWallPreset({
...
const commercialExterior = resolveText4cWallPreset({
  semanticRole: 'exterior',
...
const measuredCustomWall = resolveText4cWallPreset({
  semanticRole: 'exterior',
  projectClass: 'residential',
...
const bathroomDoor = resolveText4cDoorPreset({ roomRole: 'common bathroom', evidenceStrength: 'weak' });
...
const bedroomDoor = resolveText4cDoorPreset({ roomRole: 'master bedroom', evidenceStrength: 'none' });
assert.equal(bedroomDoor.presetId, 'door_single_md');
assert.equal(bedroomDoor.resolvedValues.widthM, 0.838);

const mainDoor = resolveText4cDoorPreset({ roomRole: 'main entrance', evidenceStrength: 'weak' });
...
const measuredBathSizedMainDoor = resolveText4cDoorPreset({
  measuredWidthM: 0.69,
...
const arbitraryDoor = resolveText4cDoorPreset({
  measuredWidthM: 0.77,
...
const noImplicitDoorTolerance = resolveText4cDoorPreset({
...
const noStrongRoleFallback = resolveText4cDoorPreset({
  roomRole: 'bathroom',
  evidenceStrength: 'strong',
});
...
const regularWindow = resolveText4cWindowPreset({
  measuredWidthM: 1.22,
...
const arbitraryWindow = resolveText4cWindowPreset({
  measuredWidthM: 1.08,
...
const wideRegularWindow = resolveText4cWindowPreset({
  measuredWidthM: 2.0,
  evidenceStrength: 'strong',
  toleranceM: 0.01,
});
...
console.log('Text 4.0 C conservative preset resolver tests passed.');
