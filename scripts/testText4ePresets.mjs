...
const resolver = await buildModule('services/text4ePresetResolver.ts');
const constants = await buildModule('constants.tsx');
const {
  TEXT4E_DOOR_PRESETS,
  TEXT4E_WALL_PRESETS,
  TEXT4E_WINDOW_PRESETS,
...
assert.deepEqual(
  TEXT4E_WALL_PRESETS.map(({ id, label, thicknessM }) => ({ id, label, thickness: thicknessM })),
  constants.WALL_PRESETS,
);
assert.deepEqual(
  TEXT4E_DOOR_PRESETS.map(({ id, label, widthM, subtype }) => ({ id, label, width: widthM, subType: subtype })),
  constants.DOOR_PRESETS,
);
assert.deepEqual(
  TEXT4E_WINDOW_PRESETS.map(({ id, label, widthM, heightM, subtype }) => ({
    id,
    label,
    width: widthM,
...
const residentialExterior = resolveText4eWallPreset({
...
const residentialInterior = resolveText4eWallPreset({
...
const commercialExterior = resolveText4eWallPreset({
  semanticRole: 'exterior',
...
const measuredCustomWall = resolveText4eWallPreset({
  semanticRole: 'exterior',
  projectClass: 'residential',
...
const bathroomDoor = resolveText4eDoorPreset({ roomRole: 'common bathroom', evidenceStrength: 'weak' });
...
const bedroomDoor = resolveText4eDoorPreset({ roomRole: 'master bedroom', evidenceStrength: 'none' });
assert.equal(bedroomDoor.presetId, 'door_single_md');
assert.equal(bedroomDoor.resolvedValues.widthM, 0.838);

const mainDoor = resolveText4eDoorPreset({ roomRole: 'main entrance', evidenceStrength: 'weak' });
...
const measuredBathSizedMainDoor = resolveText4eDoorPreset({
  measuredWidthM: 0.69,
...
const arbitraryDoor = resolveText4eDoorPreset({
  measuredWidthM: 0.77,
...
const noImplicitDoorTolerance = resolveText4eDoorPreset({
...
const noStrongRoleFallback = resolveText4eDoorPreset({
  roomRole: 'bathroom',
  evidenceStrength: 'strong',
});
...
const regularWindow = resolveText4eWindowPreset({
  measuredWidthM: 1.22,
...
const arbitraryWindow = resolveText4eWindowPreset({
  measuredWidthM: 1.08,
...
const wideRegularWindow = resolveText4eWindowPreset({
  measuredWidthM: 2.0,
  evidenceStrength: 'strong',
  toleranceM: 0.01,
});
...
console.log('Text 4.0 E conservative preset resolver tests passed.');
