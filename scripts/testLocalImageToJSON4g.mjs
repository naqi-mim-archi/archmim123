...
import { allElementsRasterFixture } from './fixtures/text4g/allElementsRasterFixture.mjs';
import { dataset4RasterFixtures } from './fixtures/text4g/dataset4RasterFixtures.mjs';
import { dataset5OcrFixtures } from './fixtures/text4g/dataset5OcrFixtures.mjs';
import { dataset5RasterFixtures } from './fixtures/text4g/dataset5RasterFixtures.mjs';

const allElementsExpected = JSON.parse(readFileSync(
  new URL('./fixtures/text4g/allElementsExpected.json', import.meta.url),
  'utf8',
));
...
const {
  ...
  findText4gExplicitHostedOffsetSwingForTest,
  findText4gPhysicalWallFaceSwingForTest,
  arbitrateText4gHostedOpenings,
  recoverText4gInteriorPartitionsForTest,
  recoverText4gConcentricInteriorRingForTest,
  recoverText4gMissingHostedWindowOrSlidingForTest,
  recoverText4gMissingFacadeWallRunsForTest,
  resolveText4gMissingHostRecoveryScaleForTest,
  resolveText4gFramedInsetPixelsForTest,
  text4gClosedLeafFitsHostForTest,
  text4gDominantEllipseGapHasRasterEvidenceForTest,
  text4gHostedAperturesOverlap,
  text4gInteriorArchitecturalCurveEvidenceForTest,
  text4gOpenShellExteriorCurveFamilyEvidenceForTest,
  text4gShouldRetainHybridCurveModeForTest,
  text4gOpeningHasHost,
  text4gSwingQuadrantForTest,
} = await import(moduleUrl);

assert.equal(text4gDominantEllipseGapHasRasterEvidenceForTest(0, 0.8), true,
  'A proven exterior ellipse must retain a strongly traced curved aperture interval.');
assert.equal(text4gDominantEllipseGapHasRasterEvidenceForTest(0, 0.79), false,
...
const leafToJambSwing = clampText4gRecoveredSwingGapToJambFacesForTest(
  {
    ...recoveredJambSwing,
    start: 110,
...
const angularClampedSwing = clampText4gRecoveredSwingGapToJambFacesForTest(
...
assert.equal(resolveText4gFramedInsetPixelsForTest(
...
assert.equal(text4gClosedLeafFitsHostForTest(56, 7), true,
  'A genuine closed leaf may remain within eight host-wall thicknesses.');
assert.equal(text4gClosedLeafFitsHostForTest(78, 7), false,
  'A long thin fixture edge must not borrow a loose arc as a closed door leaf.');
assert.deepEqual(
  text4gSwingQuadrantForTest('horizontal', false, true),
  text4gSwingQuadrantForTest('vertical', true, true),
  'Perpendicular interpretations of the same swing must resolve to the same world-space quadrant.',
);
assert.notDeepEqual(
  text4gSwingQuadrantForTest('horizontal', false, true),
  text4gSwingQuadrantForTest('vertical', false, true),
...
const ds12691OffsetSwing = findText4gExplicitHostedOffsetSwingForTest(
  ds12691Gap,
  ds12691Arc,
  ds12691Arc,
...
assert.equal(findText4gExplicitHostedOffsetSwingForTest(
  ds12691Gap,
  ds12691Arc,
...
const ds1269RecoveryScale = resolveText4gMissingHostRecoveryScaleForTest(
...
const slidingMissingHost = recoverText4gMissingHostedWindowOrSlidingForTest(
  missingHostRetained,
  [
...
const windowMissingHost = recoverText4gMissingHostedWindowOrSlidingForTest(
  missingHostRetained,
  [
...
const bareMissingHost = recoverText4gMissingHostedWindowOrSlidingForTest(
  missingHostRetained,
  [{ axis: 'horizontal', line: 100, start: 40, end: 100, thickness: 2 }],
  { x: 0.02, y: 0.02 },
  12,
...
assert.equal(text4gHostedAperturesOverlap(
  hostedDoor,
...
const arbitrated673Openings = arbitrateText4gHostedOpenings([hostedDoor], [
...
assert.equal(text4gOpeningHasHost(
...
assert.equal(text4gOpeningHasHost({ pos: [1, 1] }, [angularHostWall]), true,
  'A generic opening on an angular straight wall must remain available.');
assert.equal(text4gOpeningHasHost({ pos: [1, 1.5] }, [angularHostWall]), false,
...
assert.equal(text4gOpeningHasHost({ pos: [Math.SQRT1_2 * 5, Math.SQRT1_2 * 5] }, [curvedHostWall]), true,
  'A generic opening on the bounded native arc must remain available.');
assert.equal(text4gOpeningHasHost({ pos: [-Math.SQRT1_2 * 5, Math.SQRT1_2 * 5] }, [curvedHostWall]), false,
...
assert.equal(text4gOpeningHasHost({ pos: [Math.SQRT1_2 * 4, Math.SQRT1_2 * 2] }, [ellipseHostWall]), true,
  'A generic opening on a bounded native ellipse must remain available.');
const curveBuildResult = await build({
  entryPoints: ['services/text4gCurveArcs.ts'],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const geometryBuildResult = await build({
  entryPoints: ['components/generative-wizard/features/text4g/geometry.ts'],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const { completeText4gGeometry } = await import(geometryModuleUrl);
const angularCleanupBuildResult = await build({
  entryPoints: ['services/text4gAngularCleanup.ts'],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const geometryModesBuildResult = await build({
  entryPoints: ['services/text4gGeometryModes.ts'],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const curveHostingBuildResult = await build({
  entryPoints: ['services/text4gCurveHosting.ts'],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const { projectText4gPointToCurve, text4gCurveHostedPose } = await import(curveHostingModuleUrl);

const hostingEllipse = {
  id: 'text4g-hosting-ellipse', type: 'wall',
...
const ellipseProjection = projectText4gPointToCurve(hostingEllipse, { x: 0.1, y: 5.3 });
assert.ok(ellipseProjection && ellipseProjection.point.y > 4.95 && Math.abs(ellipseProjection.point.x) < 0.15,
  'Text 4.0 G curved inserts must project to the analytic ellipse instead of its p1-p2 chord.');
const ellipseHostedPose = text4gCurveHostedPose(hostingEllipse, 0.5, 1.2);
...
// be a narrow wall/furniture stroke; Text 4.0 G deliberately requires the
...
// Reported Text 4.0 G direct-upload signature: wall-band merging can stop a
...
assert.ok(findText4gPhysicalWallFaceSwingForTest(
...
assert.equal(findText4gPhysicalWallFaceSwingForTest(
...
const completedAllElements = runQuietly(() => completeText4gGeometry(allElementsWithBalconyOcr));
...
// shared converter used by generated and manually uploaded Text 4.0 G images.
const dataset4Cases = {
  '005': {
...
// Matched DS05 regression set: these Text 4.0 G-generated images contain
...
assert.ok(exactWallFaces.width <= 10 + 1e-9 && exactWallFaces.height <= 12 + 1e-9, 'Text 4.0 G exterior wall faces must fit inside the confirmed envelope.');
...
const completedExactEnvelope = runQuietly(() => completeText4gGeometry(exactEnvelope, options.designSummary));
...
const completedOcrAware = runQuietly(() => completeText4gGeometry(ocrAware));
...
const balconyPlan = runQuietly(() => completeText4gGeometry({
  walls: [
...
const recoveredMissingFacade = recoverText4gMissingFacadeWallRunsForTest(
...
const recoveredInteriorPartition = recoverText4gInteriorPartitionsForTest(
...
const supplementalAngularDoor = recoverText4gSupplementalAngularDoorHosts(
...
const unsupportedSupplementalDoor = recoverText4gSupplementalAngularDoorHosts(
  supplementalDoorStructuralMask,
  supplementalDoorStructuralMask,
  supplementalDoorMaskWidth,
  supplementalDoorMaskHeight,
...
const cleanedAngular = cleanupText4gAngularWalls(cleanupInput, cleanupAxisInput, {
...
const unsupportedGap = cleanupText4gAngularWalls([
...
const cleanedMixedJunction = cleanupText4gAngularWalls([
...
const chamferedExteriorCorner = collapseText4gExteriorAngularChamfers([
...
const protectedInteriorCap = collapseText4gExteriorAngularChamfers([
  { p1: { x: 0, y: 0 }, p2: { x: 100, y: 100 }, thickness: 8, confidence: 0.84 },
  { p1: { x: 100, y: 100 }, p2: { x: 110, y: 100 }, thickness: 8, confidence: 0.72 },
  { p1: { x: 110, y: 100 }, p2: { x: 210, y: 0 }, thickness: 8, confidence: 0.84 },
], {
...
const canonicalAngles = cleanupText4gAngularWalls([
...
const freeAngleResult = cleanupText4gAngularWalls(freeAngleInput, [], {
...
const absorbedBridge = cleanupText4gAngularWalls([
...
const microConnector = cleanupText4gAngularWalls([
...
const supportedCompletion = cleanupText4gAngularWalls([
...
const unsupportedCompletion = cleanupText4gAngularWalls([
  { p1: { x: 20, y: 100 }, p2: { x: 50, y: 70 }, thickness: 8, confidence: 0.84 },
  { p1: { x: 80, y: 40 }, p2: { x: 112, y: 72 }, thickness: 8, confidence: 0.84 },
], [], {
...
const jitteredRun = cleanupText4gAngularWalls([
...
const sharpCorner = cleanupText4gAngularWalls(sharpCornerInput, [], {
  mode: 'angular', structuralMask: cleanupMask,
  width: cleanupMaskWidth, height: cleanupMaskHeight, typicalThickness: 8,
});
assert.equal(sharpCorner.freeformSegments.length, 2,
  'A real angular corner must not be flattened by straight-run cleanup.');
const orthogonalNoOp = cleanupText4gAngularWalls(cleanupInput, cleanupAxisInput, {
...
const curvedNoOp = cleanupText4gAngularWalls(cleanupInput, cleanupAxisInput, {
...
const fragmentedCurves = consolidateText4gCurveArcs(fragmentedCurveSegments, {
...
assert.equal(detectText4gSparseAngularStairs(
...
assert.equal(detectText4gSparseAngularRailings(
...
assert.equal(detectText4gSparseAngularColumns(
...
const openBifoldEvidence = inspectText4gFreeformGap({
...
const compoundCurves = consolidateText4gCurveArcs(compoundCurveSegments, {
...
const compoundHybridCurves = consolidateText4gCurveArcs(compoundCurveSegments, {
...
assert.equal(text4gInteriorArchitecturalCurveEvidenceForTest(
  ds84InnerArc,
  ds84InnerSegments,
...
const ds84RecoveredRasterRing = recoverText4gConcentricInteriorRingForTest(
  ds84InnerMask,
  compoundCurveMaskSize,
  compoundCurveMaskSize,
  12,
...
assert.equal(recoverText4gConcentricInteriorRingForTest(
  ds84InnerMask,
  compoundCurveMaskSize,
  compoundCurveMaskSize,
  12,
...
const ds84ConsolidatedInner = consolidateText4gCurveArcs(ds84InnerSegments, {
  mode: 'hybrid', structuralMask: ds84InnerMask,
  width: compoundCurveMaskSize, height: compoundCurveMaskSize, typicalThickness: 12,
  curveEvidence: true,
  exteriorEvidence: () => false,
  interiorArchitecturalEvidence: (arc, sources) => text4gInteriorArchitecturalCurveEvidenceForTest(
...
assert.equal(text4gCurveArcSourceEnvelopeCoherentForTest({
  ...ds84InnerArc,
  center: { x: 280, y: 250 },
  ellipseRadiusX: 260,
  ellipseRadiusY: 80,
...
assert.equal(text4gShouldRetainHybridCurveModeForTest(
...
assert.equal(text4gOpenShellExteriorCurveFamilyEvidenceForTest(
  ds84InnerArc,
  ds84InnerSegments,
...
const driftingAxisCurves = consolidateText4gCurveArcs(driftingAxisSegments, {
...
const tangentSliverCurves = consolidateText4gCurveArcs(tangentSliverSegments, {
...
assert.ok(consolidateText4gCurveArcs(shallowArcSegments.concat([
