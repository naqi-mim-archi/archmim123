...
const briefModule = await buildModule('services/text4gBrief.ts');
const promptModule = await buildModule('services/text4gPromptBuilder.ts');
const chatVariationModule = await buildModule('services/text4gConversationVariation.ts');
const fallbackModule = await buildModule('services/text4gFallbackGeometry.ts');
const imageConfigModule = await buildModule('services/text4gImageConfig.ts');
const imageAspectGuardModule = await buildModule('services/text4gGeneratedImageAspectGuard.ts');
const roomLabelModule = await buildModule('services/text4gGeneratedImageRoomLabels.ts');
const directUploadModule = await buildModule('services/text4gDirectUpload.ts');
const importHandoffModule = await buildModule('services/text4gImportHandoff.ts');
const masterFloorplanModule = await buildModule('services/text4gMasterFloorplanData.ts');
const text4gFeatureSource = await readFile('components/generative-wizard/features/text4g/index.ts', 'utf8');
const wizardCoreSource = await readFile('components/generative-wizard/GenerativeWizardCore.tsx', 'utf8');
const masterFloorplanSource = await readFile('services/text4gMasterFloorplanData.ts', 'utf8');
const masterBackendSource = await readFile('services/text4gBackend.ts', 'utf8');
const {
  ...
  text4gHasExplicitRectangularRequest,
  text4gBriefBoundaryMeters,
  text4gBriefToDesignSummary,
  updateText4gBriefFromSummary,
  validateText4gBrief,
} = briefModule;
const { buildText4gImagePrompt } = promptModule;
const { getText4gConversationProfile } = chatVariationModule;
const { buildText4gFallbackGeometry } = fallbackModule;
const {
  TEXT4G_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION,
  TEXT4G_LOW_LATENCY_GENERATION_CONFIG,
} = imageConfigModule;
const {
  detectText4gLockedStructuralFrame,
  getText4gLockedImageAspectTarget,
  planText4gLockedAspectCorrection,
} = imageAspectGuardModule;
const {
  buildText4gRoomCrossDimensionLabels,
  formatText4gRoomCrossDimensions,
} = roomLabelModule;
const {
...
const {
  finalizeText4gImportHandoff,
  isText4gAuthoritativePreview,
  markText4gAuthoritativePreview,
} = importHandoffModule;
const {
  buildText4gMasterFloorplanPrompt,
  isText4gMasterFloorplanDataUsable,
  normalizeText4gMasterFloorplanData,
  splitText4gDataUrl,
  validateText4gMasterFloorplanGraph,
...
const authoritativePreview = markText4gAuthoritativePreview(previewHandoffFixture);
assert.equal(isText4gAuthoritativePreview(authoritativePreview), true);
let handoffNormalizerCalls = 0;
const handedOffPreview = finalizeText4gImportHandoff(authoritativePreview, element => {
...
const legacyPreview = finalizeText4gImportHandoff(previewHandoffFixture, element => ({
  ...element,
  pos: { x: 999, y: 999 },
}));
...
assert.match(appImportSource, /finalizeText4gImportHandoff\(\s*remappedElements/,
  'Main-canvas placement must honor Text 4.0 G authoritative preview geometry before any legacy host normalization.');
const canvasSource = await readFile(new URL('../components/Canvas.tsx', import.meta.url), 'utf8');
assert.match(canvasSource, /text4gAuthoritativePreview/,
  'Canvas auto-split must bypass Text 4.0 G authoritative preview geometry after import.');
assert.match(text4gFeatureSource, /transcribeText4gMasterFloorplanData/,
  'Text 4.0 G conversion should try the master floorplan data path before local raster fallback.');
assert.match(text4gFeatureSource, /Master floorplan data unavailable; continuing with local image-to-JSON fallback/,
  'Text 4.0 G master-data transcription must keep the mature local extractor as fallback.');
assert.match(wizardCoreSource, /runText4gComparisonConversion/,
  'Text 4.0 G Design Copilot should expose the Gemini master and local fallback comparison workflow.');
assert.match(wizardCoreSource, /useMasterFloorplanData:\s*false/,
  'Text 4.0 G local comparison lane must remain the mature local image-to-JSON fallback, not the master data path.');
assert.match(wizardCoreSource, /Import \${selectedText4gComparison === 'master' \? 'Gemini Master' : 'Local Fallback'}/,
  'Text 4.0 G import button should identify which comparison result will be imported.');
assert.match(wizardCoreSource, /Text 4\.0 G JSON Comparison/,
  'Text 4.0 G should show the Gemini master and local fallback previews side by side for testing.');
assert.match(wizardCoreSource, /Download All Test Data/,
  'Text 4.0 G temporary testing UI should provide one-click dataset export.');
assert.match(wizardCoreSource, /gemini-master-screenshot/,
  'Text 4.0 G dataset export should include the Gemini master preview screenshot.');
assert.match(wizardCoreSource, /local-fallback-screenshot/,
  'Text 4.0 G dataset export should include the local fallback preview screenshot.');

const chatProfiles = [0, 1, 2].map(getText4gConversationProfile);
...
const imperialBrief = legacySummaryToText4gBrief(imperialSummary);
...
assert.equal(validateText4gBrief(imperialBrief).valid, true);
assert.equal(getText4gRectangularBoundaryEligibility(imperialBrief).coverageRatio, 1);
assert.equal(getText4gRectangularBoundaryEligibility(imperialBrief).eligible, true);
assert.deepEqual(imperialBrief.dimensions.rectangularBoundary, { locked: true, source: 'automatic_area_match' });

const imperialPrompt = buildText4gImagePrompt(imperialBrief);
const masterPromptForUpload = buildText4gMasterFloorplanPrompt(imperialBrief, 'uploaded', undefined, { width: 6.096, depth: 9.144 });
...
assert.doesNotMatch(masterFloorplanSource, /localGeometryEvidence|from ['"].*localImageToJSON4g/i,
...
assert.match(wizardCoreSource, /thinkingLevel: text4gMasterThinkingLevel/,
...
assert.match(masterBackendSource, /validateText4gMasterFloorplanGraph/);
const masterRouteSource = masterBackendSource.slice(
  masterBackendSource.indexOf("if (url.startsWith('/api/text4g/master-geometry'))"),
  masterBackendSource.indexOf('const ai = new GoogleGenAI({', masterBackendSource.indexOf("if (url.startsWith('/api/text4g/master-geometry'))") + 100),
);
...
assert.deepEqual(splitText4gDataUrl('data:image/png;base64,abc123'), { mimeType: 'image/png', data: 'abc123' });
const normalizedMasterData = normalizeText4gMasterFloorplanData({
  walls: [{
    p1: [0, 0],
    p2: [4, 0],
    type: 'exterior',
...
assert.equal(isText4gMasterFloorplanDataUsable(normalizedMasterData), false);
const usableMasterData = normalizeText4gMasterFloorplanData({
  walls: [
...
assert.equal(isText4gMasterFloorplanDataUsable(usableMasterData), true);
const ds77NormalizedMaster = normalizeText4gMasterFloorplanData({
...
const ds77WidthOnlyMaster = normalizeText4gMasterFloorplanData({
  coordinateSpace: 'normalized_0_1000',
  junctions: [
...
assert.equal(validateText4gMasterFloorplanGraph(invalidHostedGraph).valid, false,
  'An unhosted aperture must invalidate Gemini Master geometry.');
assert.match(validateText4gMasterFloorplanGraph(invalidHostedGraph).errors.join(' '), /existing host wall/i);
const invalidHostedResult = normalizeText4gMasterFloorplanData(invalidHostedGraph, {
...
const diagonalHostedMaster = normalizeText4gMasterFloorplanData({
  coordinateSpace: 'normalized_0_1000',
  junctions: [
    { id: 'J1', x: 0, y: 0 }, { id: 'J2', x: 1000, y: 0 },
    { id: 'J3', x: 1000, y: 1000 }, { id: 'J4', x: 0, y: 1000 },
...
assert.equal(validateText4gMasterFloorplanGraph(selfIntersectingGraph).valid, false);
assert.match(validateText4gMasterFloorplanGraph(selfIntersectingGraph).errors.join(' '), /self-intersects/i,
  'Self-intersecting exterior geometry must never be importable.');
const curvedHostedMaster = normalizeText4gMasterFloorplanData({
  coordinateSpace: 'normalized_0_1000',
  junctions: [
    { id: 'J1', x: 0, y: 0 }, { id: 'J2', x: 1000, y: 0 },
    { id: 'J3', x: 1000, y: 1000 }, { id: 'J4', x: 0, y: 1000 },
...
const orderedHeadings = [
  ...
  'STYLE text4g-digitizable-floorplan-v1:',
  'DIMENSIONS:',
];
let previousIndex = -1;
...
assert.match(imperialPrompt, /STYLE text4g-digitizable-floorplan-v1/);
...
const narrowDeepPrompt = buildText4gImagePrompt(narrowDeepBrief);
...
const requestedCasePrompt = buildText4gImagePrompt(requestedCaseBrief);
...
const creativeBrief = legacySummaryToText4gBrief(creativeSummary);
const creativeEligibility = getText4gRectangularBoundaryEligibility(creativeBrief);
...
assert.deepEqual(
  setText4gRectangularBoundaryLock(creativeBrief, true).dimensions.rectangularBoundary,
  { locked: false, source: 'ineligible' },
  'An incompatible area must not be manually rectangle-locked.',
);
const creativePrompt = buildText4gImagePrompt(creativeBrief);
...
const thresholdBrief = legacySummaryToText4gBrief(imperialSummary.replace("600 sq ft (20' x 30')", "950 sq ft (25' x 40')"));
assert.equal(getText4gRectangularBoundaryEligibility(thresholdBrief).eligible, true, '95% coverage must be rectangle-eligible.');
assert.equal(thresholdBrief.dimensions.rectangularBoundary.locked, true);
const belowThresholdBrief = legacySummaryToText4gBrief(imperialSummary.replace("600 sq ft (20' x 30')", "949 sq ft (25' x 40')"));
assert.equal(getText4gRectangularBoundaryEligibility(belowThresholdBrief).eligible, false);

const manualUnlock = setText4gRectangularBoundaryLock(requestedCaseBrief, false);
assert.deepEqual(manualUnlock.dimensions.rectangularBoundary, { locked: false, source: 'user_confirmed' });
const explicitLock = applyText4gRectangularBoundaryPolicy(manualUnlock, 'Use a rectangular boundary.');
assert.deepEqual(explicitLock.dimensions.rectangularBoundary, { locked: true, source: 'explicit_user_request' });
assert.equal(text4gHasExplicitRectangularRequest('Please make it rectangular.'), true);
assert.equal(text4gHasExplicitRectangularRequest('Use a non-rectangular creative plan.'), false);

const landscapePrompt = buildText4gImagePrompt({
  ...requestedCaseBrief,
  dimensions: {
...
const multiFloorPrompt = buildText4gImagePrompt({
  ...requestedCaseBrief,
...
for (const example of reportedExamples) {
  const prompt = buildText4gImagePrompt({
    ...requestedCaseBrief,
    dimensions: {
      ...requestedCaseBrief.dimensions,
...
const circularPrompt = buildText4gImagePrompt({
  ...creativeBrief,
  project: {
...
assert.deepEqual(TEXT4G_LOW_LATENCY_GENERATION_CONFIG.responseModalities, ['IMAGE']);
assert.equal(TEXT4G_LOW_LATENCY_GENERATION_CONFIG.thinkingConfig.thinkingLevel, 'minimal');
assert.equal(TEXT4G_LOW_LATENCY_GENERATION_CONFIG.thinkingConfig.includeThoughts, false);
assert.equal(TEXT4G_LOW_LATENCY_GENERATION_CONFIG.imageConfig.imageSize, '1K');
assert.equal(TEXT4G_LOW_LATENCY_GENERATION_CONFIG.imageConfig.aspectRatio, '1:1');
assert.equal(TEXT4G_LOW_LATENCY_GENERATION_CONFIG.candidateCount, 1);
assert.match(TEXT4G_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /hard rendering constraint/i);
assert.match(TEXT4G_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /within 2%/i);
assert.match(TEXT4G_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /Count balconies, terraces, porches, decks, steps, and railings/i);
assert.match(TEXT4G_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /Exclude annotations and every door leaf or swing arc/i);
assert.match(TEXT4G_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /RECTANGLE LOCKED/i);
assert.match(TEXT4G_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /CREATIVE UNLOCKED/i);
assert.match(TEXT4G_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /never force a rectangle by default/i);
assert.match(TEXT4G_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /Never render prompt instructions, pixel coordinates/i);
assert.match(TEXT4G_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /reserving clear white label zones/i);
assert.match(TEXT4G_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /one full Design Brief room name/i);
assert.match(TEXT4G_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /0\.33 times normal room-label height/i);
assert.match(TEXT4G_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /never abbreviate and never render room dimensions, room areas, label backgrounds, legends, or duplicate labels/i);
assert.match(TEXT4G_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /complete glyph box fits inside clear white floor space/i);
assert.match(TEXT4G_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /No label glyph may touch, cross, obscure, or replace any wall, door leaf, swing arc, window, wall opening, column, stair, railing, fixture, or other architectural mark/i);
assert.match(text4gFeatureSource, /annotateGeneratedPreview:\s*false/, 'The normal D-generated preview must not re-add room dimensions after OCR.');

assert.equal(formatText4gRoomCrossDimensions(3.77, 2.77, 'ft'), `12'-4\" x 9'-1\"`);
assert.equal(formatText4gRoomCrossDimensions(3.77, 2.77, 'm'), '3.77 m x 2.77 m');
const deterministicRoomLabels = buildText4gRoomCrossDimensionLabels({
  walls: [
    { p1: [0, 0], p2: [4, 0], type: 'exterior' },
    { p1: [4, 0], p2: [4, 3], type: 'exterior' },
...
assert.equal(buildText4gRoomCrossDimensionLabels({
  walls: [
    { p1: [0, 0], p2: [4, 0], type: 'exterior' },
    { p1: [4, 0], p2: [4, 3], type: 'exterior' },
    { p1: [4, 3], p2: [0, 3], type: 'exterior' },
...
const detectedLockedFrame = detectText4gLockedStructuralFrame({
...
const lockedTarget = getText4gLockedImageAspectTarget(requestedCaseBrief);
assert.equal(lockedTarget, 0.625);
const lockedCorrection = planText4gLockedAspectCorrection(detectedLockedFrame, lockedTarget);
...
assert.equal(getText4gLockedImageAspectTarget(creativeBrief), undefined, 'Creative-unlocked plans must bypass deterministic axis correction.');
assert.equal(planText4gLockedAspectCorrection({ ratio: 0.63 }, 0.625).applied, false);
assert.equal(getText4gLockedImageAspectTarget({
  ...requestedCaseBrief,
  project: { ...requestedCaseBrief.project, floors: 2 },
...
const metricBrief = legacySummaryToText4gBrief(metricSummary);
const metricPrompt = buildText4gImagePrompt(metricBrief);
...
const editedSummary = text4gBriefToDesignSummary(imperialBrief).replace("20'-0\" x 30'-0\"", "24'-0\" x 25'-0\"");
const editedBrief = updateText4gBriefFromSummary(editedSummary, imperialBrief, 'dimensions');
...
assert.match(buildText4gImagePrompt(editedBrief), /width 24'-0" above; depth 25'-0" left/);

const boundary = text4gBriefBoundaryMeters(imperialBrief);
assert.ok(boundary);
assert.ok(Math.abs(boundary[1].x - 6.096) < 0.000001);
assert.ok(Math.abs(boundary[2].y - 9.144) < 0.000001);

const blankDirectUpload = validateText4gDirectUploadScale({ unitSystem: 'imperial', width: '', depth: '', area: '' });
assert.equal(blankDirectUpload.valid, false, 'A direct upload must provide at least one dimension.');
const partialDirectUpload = prepareText4gDirectUpload(imperialBrief, { unitSystem: 'imperial', width: '25', depth: '', area: '' });
...
const verticalOnlyDirectUpload = prepareText4gDirectUpload(imperialBrief, { unitSystem: 'imperial', width: '', depth: '40', area: '' });
...
const areaOnlyDirectUpload = prepareText4gDirectUpload(imperialBrief, { unitSystem: 'imperial', width: '', depth: '', area: '1000' });
...
const dimensionsOnlyDirectUpload = prepareText4gDirectUpload(imperialBrief, { unitSystem: 'imperial', width: '25', depth: '40', area: '' });
...
const architecturalDirectUpload = prepareText4gDirectUpload(imperialBrief, { unitSystem: 'imperial', width: `25'-6"`, depth: '40 ft 0 in', area: '1,000 sq ft' });
...
const shorthandDirectUpload = validateText4gDirectUploadScale({ unitSystem: 'imperial', width: '36-6', depth: `26'-6"`, area: '1000 sft' });
...
const metricDirectUpload = prepareText4gDirectUpload(imperialBrief, { unitSystem: 'metric', width: '7.5', depth: '12', area: '90' });
...
const formattedMetricDirectUpload = validateText4gDirectUploadScale({ unitSystem: 'metric', width: '762 cm', depth: '12,192 mm', area: '92.9 m²' });
...
const wrongUnitDirectUpload = validateText4gDirectUploadScale({ unitSystem: 'imperial', width: '7.62 m', depth: '12.19 m', area: '' });
...
const fallback = buildText4gFallbackGeometry(imperialBrief, boundary, new Error('synthetic trace failure'));
...
const invalidBrief = legacySummaryToText4gBrief('Purpose: House\nRooms Included:\n- 1 Living Room\nFloors: Single-story');
assert.equal(validateText4gBrief(invalidBrief).valid, false);
assert.ok(validateText4gBrief(invalidBrief).errors.some(error => /width and depth/i.test(error)));

console.log(`Text 4.0 G canonical brief and prompt regression tests passed (${imperialPromptWords} prompt words).`);
