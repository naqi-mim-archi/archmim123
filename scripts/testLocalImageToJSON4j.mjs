...
import { allElementsRasterFixture } from './fixtures/text4j/allElementsRasterFixture.mjs';
import { dataset4RasterFixtures } from './fixtures/text4j/dataset4RasterFixtures.mjs';
import { dataset5OcrFixtures } from './fixtures/text4j/dataset5OcrFixtures.mjs';
import { dataset5RasterFixtures } from './fixtures/text4j/dataset5RasterFixtures.mjs';

const allElementsExpected = JSON.parse(readFileSync(
  new URL('./fixtures/text4j/allElementsExpected.json', import.meta.url),
  'utf8',
));

const buildResult = await build({
  entryPoints: ['services/localImageToJSON4j.ts'],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const {
  ...
  createText4jDashedOpeningEvidenceMaskForTest,
  detectDoorSwing,
  extractGeometryFromImageData,
  findText4jExplicitHostedOffsetSwingForTest,
  findText4jPhysicalWallFaceSwingForTest,
  arbitrateText4jHostedOpenings,
  recoverText4jInteriorPartitionsForTest,
  recoverText4jTopologyValidatedPartitionsForTest,
  retainText4jCooperatingExteriorWallNetworksForTest,
  resolveText4jHostedNonDoorRasterKindForTest,
  recoverText4jConcentricInteriorRingForTest,
  recoverText4jMissingHostedWindowOrSlidingForTest,
  recoverText4jMissingFacadeWallRunsForTest,
  resolveText4jMissingHostRecoveryScaleForTest,
  resolveText4jFramedInsetPixelsForTest,
  text4jClosedLeafFitsHostForTest,
  text4jDominantEllipseGapHasRasterEvidenceForTest,
  text4jHostedAperturesOverlap,
  ...
  text4jOpeningHasHost,
  text4jSwingQuadrantForTest,
  text4jParallelDashedOpeningBandCountForTest,
...
const retainedSplitEnvelope = retainText4jCooperatingExteriorWallNetworksForTest(
...
const recoveredOrthogonalTopology = recoverText4jTopologyValidatedPartitionsForTest(
  topologyShell,
  [],
...
const recoveredThinInteriorTopology = recoverText4jTopologyValidatedPartitionsForTest(
  topologyShell,
  [],
...
const recoveredDoorTerminatedTopology = recoverText4jTopologyValidatedPartitionsForTest(
  topologyShell,
  [],
...
const rejectedOpenPlanDoorTerminatedTopology = recoverText4jTopologyValidatedPartitionsForTest(
  topologyShell,
  [],
  [...topologyShell, doorTerminatedTopologyCandidate],
  [],
...
const completedPartialTopology = recoverText4jTopologyValidatedPartitionsForTest(
...
const preservedOpenPlanTopology = recoverText4jTopologyValidatedPartitionsForTest(
  topologyShell,
  [],
  [...topologyShell, verticalTopologyCandidate],
  [],
...
const recoveredRadialTopology = recoverText4jTopologyValidatedPartitionsForTest(
  topologyShell,
  [],
  topologyShell,
  [{
...
const dashedOpeningMask = createText4jDashedOpeningEvidenceMaskForTest({
...
const dashedOpeningBands = text4jParallelDashedOpeningBandCountForTest(
  verticalApertureGap,
  dashedOpeningMask,
  apertureMaskWidth,
  apertureMaskHeight,
...
assert.equal(resolveText4jHostedNonDoorRasterKindForTest({
  exterior: false,
...
const leafToJambSwing = clampText4jRecoveredSwingGapToJambFacesForTest(
  {
    ...recoveredJambSwing,
    start: 110,
...
const angularClampedSwing = clampText4jRecoveredSwingGapToJambFacesForTest(
...
assert.equal(resolveText4jFramedInsetPixelsForTest(
...
assert.equal(text4jClosedLeafFitsHostForTest(56, 7), true,
  'A genuine closed leaf may remain within eight host-wall thicknesses.');
assert.equal(text4jClosedLeafFitsHostForTest(78, 7), false,
  'A long thin fixture edge must not borrow a loose arc as a closed door leaf.');
assert.deepEqual(
  text4jSwingQuadrantForTest('horizontal', false, true),
  text4jSwingQuadrantForTest('vertical', true, true),
  'Perpendicular interpretations of the same swing must resolve to the same world-space quadrant.',
);
assert.notDeepEqual(
  text4jSwingQuadrantForTest('horizontal', false, true),
  text4jSwingQuadrantForTest('vertical', false, true),
...
const ds12691OffsetSwing = findText4jExplicitHostedOffsetSwingForTest(
  ds12691Gap,
  ds12691Arc,
  ds12691Arc,
...
assert.equal(findText4jExplicitHostedOffsetSwingForTest(
  ds12691Gap,
  ds12691Arc,
...
const ds1269RecoveryScale = resolveText4jMissingHostRecoveryScaleForTest(
...
const slidingMissingHost = recoverText4jMissingHostedWindowOrSlidingForTest(
  missingHostRetained,
  [
...
const windowMissingHost = recoverText4jMissingHostedWindowOrSlidingForTest(
  missingHostRetained,
  [
...
const bareMissingHost = recoverText4jMissingHostedWindowOrSlidingForTest(
  missingHostRetained,
  [{ axis: 'horizontal', line: 100, start: 40, end: 100, thickness: 2 }],
  { x: 0.02, y: 0.02 },
  12,
...
assert.equal(text4jHostedAperturesOverlap(
  hostedDoor,
...
const arbitrated673Openings = arbitrateText4jHostedOpenings([hostedDoor], [
...
assert.equal(text4jOpeningHasHost(
...
assert.equal(text4jOpeningHasHost({ pos: [1, 1] }, [angularHostWall]), true,
  'A generic opening on an angular straight wall must remain available.');
assert.equal(text4jOpeningHasHost({ pos: [1, 1.5] }, [angularHostWall]), false,
...
assert.equal(text4jOpeningHasHost({ pos: [Math.SQRT1_2 * 5, Math.SQRT1_2 * 5] }, [curvedHostWall]), true,
  'A generic opening on the bounded native arc must remain available.');
assert.equal(text4jOpeningHasHost({ pos: [-Math.SQRT1_2 * 5, Math.SQRT1_2 * 5] }, [curvedHostWall]), false,
...
assert.equal(text4jOpeningHasHost({ pos: [Math.SQRT1_2 * 4, Math.SQRT1_2 * 2] }, [ellipseHostWall]), true,
  'A generic opening on a bounded native ellipse must remain available.');
const curveBuildResult = await build({
  entryPoints: ['services/text4jCurveArcs.ts'],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const geometryBuildResult = await build({
  entryPoints: ['components/generative-wizard/features/text4j/geometry.ts'],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const { completeText4jGeometry } = await import(geometryModuleUrl);
const angularCleanupBuildResult = await build({
  entryPoints: ['services/text4jAngularCleanup.ts'],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const geometryModesBuildResult = await build({
  entryPoints: ['services/text4jGeometryModes.ts'],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const curveHostingBuildResult = await build({
  entryPoints: ['services/text4jCurveHosting.ts'],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const { projectText4jPointToCurve, text4jCurveHostedPose } = await import(curveHostingModuleUrl);

const hostingEllipse = {
  id: 'text4j-hosting-ellipse', type: 'wall',
...
const ellipseProjection = projectText4jPointToCurve(hostingEllipse, { x: 0.1, y: 5.3 });
assert.ok(ellipseProjection && ellipseProjection.point.y > 4.95 && Math.abs(ellipseProjection.point.x) < 0.15,
  'Text 4.0 J curved inserts must project to the analytic ellipse instead of its p1-p2 chord.');
const ellipseHostedPose = text4jCurveHostedPose(hostingEllipse, 0.5, 1.2);
...
// be a narrow wall/furniture stroke; Text 4.0 J deliberately requires the
...
// Reported Text 4.0 J direct-upload signature: wall-band merging can stop a
...
assert.ok(findText4jPhysicalWallFaceSwingForTest(
...
assert.equal(findText4jPhysicalWallFaceSwingForTest(
...
const completedAllElements = runQuietly(() => completeText4jGeometry(allElementsWithBalconyOcr));
...
// shared converter used by generated and manually uploaded Text 4.0 J images.
const dataset4Cases = {
  '005': {
...
// Matched DS05 regression set: these Text 4.0 J-generated images contain
...
assert.ok(exactWallFaces.width <= 10 + 1e-9 && exactWallFaces.height <= 12 + 1e-9, 'Text 4.0 J exterior wall faces must fit inside the confirmed envelope.');
...
const completedExactEnvelope = runQuietly(() => completeText4jGeometry(exactEnvelope, options.designSummary));
...
const completedOcrAware = runQuietly(() => completeText4jGeometry(ocrAware));
...
const balconyPlan = runQuietly(() => completeText4jGeometry({
  walls: [
...
const recoveredMissingFacade = recoverText4jMissingFacadeWallRunsForTest(
...
const recoveredInteriorPartition = recoverText4jInteriorPartitionsForTest(
...
const supplementalAngularDoor = recoverText4jSupplementalAngularDoorHosts(
...
const unsupportedSupplementalDoor = recoverText4jSupplementalAngularDoorHosts(
  supplementalDoorStructuralMask,
  supplementalDoorStructuralMask,
  supplementalDoorMaskWidth,
  supplementalDoorMaskHeight,
...
const cleanedAngular = cleanupText4jAngularWalls(cleanupInput, cleanupAxisInput, {
...
const unsupportedGap = cleanupText4jAngularWalls([
...
const cleanedMixedJunction = cleanupText4jAngularWalls([
...
const chamferedExteriorCorner = collapseText4jExteriorAngularChamfers([
...
const protectedInteriorCap = collapseText4jExteriorAngularChamfers([
  { p1: { x: 0, y: 0 }, p2: { x: 100, y: 100 }, thickness: 8, confidence: 0.84 },
  { p1: { x: 100, y: 100 }, p2: { x: 110, y: 100 }, thickness: 8, confidence: 0.72 },
  { p1: { x: 110, y: 100 }, p2: { x: 210, y: 0 }, thickness: 8, confidence: 0.84 },
], {
...
const canonicalAngles = cleanupText4jAngularWalls([
...
const freeAngleResult = cleanupText4jAngularWalls(freeAngleInput, [], {
...
const absorbedBridge = cleanupText4jAngularWalls([
...
const microConnector = cleanupText4jAngularWalls([
...
const supportedCompletion = cleanupText4jAngularWalls([
...
const unsupportedCompletion = cleanupText4jAngularWalls([
  { p1: { x: 20, y: 100 }, p2: { x: 50, y: 70 }, thickness: 8, confidence: 0.84 },
  { p1: { x: 80, y: 40 }, p2: { x: 112, y: 72 }, thickness: 8, confidence: 0.84 },
], [], {
...
const jitteredRun = cleanupText4jAngularWalls([
...
const sharpCorner = cleanupText4jAngularWalls(sharpCornerInput, [], {
  mode: 'angular', structuralMask: cleanupMask,
  width: cleanupMaskWidth, height: cleanupMaskHeight, typicalThickness: 8,
});
assert.equal(sharpCorner.freeformSegments.length, 2,
  'A real angular corner must not be flattened by straight-run cleanup.');
const orthogonalNoOp = cleanupText4jAngularWalls(cleanupInput, cleanupAxisInput, {
...
const curvedNoOp = cleanupText4jAngularWalls(cleanupInput, cleanupAxisInput, {
...
const fragmentedCurves = consolidateText4jCurveArcs(fragmentedCurveSegments, {
...
assert.equal(detectText4jSparseAngularStairs(
...
assert.equal(detectText4jSparseAngularRailings(
...
assert.equal(detectText4jSparseAngularColumns(
...
const openBifoldEvidence = inspectText4jFreeformGap({
...
const compoundCurves = consolidateText4jCurveArcs(compoundCurveSegments, {
...
const compoundHybridCurves = consolidateText4jCurveArcs(compoundCurveSegments, {
...
const ds84RecoveredRasterRing = recoverText4jConcentricInteriorRingForTest(
  ds84InnerMask,
  compoundCurveMaskSize,
  compoundCurveMaskSize,
  12,
...
const ds84ConsolidatedInner = consolidateText4jCurveArcs(ds84InnerSegments, {
...
const driftingAxisCurves = consolidateText4jCurveArcs(driftingAxisSegments, {
...
const tangentSliverCurves = consolidateText4jCurveArcs(tangentSliverSegments, {
...
assert.ok(consolidateText4jCurveArcs(shallowArcSegments.concat([
