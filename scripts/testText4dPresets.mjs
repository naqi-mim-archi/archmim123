...
const resolver = await buildModule('services/text4dPresetResolver.ts');
const constants = await buildModule('constants.tsx');
const {
  TEXT4D_DOOR_PRESETS,
  TEXT4D_WALL_PRESETS,
  TEXT4D_WINDOW_PRESETS,
...
assert.deepEqual(
  TEXT4D_WALL_PRESETS.map(({ id, label, thicknessM }) => ({ id, label, thickness: thicknessM })),
  constants.WALL_PRESETS,
);
assert.deepEqual(
  TEXT4D_DOOR_PRESETS.map(({ id, label, widthM, subtype }) => ({ id, label, width: widthM, subType: subtype })),
  constants.DOOR_PRESETS,
);
assert.deepEqual(
  TEXT4D_WINDOW_PRESETS.map(({ id, label, widthM, heightM, subtype }) => ({
    id,
    label,
    width: widthM,
...
const residentialExterior = resolveText4dWallPreset({
...
const residentialInterior = resolveText4dWallPreset({
...
const commercialExterior = resolveText4dWallPreset({
  semanticRole: 'exterior',
...
const measuredCustomWall = resolveText4dWallPreset({
  semanticRole: 'exterior',
  projectClass: 'residential',
...
const bathroomDoor = resolveText4dDoorPreset({ roomRole: 'common bathroom', evidenceStrength: 'weak' });
...
const bedroomDoor = resolveText4dDoorPreset({ roomRole: 'master bedroom', evidenceStrength: 'none' });
assert.equal(bedroomDoor.presetId, 'door_single_md');
assert.equal(bedroomDoor.resolvedValues.widthM, 0.838);

const mainDoor = resolveText4dDoorPreset({ roomRole: 'main entrance', evidenceStrength: 'weak' });
...
const measuredBathSizedMainDoor = resolveText4dDoorPreset({
  measuredWidthM: 0.69,
...
const arbitraryDoor = resolveText4dDoorPreset({
  measuredWidthM: 0.77,
...
const noImplicitDoorTolerance = resolveText4dDoorPreset({
...
const noStrongRoleFallback = resolveText4dDoorPreset({
  roomRole: 'bathroom',
  evidenceStrength: 'strong',
});
...
const regularWindow = resolveText4dWindowPreset({
  measuredWidthM: 1.22,
...
const arbitraryWindow = resolveText4dWindowPreset({
  measuredWidthM: 1.08,
...
const wideRegularWindow = resolveText4dWindowPreset({
  measuredWidthM: 2.0,
  evidenceStrength: 'strong',
  toleranceM: 0.01,
});
...
console.log('Text 4.0 D conservative preset resolver tests passed.');
