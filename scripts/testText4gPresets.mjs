...
const resolver = await buildModule('services/text4gPresetResolver.ts');
const constants = await buildModule('constants.tsx');
const {
  TEXT4G_DOOR_PRESETS,
  TEXT4G_WALL_PRESETS,
  TEXT4G_WINDOW_PRESETS,
...
assert.deepEqual(
  TEXT4G_WALL_PRESETS.map(({ id, label, thicknessM }) => ({ id, label, thickness: thicknessM })),
  constants.WALL_PRESETS,
);
assert.deepEqual(
  TEXT4G_DOOR_PRESETS.map(({ id, label, widthM, subtype }) => ({ id, label, width: widthM, subType: subtype })),
  constants.DOOR_PRESETS,
);
assert.deepEqual(
  TEXT4G_WINDOW_PRESETS.map(({ id, label, widthM, heightM, subtype }) => ({
    id,
    label,
    width: widthM,
...
const residentialExterior = resolveText4gWallPreset({
...
const residentialInterior = resolveText4gWallPreset({
...
const commercialExterior = resolveText4gWallPreset({
  semanticRole: 'exterior',
...
const measuredCustomWall = resolveText4gWallPreset({
  semanticRole: 'exterior',
  projectClass: 'residential',
...
const bathroomDoor = resolveText4gDoorPreset({ roomRole: 'common bathroom', evidenceStrength: 'weak' });
...
const bedroomDoor = resolveText4gDoorPreset({ roomRole: 'master bedroom', evidenceStrength: 'none' });
assert.equal(bedroomDoor.presetId, 'door_single_md');
assert.equal(bedroomDoor.resolvedValues.widthM, 0.838);

const mainDoor = resolveText4gDoorPreset({ roomRole: 'main entrance', evidenceStrength: 'weak' });
...
const measuredBathSizedMainDoor = resolveText4gDoorPreset({
  measuredWidthM: 0.69,
...
const arbitraryDoor = resolveText4gDoorPreset({
  measuredWidthM: 0.77,
...
const noImplicitDoorTolerance = resolveText4gDoorPreset({
...
const noStrongRoleFallback = resolveText4gDoorPreset({
  roomRole: 'bathroom',
  evidenceStrength: 'strong',
});
...
const regularWindow = resolveText4gWindowPreset({
  measuredWidthM: 1.22,
...
const arbitraryWindow = resolveText4gWindowPreset({
  measuredWidthM: 1.08,
...
const wideRegularWindow = resolveText4gWindowPreset({
  measuredWidthM: 2.0,
  evidenceStrength: 'strong',
  toleranceM: 0.01,
});
...
console.log('Text 4.0 G conservative preset resolver tests passed.');
