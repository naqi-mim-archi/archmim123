...
const resolver = await buildModule('services/text4hPresetResolver.ts');
const constants = await buildModule('constants.tsx');
const {
  TEXT4H_DOOR_PRESETS,
  TEXT4H_WALL_PRESETS,
  TEXT4H_WINDOW_PRESETS,
...
assert.deepEqual(
  TEXT4H_WALL_PRESETS.map(({ id, label, thicknessM }) => ({ id, label, thickness: thicknessM })),
  constants.WALL_PRESETS,
);
assert.deepEqual(
  TEXT4H_DOOR_PRESETS.map(({ id, label, widthM, subtype }) => ({ id, label, width: widthM, subType: subtype })),
  constants.DOOR_PRESETS,
);
assert.deepEqual(
  TEXT4H_WINDOW_PRESETS.map(({ id, label, widthM, heightM, subtype }) => ({
    id,
    label,
    width: widthM,
...
const residentialExterior = resolveText4hWallPreset({
...
const residentialInterior = resolveText4hWallPreset({
...
const commercialExterior = resolveText4hWallPreset({
  semanticRole: 'exterior',
...
const measuredCustomWall = resolveText4hWallPreset({
  semanticRole: 'exterior',
  projectClass: 'residential',
...
const bathroomDoor = resolveText4hDoorPreset({ roomRole: 'common bathroom', evidenceStrength: 'weak' });
...
const bedroomDoor = resolveText4hDoorPreset({ roomRole: 'master bedroom', evidenceStrength: 'none' });
assert.equal(bedroomDoor.presetId, 'door_single_md');
assert.equal(bedroomDoor.resolvedValues.widthM, 0.838);

const mainDoor = resolveText4hDoorPreset({ roomRole: 'main entrance', evidenceStrength: 'weak' });
...
const measuredBathSizedMainDoor = resolveText4hDoorPreset({
  measuredWidthM: 0.69,
...
const arbitraryDoor = resolveText4hDoorPreset({
  measuredWidthM: 0.77,
...
const noImplicitDoorTolerance = resolveText4hDoorPreset({
...
const noStrongRoleFallback = resolveText4hDoorPreset({
  roomRole: 'bathroom',
  evidenceStrength: 'strong',
});
...
const regularWindow = resolveText4hWindowPreset({
  measuredWidthM: 1.22,
...
const arbitraryWindow = resolveText4hWindowPreset({
  measuredWidthM: 1.08,
...
const wideRegularWindow = resolveText4hWindowPreset({
  measuredWidthM: 2.0,
  evidenceStrength: 'strong',
  toleranceM: 0.01,
});
...
console.log('Text 4.0 H conservative preset resolver tests passed.');
