...
import { allElementsRasterFixture } from './fixtures/text4d/allElementsRasterFixture.mjs';
import { dataset4RasterFixtures } from './fixtures/text4d/dataset4RasterFixtures.mjs';
import { dataset5OcrFixtures } from './fixtures/text4d/dataset5OcrFixtures.mjs';
import { dataset5RasterFixtures } from './fixtures/text4d/dataset5RasterFixtures.mjs';

const allElementsExpected = JSON.parse(readFileSync(
  new URL('./fixtures/text4d/allElementsExpected.json', import.meta.url),
  'utf8',
));

const buildResult = await build({
  entryPoints: ['services/localImageToJSON4d.ts'],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const geometryBuildResult = await build({
  entryPoints: ['components/generative-wizard/features/text4d/geometry.ts'],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const { completeText4dGeometry } = await import(geometryModuleUrl);
...
const completedAllElements = runQuietly(() => completeText4dGeometry(allElementsWithBalconyOcr));
...
// shared converter used by generated and manually uploaded Text 4.0 D images.
const dataset4Cases = {
  '005': {
...
// Matched DS05 regression set: these Text 4.0 D-generated images contain
...
assert.ok(exactWallFaces.width <= 10 + 1e-9 && exactWallFaces.height <= 12 + 1e-9, 'Text 4.0 D exterior wall faces must fit inside the confirmed envelope.');
...
const completedExactEnvelope = runQuietly(() => completeText4dGeometry(exactEnvelope, options.designSummary));
...
const completedOcrAware = runQuietly(() => completeText4dGeometry(ocrAware));
...
const balconyPlan = runQuietly(() => completeText4dGeometry({
  walls: [
