...
} = await load('services/localImageToJSON4g.ts');
const {
  consolidateText4gCurveArcs,
  text4gCurveArcSourceEnvelopeCoherentForTest,
} = await load('services/text4gCurveArcs.ts');
...
const recoveredRasterRing = recoverText4gConcentricInteriorRingForTest(
...
const retainedRing = consolidateText4gCurveArcs(ring, {
...
assert.equal(text4gCurveArcSourceEnvelopeCoherentForTest(ringArc, ring, size, size, 12), true,
...
assert.equal(text4gDominantEllipseGapHasRasterEvidenceForTest(0.5, 0), true,
  'Observed structural ink must continue a proven dominant ellipse.');
assert.equal(text4gDominantEllipseGapHasRasterEvidenceForTest(0.24, 0.6), true,
