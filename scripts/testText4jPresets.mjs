...
const resolver = await buildModule('services/text4jPresetResolver.ts');
const constants = await buildModule('constants.tsx');
const {
  TEXT4J_DOOR_PRESETS,
  TEXT4J_WALL_PRESETS,
  TEXT4J_WINDOW_PRESETS,
...
assert.deepEqual(
  TEXT4J_WALL_PRESETS.map(({ id, label, thicknessM }) => ({ id, label, thickness: thicknessM })),
  constants.WALL_PRESETS,
);
assert.deepEqual(
  TEXT4J_DOOR_PRESETS.map(({ id, label, widthM, subtype }) => ({ id, label, width: widthM, subType: subtype })),
  constants.DOOR_PRESETS,
);
assert.deepEqual(
  TEXT4J_WINDOW_PRESETS.map(({ id, label, widthM, heightM, subtype }) => ({
    id,
    label,
    width: widthM,
...
const residentialExterior = resolveText4jWallPreset({
...
const residentialInterior = resolveText4jWallPreset({
...
const commercialExterior = resolveText4jWallPreset({
  semanticRole: 'exterior',
...
const measuredCustomWall = resolveText4jWallPreset({
  semanticRole: 'exterior',
  projectClass: 'residential',
...
const bathroomDoor = resolveText4jDoorPreset({ roomRole: 'common bathroom', evidenceStrength: 'weak' });
...
const bedroomDoor = resolveText4jDoorPreset({ roomRole: 'master bedroom', evidenceStrength: 'none' });
assert.equal(bedroomDoor.presetId, 'door_single_md');
assert.equal(bedroomDoor.resolvedValues.widthM, 0.838);

const mainDoor = resolveText4jDoorPreset({ roomRole: 'main entrance', evidenceStrength: 'weak' });
...
const measuredBathSizedMainDoor = resolveText4jDoorPreset({
  measuredWidthM: 0.69,
...
const arbitraryDoor = resolveText4jDoorPreset({
  measuredWidthM: 0.77,
...
const noImplicitDoorTolerance = resolveText4jDoorPreset({
...
const noStrongRoleFallback = resolveText4jDoorPreset({
  roomRole: 'bathroom',
  evidenceStrength: 'strong',
});
...
const regularWindow = resolveText4jWindowPreset({
  measuredWidthM: 1.22,
...
const arbitraryWindow = resolveText4jWindowPreset({
  measuredWidthM: 1.08,
...
const wideRegularWindow = resolveText4jWindowPreset({
  measuredWidthM: 2.0,
  evidenceStrength: 'strong',
  toleranceM: 0.01,
});
...
console.log('Text 4.0 J conservative preset resolver tests passed.');
