...
const briefModule = await buildModule('services/text4hBrief.ts');
const promptModule = await buildModule('services/text4hPromptBuilder.ts');
const chatVariationModule = await buildModule('services/text4hConversationVariation.ts');
const fallbackModule = await buildModule('services/text4hFallbackGeometry.ts');
const imageConfigModule = await buildModule('services/text4hImageConfig.ts');
const imageAspectGuardModule = await buildModule('services/text4hGeneratedImageAspectGuard.ts');
const roomLabelModule = await buildModule('services/text4hGeneratedImageRoomLabels.ts');
const directUploadModule = await buildModule('services/text4hDirectUpload.ts');
const importHandoffModule = await buildModule('services/text4hImportHandoff.ts');
const masterFloorplanModule = await buildModule('services/text4hMasterFloorplanData.ts');
const text4hFeatureSource = await readFile('components/generative-wizard/features/text4h/index.ts', 'utf8');
const wizardCoreSource = await readFile('components/generative-wizard/GenerativeWizardCore.tsx', 'utf8');
const canvasSource = await readFile('components/Canvas.tsx', 'utf8');
const imageServiceSource = await readFile('services/imageGenService4h.ts', 'utf8');
const masterFloorplanSource = await readFile('services/text4hMasterFloorplanData.ts', 'utf8');
const masterBackendSource = await readFile('services/text4hBackend.ts', 'utf8');
const {
  ...
  text4hHasExplicitRectangularRequest,
  text4hBriefBoundaryMeters,
  text4hBriefToDesignSummary,
  updateText4hBriefFromSummary,
  validateText4hBrief,
} = briefModule;
const { buildText4hImagePrompt } = promptModule;
const { getText4hConversationProfile } = chatVariationModule;
const { buildText4hFallbackGeometry } = fallbackModule;
const {
  TEXT4H_AUTOSCAN_REDRAW_PROMPT,
  TEXT4H_IMAGE_MODEL,
  TEXT4H_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION,
  TEXT4H_LOW_LATENCY_GENERATION_CONFIG,
} = imageConfigModule;

assert.equal(TEXT4H_IMAGE_MODEL, 'gemini-3.1-flash-lite-image');
assert.doesNotMatch(wizardCoreSource, /Floorplan Image Model|imagen-3\.0-(?:fast-)?generate/,
  'AutoPlan must not expose image-model selection or obsolete Imagen models.');
assert.doesNotMatch(imageServiceSource, /imageModel|targetModel/,
  'The browser must not select or submit the AutoPlan image model.');
assert.match(masterBackendSource, /const selectedModel = TEXT4H_IMAGE_MODEL/,
  'The Text 4.0 H backend must own image-model selection.');
assert.doesNotMatch(masterBackendSource, /generateImages|imagen-3\.0|Attempting Imagen/,
  'The Text 4.0 H backend must not retain obsolete Imagen generation paths.');
const {
  detectText4hLockedStructuralFrame,
  getText4hLockedImageAspectTarget,
  planText4hLockedAspectCorrection,
} = imageAspectGuardModule;
const {
  buildText4hRoomCrossDimensionLabels,
  formatText4hRoomCrossDimensions,
} = roomLabelModule;
const {
  prepareText4hDirectUpload,
  validateText4hDirectUploadScale,
} = directUploadModule;
assert.match(TEXT4H_AUTOSCAN_REDRAW_PROMPT, /exterior walls are exactly 9 inches thick/);
assert.match(TEXT4H_AUTOSCAN_REDRAW_PROMPT, /interior partition walls are exactly 4\.5 inches thick/);
assert.match(TEXT4H_AUTOSCAN_REDRAW_PROMPT, /ditto 100% reproduce/);
assert.match(masterBackendSource, /\/api\/text4h\/image-redraw[\s\S]*inlineData: \{ data: imageBytes, mimeType \}/,
  'AutoScan must attach the uploaded raster to the backend-owned H Gemini redraw request.');
assert.match(masterBackendSource, /const redrawPrompt = scaleParts\.length > 0[\s\S]*Confirmed user scale marking:[\s\S]*\{ text: redrawPrompt \}/,
  'The AutoScan draftsman prompt must be selected by the backend rather than supplied by the browser.');
assert.match(wizardCoreSource, /label: 'Direct Conversion'[\s\S]*label: 'App-Generated Image Conversion'/,
  'AutoScan must present direct and app-generated conversion results together.');
assert.match(wizardCoreSource, /convertAutoScanImage\(selectedImage, 'local'[\s\S]*normalizedImage = await redrawAutoScanFloorplan\(selectedImage, \{[\s\S]*convertAutoScanImage\(normalizedImage, 'master'/,
  'AutoScan must digitize both the original upload and the Gemini-standardized raster using the shared local H extractor.');
assert.match(wizardCoreSource, /Promise\.allSettled\(\[directTask, generatedTask\]\)/,
  'AutoScan must complete both conversion paths before enabling import.');
assert.match(wizardCoreSource, /Show Image[\s\S]*App-generated standardized floorplan/,
  'The app-generated result must provide a subtle control for viewing its generated raster.');
assert.match(wizardCoreSource, /!isAutoScanFlashMode && !isAutoPlanGenerationMode && \(generatedData\?\.sourceImageBase64/,
  'AutoScan and AutoPlan must keep source rasters out of the separate legacy image preview.');
assert.match(wizardCoreSource, /useState\(false\);[\s\S]*showImageToJsonLogs && !isText4jMode[\s\S]*Extraction confidence:/,
  'Image-to-JSON logs must be hidden by default and rendered only after the Logs toggle is enabled.');
assert.match(wizardCoreSource, />\s*Logs\s*<\/button>/,
  'Image-to-JSON previews must provide a Logs button.');
assert.match(wizardCoreSource, /const hasSelectedSimplifiedFloorplan =[\s\S]*previewElements\.length > 0[\s\S]*isText4dDigitizationPending && !hasSelectedSimplifiedFloorplan[\s\S]*canImport === false && !hasSelectedSimplifiedFloorplan/,
  'A selected AutoPlan or AutoScan floorplan with geometry must bypass pending and advisory confidence import gates.');
assert.match(wizardCoreSource, /spinner\.animate\([\s\S]*iterations: Infinity[\s\S]*willChange: 'transform'/,
  'AutoPlan and AutoScan processing must use an indefinitely composited spinner during main-thread-heavy digitization.');
assert.match(wizardCoreSource, /isAutoScanFlashMode \|\| isAutoPlanGenerationMode \? <CompositedProcessingSpinner/,
  'The composited spinner must be active for both AutoScan and AutoPlan processing views.');
assert.match(wizardCoreSource, /label: 'Generated Floorplan - Variant 1'[\s\S]*label: 'Generated Floorplan - Variant 2'/,
  'AutoPlan must present two generated floorplan variants side by side.');
assert.match(wizardCoreSource, /const generateVariant = async[\s\S]*generateFloorplanImage4h\(confirmedBrief, \{ variantIndex, variantCount: 2 \}\)[\s\S]*convertFloorplanImage4h\(image[\s\S]*useMasterFloorplanData: false/,
  'Each AutoPlan variant must be generated as an image and converted only through the shared local H extractor.');
assert.match(wizardCoreSource, /Promise\.allSettled\(\[[\s\S]*generateVariant\('master'\)[\s\S]*generateVariant\('local'\)/,
  'AutoPlan must generate and digitize two independent variants.');
assert.match(wizardCoreSource, /isAutoPlanGenerationMode \? 'Generated Floorplans'[\s\S]*Choose between two locally digitized variants/,
  'AutoPlan result UI must use floorplan language rather than Gemini JSON language.');
assert.match(wizardCoreSource, /autoPlanRasterRegistration[\s\S]*pixelReferenceBounds: rasterPixelBounds[\s\S]*worldReferenceBounds: rasterWorldBounds/,
  'AutoPlan image underlays must register raster wall evidence against digitized wall-face bounds.');
assert.match(wizardCoreSource, /autoPlanImageVisibility\[key\] \? 'Hide Image' : 'Show Image'[\s\S]*rasterUnderlay=\{autoPlanImageVisibility\[key\] \? autoPlanRasterRegistration : null\}/,
  'Each AutoPlan variant must provide an independent Show Image underlay toggle.');
assert.match(canvasSource, /if \(!isExport && rasterUnderlay && rasterUnderlayImageRef\.current\)/,
  'Raster underlays must remain preview-only and be excluded from vector exports.');
const {
  finalizeText4hImportHandoff,
  isText4hAuthoritativePreview,
  markText4hAuthoritativePreview,
} = importHandoffModule;
const {
  buildText4hMasterFloorplanPrompt,
  isText4hMasterFloorplanDataUsable,
  normalizeText4hMasterFloorplanData,
  splitText4hDataUrl,
  validateText4hMasterFloorplanGraph,
...
const authoritativePreview = markText4hAuthoritativePreview(previewHandoffFixture);
assert.equal(isText4hAuthoritativePreview(authoritativePreview), true);
let handoffNormalizerCalls = 0;
const handedOffPreview = finalizeText4hImportHandoff(authoritativePreview, element => {
...
const legacyPreview = finalizeText4hImportHandoff(previewHandoffFixture, element => ({
  ...element,
  pos: { x: 999, y: 999 },
}));
...
assert.match(appImportSource, /finalizeText4hImportHandoff\(\s*remappedElements/,
  'Main-canvas placement must honor Text 4.0 H authoritative preview geometry before any legacy host normalization.');
const canvasSource = await readFile(new URL('../components/Canvas.tsx', import.meta.url), 'utf8');
assert.match(canvasSource, /text4hAuthoritativePreview/,
  'Canvas auto-split must bypass Text 4.0 H authoritative preview geometry after import.');
assert.match(text4hFeatureSource, /transcribeText4hMasterFloorplanData/,
  'Text 4.0 H conversion should try the master floorplan data path before local raster fallback.');
assert.match(text4hFeatureSource, /Master floorplan data unavailable; continuing with local image-to-JSON fallback/,
  'Text 4.0 H master-data transcription must keep the mature local extractor as fallback.');
assert.match(wizardCoreSource, /runText4hComparisonConversion/,
  'Text 4.0 H Design Copilot should expose the Gemini master and local fallback comparison workflow.');
assert.match(wizardCoreSource, /useMasterFloorplanData:\s*false/,
  'Text 4.0 H local comparison lane must remain the mature local image-to-JSON fallback, not the master data path.');
assert.match(wizardCoreSource, /Import \${selectedText4hComparison === 'master' \? 'Gemini Master' : 'Local Fallback'}/,
  'Text 4.0 H import button should identify which comparison result will be imported.');
assert.match(wizardCoreSource, /Text 4\.0 G JSON Comparison/,
  'Text 4.0 H should show the Gemini master and local fallback previews side by side for testing.');
assert.match(wizardCoreSource, /Download All Test Data/,
  'Text 4.0 H temporary testing UI should provide one-click dataset export.');
assert.match(wizardCoreSource, /gemini-master-screenshot/,
  'Text 4.0 H dataset export should include the Gemini master preview screenshot.');
assert.match(wizardCoreSource, /local-fallback-screenshot/,
  'Text 4.0 H dataset export should include the local fallback preview screenshot.');

const chatProfiles = [0, 1, 2].map(getText4hConversationProfile);
...
const imperialBrief = legacySummaryToText4hBrief(imperialSummary);
...
assert.equal(validateText4hBrief(imperialBrief).valid, true);
assert.equal(getText4hRectangularBoundaryEligibility(imperialBrief).coverageRatio, 1);
assert.equal(getText4hRectangularBoundaryEligibility(imperialBrief).eligible, true);
assert.deepEqual(imperialBrief.dimensions.rectangularBoundary, { locked: true, source: 'automatic_area_match' });

const imperialPrompt = buildText4hImagePrompt(imperialBrief);
const masterPromptForUpload = buildText4hMasterFloorplanPrompt(imperialBrief, 'uploaded', undefined, { width: 6.096, depth: 9.144 });
...
assert.doesNotMatch(masterFloorplanSource, /localGeometryEvidence|from ['"].*localImageToJSON4h/i,
...
assert.match(wizardCoreSource, /aria-label="Gemini Master thinking level"/,
  'Gemini Master comparison must expose the thinking-level dropdown.');
assert.match(wizardCoreSource, /thinkingLevel: text4hMasterThinkingLevel/,
  'The selected Gemini Master thinking level must be used for conversion.');
assert.match(masterBackendSource, /location: "global"/);
assert.match(masterBackendSource, /MEDIA_RESOLUTION_HIGH/);
assert.match(masterBackendSource, /validateText4hMasterFloorplanGraph/);
const masterRouteSource = masterBackendSource.slice(
  masterBackendSource.indexOf("if (url.startsWith('/api/text4h/master-geometry'))"),
  masterBackendSource.indexOf('const ai = new GoogleGenAI({', masterBackendSource.indexOf("if (url.startsWith('/api/text4h/master-geometry'))") + 100),
);
...
assert.deepEqual(splitText4hDataUrl('data:image/png;base64,abc123'), { mimeType: 'image/png', data: 'abc123' });
const normalizedMasterData = normalizeText4hMasterFloorplanData({
  walls: [{
    p1: [0, 0],
    p2: [4, 0],
    type: 'exterior',
...
assert.equal(isText4hMasterFloorplanDataUsable(normalizedMasterData), false);
const usableMasterData = normalizeText4hMasterFloorplanData({
  walls: [
...
assert.equal(isText4hMasterFloorplanDataUsable(usableMasterData), true);
const ds77NormalizedMaster = normalizeText4hMasterFloorplanData({
...
const ds77WidthOnlyMaster = normalizeText4hMasterFloorplanData({
  coordinateSpace: 'normalized_0_1000',
  junctions: [
...
assert.equal(validateText4hMasterFloorplanGraph(invalidHostedGraph).valid, false,
  'An unhosted aperture must invalidate Gemini Master geometry.');
assert.match(validateText4hMasterFloorplanGraph(invalidHostedGraph).errors.join(' '), /existing host wall/i);
const invalidHostedResult = normalizeText4hMasterFloorplanData(invalidHostedGraph, {
...
const diagonalHostedMaster = normalizeText4hMasterFloorplanData({
  coordinateSpace: 'normalized_0_1000',
  junctions: [
    { id: 'J1', x: 0, y: 0 }, { id: 'J2', x: 1000, y: 0 },
    { id: 'J3', x: 1000, y: 1000 }, { id: 'J4', x: 0, y: 1000 },
...
assert.equal(validateText4hMasterFloorplanGraph(selfIntersectingGraph).valid, false);
assert.match(validateText4hMasterFloorplanGraph(selfIntersectingGraph).errors.join(' '), /self-intersects/i,
  'Self-intersecting exterior geometry must never be importable.');
const curvedHostedMaster = normalizeText4hMasterFloorplanData({
  coordinateSpace: 'normalized_0_1000',
  junctions: [
    { id: 'J1', x: 0, y: 0 }, { id: 'J2', x: 1000, y: 0 },
    { id: 'J3', x: 1000, y: 1000 }, { id: 'J4', x: 0, y: 1000 },
...
const orderedHeadings = [
  ...
  'STYLE text4h-digitizable-floorplan-v1:',
  'DIMENSIONS:',
];
let previousIndex = -1;
...
assert.match(imperialPrompt, /STYLE text4h-digitizable-floorplan-v1/);
...
const narrowDeepPrompt = buildText4hImagePrompt(narrowDeepBrief);
...
const requestedCasePrompt = buildText4hImagePrompt(requestedCaseBrief);
...
const creativeBrief = legacySummaryToText4hBrief(creativeSummary);
const creativeEligibility = getText4hRectangularBoundaryEligibility(creativeBrief);
...
assert.deepEqual(
  setText4hRectangularBoundaryLock(creativeBrief, true).dimensions.rectangularBoundary,
  { locked: false, source: 'ineligible' },
  'An incompatible area must not be manually rectangle-locked.',
);
const creativePrompt = buildText4hImagePrompt(creativeBrief);
...
const thresholdBrief = legacySummaryToText4hBrief(imperialSummary.replace("600 sq ft (20' x 30')", "950 sq ft (25' x 40')"));
assert.equal(getText4hRectangularBoundaryEligibility(thresholdBrief).eligible, true, '95% coverage must be rectangle-eligible.');
assert.equal(thresholdBrief.dimensions.rectangularBoundary.locked, true);
const belowThresholdBrief = legacySummaryToText4hBrief(imperialSummary.replace("600 sq ft (20' x 30')", "949 sq ft (25' x 40')"));
assert.equal(getText4hRectangularBoundaryEligibility(belowThresholdBrief).eligible, false);

const manualUnlock = setText4hRectangularBoundaryLock(requestedCaseBrief, false);
assert.deepEqual(manualUnlock.dimensions.rectangularBoundary, { locked: false, source: 'user_confirmed' });
const explicitLock = applyText4hRectangularBoundaryPolicy(manualUnlock, 'Use a rectangular boundary.');
assert.deepEqual(explicitLock.dimensions.rectangularBoundary, { locked: true, source: 'explicit_user_request' });
assert.equal(text4hHasExplicitRectangularRequest('Please make it rectangular.'), true);
assert.equal(text4hHasExplicitRectangularRequest('Use a non-rectangular creative plan.'), false);

const landscapePrompt = buildText4hImagePrompt({
  ...requestedCaseBrief,
  dimensions: {
...
const multiFloorPrompt = buildText4hImagePrompt({
  ...requestedCaseBrief,
...
for (const example of reportedExamples) {
  const prompt = buildText4hImagePrompt({
    ...requestedCaseBrief,
    dimensions: {
      ...requestedCaseBrief.dimensions,
...
const circularPrompt = buildText4hImagePrompt({
  ...creativeBrief,
  project: {
...
assert.deepEqual(TEXT4H_LOW_LATENCY_GENERATION_CONFIG.responseModalities, ['IMAGE']);
assert.equal(TEXT4H_LOW_LATENCY_GENERATION_CONFIG.thinkingConfig.thinkingLevel, 'minimal');
assert.equal(TEXT4H_LOW_LATENCY_GENERATION_CONFIG.thinkingConfig.includeThoughts, false);
assert.equal(TEXT4H_LOW_LATENCY_GENERATION_CONFIG.imageConfig.imageSize, '1K');
assert.equal(TEXT4H_LOW_LATENCY_GENERATION_CONFIG.imageConfig.aspectRatio, '1:1');
assert.equal(TEXT4H_LOW_LATENCY_GENERATION_CONFIG.candidateCount, 1);
assert.match(TEXT4H_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /hard rendering constraint/i);
assert.match(TEXT4H_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /within 2%/i);
assert.match(TEXT4H_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /Count balconies, terraces, porches, decks, steps, and railings/i);
assert.match(TEXT4H_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /Exclude annotations and every door leaf or swing arc/i);
assert.match(TEXT4H_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /RECTANGLE LOCKED/i);
assert.match(TEXT4H_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /CREATIVE UNLOCKED/i);
assert.match(TEXT4H_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /never force a rectangle by default/i);
assert.match(TEXT4H_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /Never render prompt instructions, pixel coordinates/i);
assert.match(TEXT4H_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /reserving clear white label zones/i);
assert.match(TEXT4H_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /one full Design Brief room name/i);
assert.match(TEXT4H_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /0\.33 times normal room-label height/i);
assert.match(TEXT4H_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /never abbreviate and never render room dimensions, room areas, label backgrounds, legends, or duplicate labels/i);
assert.match(TEXT4H_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /complete glyph box fits inside clear white floor space/i);
assert.match(TEXT4H_IMAGE_GEOMETRY_SYSTEM_INSTRUCTION, /No label glyph may touch, cross, obscure, or replace any wall, door leaf, swing arc, window, wall opening, column, stair, railing, fixture, or other architectural mark/i);
assert.match(text4hFeatureSource, /annotateGeneratedPreview:\s*false/, 'The normal D-generated preview must not re-add room dimensions after OCR.');

assert.equal(formatText4hRoomCrossDimensions(3.77, 2.77, 'ft'), `12'-4\" x 9'-1\"`);
assert.equal(formatText4hRoomCrossDimensions(3.77, 2.77, 'm'), '3.77 m x 2.77 m');
const deterministicRoomLabels = buildText4hRoomCrossDimensionLabels({
  walls: [
    { p1: [0, 0], p2: [4, 0], type: 'exterior' },
    { p1: [4, 0], p2: [4, 3], type: 'exterior' },
...
assert.equal(buildText4hRoomCrossDimensionLabels({
  walls: [
    { p1: [0, 0], p2: [4, 0], type: 'exterior' },
    { p1: [4, 0], p2: [4, 3], type: 'exterior' },
    { p1: [4, 3], p2: [0, 3], type: 'exterior' },
...
const detectedLockedFrame = detectText4hLockedStructuralFrame({
...
const lockedTarget = getText4hLockedImageAspectTarget(requestedCaseBrief);
assert.equal(lockedTarget, 0.625);
const lockedCorrection = planText4hLockedAspectCorrection(detectedLockedFrame, lockedTarget);
...
assert.equal(getText4hLockedImageAspectTarget(creativeBrief), undefined, 'Creative-unlocked plans must bypass deterministic axis correction.');
assert.equal(planText4hLockedAspectCorrection({ ratio: 0.63 }, 0.625).applied, false);
assert.equal(getText4hLockedImageAspectTarget({
  ...requestedCaseBrief,
  project: { ...requestedCaseBrief.project, floors: 2 },
...
const metricBrief = legacySummaryToText4hBrief(metricSummary);
const metricPrompt = buildText4hImagePrompt(metricBrief);
...
const editedSummary = text4hBriefToDesignSummary(imperialBrief).replace("20'-0\" x 30'-0\"", "24'-0\" x 25'-0\"");
const editedBrief = updateText4hBriefFromSummary(editedSummary, imperialBrief, 'dimensions');
...
assert.match(buildText4hImagePrompt(editedBrief), /width 24'-0" above; depth 25'-0" left/);

const boundary = text4hBriefBoundaryMeters(imperialBrief);
assert.ok(boundary);
assert.ok(Math.abs(boundary[1].x - 6.096) < 0.000001);
assert.ok(Math.abs(boundary[2].y - 9.144) < 0.000001);

const blankDirectUpload = validateText4hDirectUploadScale({ unitSystem: 'imperial', width: '', depth: '', area: '' });
assert.equal(blankDirectUpload.valid, false, 'A direct upload must provide at least one dimension.');
const partialDirectUpload = prepareText4hDirectUpload(imperialBrief, { unitSystem: 'imperial', width: '25', depth: '', area: '' });
...
const verticalOnlyDirectUpload = prepareText4hDirectUpload(imperialBrief, { unitSystem: 'imperial', width: '', depth: '40', area: '' });
...
const areaOnlyDirectUpload = prepareText4hDirectUpload(imperialBrief, { unitSystem: 'imperial', width: '', depth: '', area: '1000' });
...
const dimensionsOnlyDirectUpload = prepareText4hDirectUpload(imperialBrief, { unitSystem: 'imperial', width: '25', depth: '40', area: '' });
...
const architecturalDirectUpload = prepareText4hDirectUpload(imperialBrief, { unitSystem: 'imperial', width: `25'-6"`, depth: '40 ft 0 in', area: '1,000 sq ft' });
...
const shorthandDirectUpload = validateText4hDirectUploadScale({ unitSystem: 'imperial', width: '36-6', depth: `26'-6"`, area: '1000 sft' });
...
const metricDirectUpload = prepareText4hDirectUpload(imperialBrief, { unitSystem: 'metric', width: '7.5', depth: '12', area: '90' });
...
const formattedMetricDirectUpload = validateText4hDirectUploadScale({ unitSystem: 'metric', width: '762 cm', depth: '12,192 mm', area: '92.9 m²' });
...
const wrongUnitDirectUpload = validateText4hDirectUploadScale({ unitSystem: 'imperial', width: '7.62 m', depth: '12.19 m', area: '' });
...
const fallback = buildText4hFallbackGeometry(imperialBrief, boundary, new Error('synthetic trace failure'));
...
const invalidBrief = legacySummaryToText4hBrief('Purpose: House\nRooms Included:\n- 1 Living Room\nFloors: Single-story');
assert.equal(validateText4hBrief(invalidBrief).valid, false);
assert.ok(validateText4hBrief(invalidBrief).errors.some(error => /width and depth/i.test(error)));

console.log(`Text 4.0 H canonical brief and prompt regression tests passed (${imperialPromptWords} prompt words).`);
