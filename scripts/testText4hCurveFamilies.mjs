...
} = await load('services/localImageToJSON4h.ts');
const {
  consolidateText4hCurveArcs,
  text4hCurveArcSourceEnvelopeCoherentForTest,
} = await load('services/text4hCurveArcs.ts');
...
const recoveredRasterRing = recoverText4hConcentricInteriorRingForTest(
...
const retainedRing = consolidateText4hCurveArcs(ring, {
...
assert.equal(text4hCurveArcSourceEnvelopeCoherentForTest(ringArc, ring, size, size, 12), true,
...
assert.equal(text4hDominantEllipseGapHasRasterEvidenceForTest(0.5, 0), true,
  'Observed structural ink must continue a proven dominant ellipse.');
assert.equal(text4hDominantEllipseGapHasRasterEvidenceForTest(0.24, 0.6), true,
