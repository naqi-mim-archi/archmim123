...
type LengthUnit = ConfirmedText4dBrief['dimensions']['envelope']['unit'];
...
export interface Text4dRoomCrossDimensionLabel {
  label: string;
...
export const formatText4dRoomCrossDimensions = (
  widthMeters: number,
  depthMeters: number,
  unit: LengthUnit,
): string => unit === 'ft'
...
export const buildText4dRoomCrossDimensionLabels = (
  geometry: GeneratedData,
  unit: LengthUnit,
): Text4dRoomCrossDimensionLabel[] => {
  ...
  return (geometry.rooms || []).flatMap(room => {
    ...
    return [{
      ...
      widthMeters,
      depthMeters,
      dimensionText: formatText4dRoomCrossDimensions(widthMeters, depthMeters, unit),
    }];
  });
};
...
const loadImage = (dataUrl: string): Promise<HTMLImageElement> => new Promise((resolve, reject) => {
  ...
  image.onerror = () => reject(new Error('Generated Text 4.0 D image could not be decoded for room annotation.'));
  image.src = dataUrl;
});
 ...
 */
export const annotateText4dGeneratedImageRooms = async (
  dataUrl: string,
  geometry: GeneratedData,
  ...
  const labels = buildText4dRoomCrossDimensionLabels(geometry, brief.dimensions.envelope.unit);
