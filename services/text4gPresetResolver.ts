/**
 * Conservative preset resolution for Text 4.0 G digitization.
 *
...
export const TEXT4G_WALL_PRESETS = [
...
export const TEXT4G_DOOR_PRESETS = [
...
export const TEXT4G_WINDOW_PRESETS = [
...
export type Text4gEvidenceStrength = 'strong' | 'medium' | 'weak' | 'none';
export type Text4gResolutionConfidence = 'high' | 'medium' | 'low' | 'unresolved';
export type Text4gResolutionProvenance =
  | 'measured-preset'
  | 'measured-custom'
  | 'role-default'
  | 'semantic-default'
  | 'unresolved';
export type Text4gWallRole = 'exterior' | 'interior' | 'structural-interior' | 'partition' | 'glass';

export interface Text4gMeasuredValues {
  widthM: number | null;
  heightM: number | null;
  thicknessM: number | null;
}

export interface Text4gPresetResolution {
  kind: 'wall' | 'door' | 'window';
  presetId: string | null;
  presetLabel: string | null;
  subtype: string;
  measuredValues: Text4gMeasuredValues;
  resolvedValues: Text4gMeasuredValues;
  confidence: Text4gResolutionConfidence;
  provenance: Text4gResolutionProvenance;
...
export interface ResolveText4gWallInput {
  semanticRole: Text4gWallRole;
  projectClass?: string | null;
  measuredThicknessM?: number | null;
  evidenceStrength?: Text4gEvidenceStrength;
  /** Required for measured-to-preset snapping. No implicit tolerance is applied. */
  toleranceM?: number | null;
}

export interface ResolveText4gDoorInput {
...
export interface ResolveText4gWindowInput {
  measuredWidthM?: number | null;
  measuredHeightM?: number | null;
  detectedSubtype?: string | null;
  evidenceStrength?: Text4gEvidenceStrength;
...
const evidenceConfidence = (
  strength: Text4gEvidenceStrength,
  unresolved = false,
): Text4gResolutionConfidence => {
...
const emptyValues = (): Text4gMeasuredValues => ({ widthM: null, heightM: null, thicknessM: null });
...
const wallCandidates = (role: Text4gWallRole): readonly (typeof TEXT4G_WALL_PRESETS)[number][] => {
  if (role === 'exterior') return TEXT4G_WALL_PRESETS.filter(preset => preset.subtype === 'exterior');
  return TEXT4G_WALL_PRESETS.filter(preset => preset.subtype === role);
};

const semanticWallPresetId = (role: Text4gWallRole, projectClass?: string | null): string => {
...
export const resolveText4gWallPreset = (input: ResolveText4gWallInput): Text4gPresetResolution => {
  ...
  const measuredValues: Text4gMeasuredValues = { ...emptyValues(), thicknessM: measuredThicknessM };
  ...
  const preset = TEXT4G_WALL_PRESETS.find(candidate => candidate.id === presetId)!;
...
export const resolveText4gDoorPreset = (input: ResolveText4gDoorInput): Text4gPresetResolution => {
  ...
  const measuredValues: Text4gMeasuredValues = {
    ...emptyValues(),
  ...
  if (measuredWidthM !== null) {
    const knownSubtype = TEXT4G_DOOR_PRESETS.some(preset => preset.subtype === detectedSubtype);
    const candidates = knownSubtype
      ? TEXT4G_DOOR_PRESETS.filter(preset => preset.subtype === detectedSubtype)
      : detectedSubtype ? [] : [...TEXT4G_DOOR_PRESETS];
  ...
  const rolePreset = rolePresetId
    ? TEXT4G_DOOR_PRESETS.find(candidate => candidate.id === rolePresetId) || null
    : null;
  if (rolePreset) {
    return {
      kind: 'door',
...
export const resolveText4gWindowPreset = (input: ResolveText4gWindowInput): Text4gPresetResolution => {
  const measuredWidthM = positiveMeasurement(input.measuredWidthM);
  const measuredHeightM = positiveMeasurement(input.measuredHeightM);
  const toleranceM = explicitTolerance(input.toleranceM);
  const evidence = input.evidenceStrength || 'none';
  ...
  if (measuredWidthM !== null) {
    const knownSubtype = TEXT4G_WINDOW_PRESETS.some(preset => preset.subtype === detectedSubtype);
    ...
    const candidates = knownSubtype
      ? TEXT4G_WINDOW_PRESETS.filter(preset => preset.subtype === detectedSubtype)
      : detectedSubtype ? [] : TEXT4G_WINDOW_PRESETS.filter(preset => preset.subtype === 'regular');
    const nearest = candidates
      .map(preset => {
