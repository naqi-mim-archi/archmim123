import type {
  Text4fFreeformWallGap,
  Text4fFreeformWallSegment,
  Text4fGeometryMode,
  Text4fPixelPoint,
} from './text4fGeometryModes';

export interface Text4fNativeArcRun {
  sourceIndices: number[];
  p1: Text4fPixelPoint;
  p2: Text4fPixelPoint;
  controlPoint: Text4fPixelPoint;
...
interface CurveArcOptions {
  ...
  exteriorEvidence?: (segment: Text4fFreeformWallSegment) => boolean;
  /** Raster-proven apertures whose temporary endpoint chord is only a host locator. */
  hostedGaps?: Text4fFreeformWallGap[];
}

interface OrderedRun {
  points: Text4fPixelPoint[];
  sourceIndices: number[];
  closed: boolean;
}
...
interface CurveFamilyModel {
  ...
  pointAt: (angle: number) => Text4fPixelPoint;
  residual: (point: Text4fPixelPoint) => number;
  tangentSupport: (segment: Text4fFreeformWallSegment) => number;
}

const TAU = Math.PI * 2;
const clamp = (value: number, min: number, max: number) => Math.max(min, Math.min(max, value));
const distance = (a: Text4fPixelPoint, b: Text4fPixelPoint) => Math.hypot(b.x - a.x, b.y - a.y);
...
const leastSquaresCircle = (points: Text4fPixelPoint[]): CircleFit | undefined => {
...
const circleThroughThreePoints = (
  start: Text4fPixelPoint,
  middle: Text4fPixelPoint,
  end: Text4fPixelPoint,
) => {
  const determinant = 2 * (
...
const rasterSupport = (
  fit: { center: Text4fPixelPoint; radius: number; startAngle: number; endAngle: number; counterclockwise: boolean },
  mask: Uint8Array,
  width: number,
  height: number,
  thickness: number,
...
const fitAxisEllipse = (points: Text4fPixelPoint[]): CurveFamilyModel | undefined => {
  ...
  const basis = (point: Text4fPixelPoint) => {
  ...
  const residual = (point: Text4fPixelPoint) => {
  ...
  const tangentSupport = (segment: Text4fFreeformWallSegment) => {
    const midpoint = {
...
const curveParameter = (model: CurveFamilyModel, point: Text4fPixelPoint) =>
...
const fitSeparatedCurveFamilies = (
  ...
  for (let family = 0; family < 4 && remaining.size >= 4; family++) {
    ...
    if (!candidate) {
      ...
      const segmentDistance = (first: Text4fFreeformWallSegment, second: Text4fFreeformWallSegment) => Math.min(
...
const buildRuns = (segments: Text4fFreeformWallSegment[], typicalThickness: number): OrderedRun[] => {
  const eligible = segments.map((segment, index) => ({ segment, index })).filter(({ segment }) =>
    !segment.bridge
...
export const consolidateText4fCurveArcs = (
  segments: Text4fFreeformWallSegment[],
  options: CurveArcOptions,
): { arcs: Text4fNativeArcRun[]; retainedSegments: Text4fFreeformWallSegment[] } => {
  ...
  let fittedFamilyArcs = fitSeparatedCurveFamilies(segments, options);
  const ellipseFamilies = fittedFamilyArcs.filter(arc => arc.ellipseRadiusX !== undefined && arc.ellipseRadiusY !== undefined);
  const dominantExteriorFamily = [...ellipseFamilies].sort((first, second) =>
    Math.min(second.ellipseRadiusX!, second.ellipseRadiusY!) - Math.min(first.ellipseRadiusX!, first.ellipseRadiusY!))[0];
  if (options.exteriorEvidence && dominantExteriorFamily) {
    const dominantRadiusX = dominantExteriorFamily.ellipseRadiusX!;
    const dominantRadiusY = dominantExteriorFamily.ellipseRadiusY!;
    const sameDominantFamily = (arc: Text4fNativeArcRun) => arc.ellipseRadiusX !== undefined
      && distance(arc.center, dominantExteriorFamily.center) <= options.typicalThickness * 0.8
      && Math.abs(arc.ellipseRadiusX - dominantRadiusX) <= options.typicalThickness * 1.2
      && Math.abs(arc.ellipseRadiusY! - dominantRadiusY) <= options.typicalThickness * 1.2;
    const fullyInsideDominantShell = (arc: Text4fNativeArcRun) => {
      if (arc.ellipseRadiusX === undefined || sameDominantFamily(arc)) return false;
      let span = arc.ellipseCounterclockwise
        ? arc.ellipseStartAngle! - arc.ellipseEndAngle!
        : arc.ellipseEndAngle! - arc.ellipseStartAngle!;
      while (span < 0) span += TAU;
      const middleAngle = arc.ellipseCounterclockwise
        ? arc.ellipseStartAngle! - span / 2
        : arc.ellipseStartAngle! + span / 2;
      const midpoint = {
        x: arc.center.x + Math.cos(middleAngle) * arc.ellipseRadiusX,
        y: arc.center.y + Math.sin(middleAngle) * arc.ellipseRadiusY!,
      };
      const margin = options.typicalThickness * 1.8 / Math.max(1, Math.min(dominantRadiusX, dominantRadiusY));
      return [arc.p1, midpoint, arc.p2].every(point => Math.hypot(
        (point.x - dominantExteriorFamily.center.x) / dominantRadiusX,
        (point.y - dominantExteriorFamily.center.y) / dominantRadiusY,
      ) <= 1 - margin);
    };
    // When the main shell is strongly fitted, a second ellipse wholly inside
    // it is an interior partition/furniture tangent, not another exterior
    // facade. Return those source segments to the common straight/local-arc
    // path instead of exporting false overlapping exterior walls.
    fittedFamilyArcs = fittedFamilyArcs.filter(arc => !fullyInsideDominantShell(arc));
  }
  const localDepartureTolerance = Math.max(1.5, options.typicalThickness * 0.45);
  const localSamples = (arc: Text4fNativeArcRun) => [0, 0.25, 0.5, 0.75, 1]
    .map(t => pointOnArc(arc.center, arc.radius, arc.startAngle, arc.endAngle, arc.counterclockwise, t));
  const familyResidual = (family: Text4fNativeArcRun, point: Text4fPixelPoint) => {
  ...
  const arcs = [...familyArcs, ...contradictoryLocalArcs, ...orderedArcs].filter(arc => {
    if (options.mode !== 'hybrid' || !options.exteriorEvidence) return true;
    const exteriorCount = arc.sourceIndices.filter(index => segments[index] && options.exteriorEvidence!(segments[index])).length;
    return exteriorCount >= Math.max(1, Math.ceil(arc.sourceIndices.length * 0.35));
  });
  const consolidated = new Set(arcs.flatMap(arc => arc.sourceIndices));
  const curveCoversSegment = (segment: Text4fFreeformWallSegment, arc: Text4fNativeArcRun) => {
