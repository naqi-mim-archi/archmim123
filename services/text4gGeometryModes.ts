export type Text4gGeometryMode = 'orthogonal' | 'angular' | 'curved' | 'hybrid';

export interface Text4gPixelPoint {
  x: number;
  y: number;
}

export interface Text4gFreeformWallSegment {
  p1: Text4gPixelPoint;
  p2: Text4gPixelPoint;
  thickness: number;
  confidence: number;
...
export interface Text4gFreeformWallGap {
  p1: Text4gPixelPoint;
  p2: Text4gPixelPoint;
  thickness: number;
  confidence: number;
...
export interface Text4gGeometryModeAnalysis {
  mode: Text4gGeometryMode;
  segments: Text4gFreeformWallSegment[];
  gaps: Text4gFreeformWallGap[];
  nonOrthogonalLength: number;
  structuralLength: number;
  confidence: number;
}

export interface Text4gFreeformGapEvidence {
  doorLeaf: boolean;
  windowFrame: boolean;
  confidence: number;
  hingeAtEnd: boolean;
...
export interface Text4gSparseAngularStair {
  p1: Text4gPixelPoint;
  p2: Text4gPixelPoint;
  widthPixels: number;
  stepCount: number;
...
export interface Text4gSparseAngularColumn {
  x: number;
  y: number;
  widthPixels: number;
  depthPixels: number;
...
const distance = (a: Text4gPixelPoint, b: Text4gPixelPoint) => Math.hypot(b.x - a.x, b.y - a.y);
const pointKey = (point: Text4gPixelPoint) => `${point.x},${point.y}`;
const direction = (a: Text4gPixelPoint, b: Text4gPixelPoint) => {
  const length = Math.max(1e-6, distance(a, b));
  return { x: (b.x - a.x) / length, y: (b.y - a.y) / length };
};
const dot = (a: Text4gPixelPoint, b: Text4gPixelPoint) => a.x * b.x + a.y * b.y;
const angleDistanceFromAxis = (segment: Pick<Text4gFreeformWallSegment, 'p1' | 'p2'>) => {
...
interface ThinSegment { p1: Text4gPixelPoint; p2: Text4gPixelPoint; }
...
export const detectText4gSparseAngularStairs = (
  ...
  const stairs: Text4gSparseAngularStair[] = [];
...
export const detectText4gSparseAngularRailings = (
...
export const detectText4gSparseAngularColumns = (
  structuralMask: Uint8Array,
  width: number,
  height: number,
  typicalThickness: number,
  ...
  const columns: Text4gSparseAngularColumn[] = [];
  unique.forEach(point => {
    ...
    const inspectCenter = (center: Text4gPixelPoint) => {
...
const rdp = (points: Text4gPixelPoint[], tolerance: number): Text4gPixelPoint[] => {
...
const simplifyPath = (points: Text4gPixelPoint[], tolerance: number, closed: boolean) => {
...
const traceSkeleton = (skeleton: Uint8Array, width: number, height: number) => {
  ...
  const toPoint = (index: number): Text4gPixelPoint => ({ x: index % width, y: Math.floor(index / width) });
...
const pathLength = (points: Text4gPixelPoint[]) => points.slice(1)
...
export const analyzeText4gGeometryMode = (
  sourceMask: Uint8Array,
  sourceWidth: number,
  sourceHeight: number,
  typicalThicknessHint: number,
): Text4gGeometryModeAnalysis => {
  ...
  const segments: Text4gFreeformWallSegment[] = [];
  interface PathEndpoint { point: Text4gPixelPoint; inner: Text4gPixelPoint; component: number; }
  ...
  const gaps: Text4gFreeformWallGap[] = [];
  ...
  const cross = (origin: Text4gPixelPoint, a: Text4gPixelPoint, b: Text4gPixelPoint) =>
    (a.x - origin.x) * (b.y - origin.y) - (a.y - origin.y) * (b.x - origin.x);
  const lower: Text4gPixelPoint[] = [];
  hullPoints.forEach(point => {
    while (lower.length >= 2 && cross(lower[lower.length - 2], lower[lower.length - 1], point) <= 0) lower.pop();
    lower.push(point);
  });
  const upper: Text4gPixelPoint[] = [];
  [...hullPoints].reverse().forEach(point => {
    while (upper.length >= 2 && cross(upper[upper.length - 2], upper[upper.length - 1], point) <= 0) upper.pop();
    upper.push(point);
  });
  const hull = [...lower.slice(0, -1), ...upper.slice(0, -1)];
  const pointToSegmentDistance = (point: Text4gPixelPoint, a: Text4gPixelPoint, b: Text4gPixelPoint) => {
  ...
  const mode: Text4gGeometryMode = nonOrthogonalRatio >= 0.76
...
const sampleLineSupport = (
  ...
  width: number,
  height: number,
  origin: Text4gPixelPoint,
  vector: Text4gPixelPoint,
  length: number,
  radius = 1,
) => {
...
export const inspectText4gFreeformGap = (
  gap: Text4gFreeformWallGap,
  structuralMask: Uint8Array,
  doorEvidenceMask: Uint8Array,
  width: number,
  height: number,
): Text4gFreeformGapEvidence => {
 ...
 */
export const recoverText4gSupplementalAngularDoorHosts = (
  structuralMask: Uint8Array,
  doorEvidenceMask: Uint8Array,
  width: number,
  height: number,
  typicalThickness: number,
  observedSegments: Text4gFreeformWallSegment[],
  observedGaps: Text4gFreeformWallGap[],
  ...
  interface Endpoint {
    point: Text4gPixelPoint;
    inner: Text4gPixelPoint;
  ...
  for (let firstIndex = 0; firstIndex < endpoints.length; firstIndex++) {
    for (let secondIndex = firstIndex + 1; secondIndex < endpoints.length; secondIndex++) {
      ...
      const evidence = inspectText4gFreeformGap(gap, structuralMask, doorEvidenceMask, width, height);
  ...
  return {
    segments: selected.map(candidate => ({
      ...
      bridge: true,
    } satisfies Text4gFreeformWallSegment)),
 ...
 */
export const recoverText4gFaintAngularWindowHosts = (
  structuralMask: Uint8Array,
  ...
  const faintPixels: Text4gPixelPoint[] = [];
  ...
  const maskNear = (point: Text4gPixelPoint, radius: number) => {
  ...
  canonicalAngles.forEach(angle => {
    ...
    for (let first = 0; first < runs.length; first++) for (let second = first + 1; second < runs.length; second++) {
      ...
      const toPoint = (along: number): Text4gPixelPoint => ({
        x: tangent.x * along + normal.x * hostRho,
        y: tangent.y * along + normal.y * hostRho,
      });
