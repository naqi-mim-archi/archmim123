...
export async function extractZoningFromImage4b(base64Image: string): Promise<ExtractedZoningData> {
  const base64Data = base64Image.replace(/^data:image\/(png|jpeg);base64,/, "");

  console.log('[Text 4.0 B] Extracting zoning via Gemini Vision using spatial boxes...');
  const startTime = Date.now();

  try {
    const response = await ai.models.generateContent({
      model: 'tunedModels/2185998999155113984',
      contents: [
        {
          role: 'user',
          parts: [
    ...
    console.log(`[Text 4.0 B] Vision extraction completed in ${elapsed}ms`);
    ...
    console.error('[Text 4.0 B] Vision Extraction Error:', error);
