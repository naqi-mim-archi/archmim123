...
export type Text4fDirectUploadUnitSystem = 'imperial' | 'metric';

export interface Text4fDirectUploadScaleInput {
  unitSystem: Text4fDirectUploadUnitSystem;
  width: string;
  depth: string;
  area: string;
}

export interface Text4fDirectUploadValidation {
  valid: boolean;
  errors: string[];
...
const propertyArea = (value: string, unitSystem: Text4fDirectUploadUnitSystem): number | undefined => {
  const normalized = normalizeMeasurement(value);
  if (!normalized) return undefined;
...
export const validateText4fDirectUploadScale = (
  input: Text4fDirectUploadScaleInput,
): Text4fDirectUploadValidation => {
  const errors: string[] = [];
...
export const prepareText4fDirectUpload = (
  sourceBrief: ConfirmedText4fBrief,
  input: Text4fDirectUploadScaleInput,
): PreparedText4fDirectUpload => {
  const validation = validateText4fDirectUploadScale(input);
