...
export type Text4gDirectUploadUnitSystem = 'imperial' | 'metric';

export interface Text4gDirectUploadScaleInput {
  unitSystem: Text4gDirectUploadUnitSystem;
  width: string;
  depth: string;
  area: string;
}

export interface Text4gDirectUploadValidation {
  valid: boolean;
  errors: string[];
...
const propertyArea = (value: string, unitSystem: Text4gDirectUploadUnitSystem): number | undefined => {
  const normalized = normalizeMeasurement(value);
  if (!normalized) return undefined;
...
export const validateText4gDirectUploadScale = (
  input: Text4gDirectUploadScaleInput,
): Text4gDirectUploadValidation => {
  const errors: string[] = [];
...
export const prepareText4gDirectUpload = (
  sourceBrief: ConfirmedText4gBrief,
  input: Text4gDirectUploadScaleInput,
): PreparedText4gDirectUpload => {
  const validation = validateText4gDirectUploadScale(input);
