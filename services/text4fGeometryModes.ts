export type Text4fGeometryMode = 'orthogonal' | 'angular' | 'curved' | 'hybrid';

export interface Text4fPixelPoint {
  x: number;
  y: number;
}

export interface Text4fFreeformWallSegment {
  p1: Text4fPixelPoint;
  p2: Text4fPixelPoint;
  thickness: number;
  confidence: number;
...
export interface Text4fFreeformWallGap {
  p1: Text4fPixelPoint;
  p2: Text4fPixelPoint;
  thickness: number;
  confidence: number;
...
export interface Text4fGeometryModeAnalysis {
  mode: Text4fGeometryMode;
  segments: Text4fFreeformWallSegment[];
  gaps: Text4fFreeformWallGap[];
  nonOrthogonalLength: number;
  structuralLength: number;
  confidence: number;
}

export interface Text4fFreeformGapEvidence {
  doorLeaf: boolean;
  windowFrame: boolean;
  confidence: number;
  hingeAtEnd: boolean;
...
export interface Text4fSparseAngularStair {
  p1: Text4fPixelPoint;
  p2: Text4fPixelPoint;
  widthPixels: number;
  stepCount: number;
...
export interface Text4fSparseAngularColumn {
  x: number;
  y: number;
  widthPixels: number;
  depthPixels: number;
...
const pointKey = (point: Text4fPixelPoint) => `${point.x},${point.y}`;
const direction = (a: Text4fPixelPoint, b: Text4fPixelPoint) => {
  const length = Math.max(1e-6, distance(a, b));
  return { x: (b.x - a.x) / length, y: (b.y - a.y) / length };
};
const dot = (a: Text4fPixelPoint, b: Text4fPixelPoint) => a.x * b.x + a.y * b.y;
const angleDistanceFromAxis = (segment: Pick<Text4fFreeformWallSegment, 'p1' | 'p2'>) => {
...
interface ThinSegment { p1: Text4fPixelPoint; p2: Text4fPixelPoint; }
...
export const detectText4fSparseAngularStairs = (
  ...
  const stairs: Text4fSparseAngularStair[] = [];
...
export const detectText4fSparseAngularRailings = (
...
export const detectText4fSparseAngularColumns = (
  structuralMask: Uint8Array,
  width: number,
  height: number,
  typicalThickness: number,
  ...
  const columns: Text4fSparseAngularColumn[] = [];
  unique.forEach(point => {
    ...
    const inspectCenter = (center: Text4fPixelPoint) => {
...
const rdp = (points: Text4fPixelPoint[], tolerance: number): Text4fPixelPoint[] => {
...
const simplifyPath = (points: Text4fPixelPoint[], tolerance: number, closed: boolean) => {
...
const traceSkeleton = (skeleton: Uint8Array, width: number, height: number) => {
  ...
  const toPoint = (index: number): Text4fPixelPoint => ({ x: index % width, y: Math.floor(index / width) });
...
const pathLength = (points: Text4fPixelPoint[]) => points.slice(1)
...
export const analyzeText4fGeometryMode = (
  sourceMask: Uint8Array,
  sourceWidth: number,
  sourceHeight: number,
  typicalThicknessHint: number,
): Text4fGeometryModeAnalysis => {
  ...
  const segments: Text4fFreeformWallSegment[] = [];
  interface PathEndpoint { point: Text4fPixelPoint; inner: Text4fPixelPoint; component: number; }
  const endpoints: PathEndpoint[] = [];
  paths.filter(path => acceptedComponents.has(path.component)).forEach(path => {
    ...
    if (!path.closed && simplified.length >= 2) {
  ...
  const gaps: Text4fFreeformWallGap[] = [];
  ...
  candidates.sort((a, b) => a.distance - b.distance || b.alignment - a.alignment).forEach(candidate => {
    ...
    segments.push({ p1, p2, thickness: detectedThickness, confidence: 0.72, bridge: true });
  ...
  const cross = (origin: Text4fPixelPoint, a: Text4fPixelPoint, b: Text4fPixelPoint) =>
    (a.x - origin.x) * (b.y - origin.y) - (a.y - origin.y) * (b.x - origin.x);
  const lower: Text4fPixelPoint[] = [];
  hullPoints.forEach(point => {
    while (lower.length >= 2 && cross(lower[lower.length - 2], lower[lower.length - 1], point) <= 0) lower.pop();
    lower.push(point);
  });
  const upper: Text4fPixelPoint[] = [];
  [...hullPoints].reverse().forEach(point => {
    while (upper.length >= 2 && cross(upper[upper.length - 2], upper[upper.length - 1], point) <= 0) upper.pop();
    upper.push(point);
  });
  const hull = [...lower.slice(0, -1), ...upper.slice(0, -1)];
  const pointToSegmentDistance = (point: Text4fPixelPoint, a: Text4fPixelPoint, b: Text4fPixelPoint) => {
  ...
  if (!sufficientEvidence) {
    return { mode: 'orthogonal', segments: [], gaps: [], nonOrthogonalLength, structuralLength, confidence: 0.92 };
  }
  ...
  const mode: Text4fGeometryMode = nonOrthogonalRatio >= 0.76
...
const sampleLineSupport = (
  ...
  width: number,
  height: number,
  origin: Text4fPixelPoint,
  vector: Text4fPixelPoint,
  length: number,
  radius = 1,
) => {
...
export const inspectText4fFreeformGap = (
  gap: Text4fFreeformWallGap,
  structuralMask: Uint8Array,
  doorEvidenceMask: Uint8Array,
  width: number,
  height: number,
): Text4fFreeformGapEvidence => {
 ...
 */
export const recoverText4fSupplementalAngularDoorHosts = (
  structuralMask: Uint8Array,
  doorEvidenceMask: Uint8Array,
  width: number,
  height: number,
  typicalThickness: number,
  observedSegments: Text4fFreeformWallSegment[],
  observedGaps: Text4fFreeformWallGap[],
  ...
  interface Endpoint {
    point: Text4fPixelPoint;
    inner: Text4fPixelPoint;
  ...
  for (let firstIndex = 0; firstIndex < endpoints.length; firstIndex++) {
    for (let secondIndex = firstIndex + 1; secondIndex < endpoints.length; secondIndex++) {
      ...
      const evidence = inspectText4fFreeformGap(gap, structuralMask, doorEvidenceMask, width, height);
  ...
  return {
    segments: selected.map(candidate => ({
      ...
      bridge: true,
    } satisfies Text4fFreeformWallSegment)),
 ...
 */
export const recoverText4fFaintAngularWindowHosts = (
  structuralMask: Uint8Array,
  ...
  const faintPixels: Text4fPixelPoint[] = [];
  ...
  const maskNear = (point: Text4fPixelPoint, radius: number) => {
  ...
  canonicalAngles.forEach(angle => {
    ...
    for (let first = 0; first < runs.length; first++) for (let second = first + 1; second < runs.length; second++) {
      ...
      const toPoint = (along: number): Text4fPixelPoint => ({
        x: tangent.x * along + normal.x * hostRho,
        y: tangent.y * along + normal.y * hostRho,
      });
