...
export interface LocalImageExtractionOptions {
  ...
  /** Text 4.0 H uses its confirmed width and depth as independent, authoritative axes. */
...
interface Text4hHostedAperture {
  pos: number[];
  rotation?: number;
  width?: number;
 ...
 */
export const text4hHostedAperturesOverlap = (
  first: Text4hHostedAperture,
  second: Text4hHostedAperture,
) => {
...
export const arbitrateText4hHostedOpenings = <T extends Text4hHostedAperture>(
  doors: Text4hHostedAperture[],
  openings: T[],
) => {
  const ranked = openings
    .filter(opening => !doors.some(door => text4hHostedAperturesOverlap(door, opening)))
  ...
  return ranked.filter((opening, index) => !ranked.slice(0, index).some(retained =>
    text4hHostedAperturesOverlap(retained, opening)));
};

interface Text4hOpeningHostWall {
  p1: number[];
  p2: number[];
  wallSource?: string;
  isCurved?: boolean;
...
const text4hHostWallPoint = (wall: Text4hOpeningHostWall, progress: number) => {
...
export const text4hOpeningHasHost = (
  opening: Pick<Text4hHostedAperture, 'pos'>,
  walls: Text4hOpeningHostWall[],
  tolerance = 0.22,
) => walls.some(wall => {
  const curved = wall.wallSource === 'arc' || wall.wallSource === 'ellipse' || wall.isCurved;
  const sampleCount = curved ? 96 : 1;
  let previous = text4hHostWallPoint(wall, 0);
  for (let index = 1; index <= sampleCount; index++) {
    const current = text4hHostWallPoint(wall, index / sampleCount);
    ...
    image.onload = async () => {
      try {
        ...
        if (!textObservations && !options.disableOcr) {
          ...
          const ocrTask = recognizeFloorplanText4h(canvas);
          if (options.onGeometryReady) {
            try {
              ...
              console.warn('[Text 4.0 H] Early raster preview was unavailable; continuing to final digitization.', error);
            }
          }
          try {
            ...
            if (result === timeout) {
              console.warn(`[Text 4.0 H] Local OCR exceeded its ${budgetMs}ms budget; continuing with raster and design-brief signals.`);
              textObservations = [];
              ocrStatus = 'timed-out';
            } else {
            ...
            ocrDurationMs = performance.now() - ocrStartedAt;
            console.warn('[Text 4.0 H] Local OCR was unavailable; continuing with raster and design-brief signals.', error);
            textObservations = [];
 ...
 */
export const arbitrateText4hHostedWindows = <T extends Text4hHostedAperture>(
  doors: Text4hHostedAperture[],
  windows: T[],
...
export const createText4hDashedOpeningEvidenceMaskForTest = createDashedOpeningEvidenceMask;
...
export const resolveText4hMissingHostRecoveryScaleForTest = resolveMissingHostRecoveryScale;
...
export const retainText4hCooperatingExteriorWallNetworksForTest = retainMainWallNetwork;
...
const rasterizeNativeText4hArcs = (
  barrier: Uint8Array,
  arcs: Array<{
...
const freeformBounds = (segments: Text4hFreeformWallSegment[]): PixelBounds | undefined => {
...
const freeformCoveredByAxisWall = (
  segment: Text4hFreeformWallSegment,
...
const isFreeformExterior = (
  segment: Pick<Text4hFreeformWallSegment, 'p1' | 'p2' | 'thickness'>,
  outside: Uint8Array,
  width: number,
  height: number,
) => {
...
export const findText4hExplicitHostedOffsetSwingForTest = findExplicitHostedOffsetSwing;
 ...
 */
export const text4hClosedLeafFitsHostForTest = (
  leafLength: number,
  hostThickness: number,
) => leafLength <= hostThickness * 8;

export const text4hSwingQuadrantForTest = (
  axis: PixelGap['axis'],
  isFlipped: boolean,
  facingFlipped: boolean,
) => {
...
const recoverClosedSwingDoorGaps = (
  ...
  symbolSegments.forEach(leaf => {
    ...
    const hosts = hostSegments.filter(host => host.axis === leaf.axis
      ...
      && text4hClosedLeafFitsHostForTest(leafLength, host.thickness)
...
export const findText4hPhysicalWallFaceSwingForTest = (
...
export const recoverText4hLongWallEndpointSwingHostsForTest = recoverLongWallEndpointSwingDoorHosts;
...
export const text4hParallelDashedOpeningBandCountForTest = parallelDashedOpeningBandCount;
...
interface Text4hHostedNonDoorRasterEvidence {
  exterior: boolean;
...
export const resolveText4hHostedNonDoorRasterKindForTest = (
  evidence: Text4hHostedNonDoorRasterEvidence,
...
export const recoverText4hMissingHostedWindowOrSlidingForTest = recoverMissingHostedWindowOrSliding;
 ...
 */
export const clampText4hRecoveredSwingGapToJambFacesForTest = (
  gap: PixelGap,
  axisWalls: PixelSegment[],
  freeformWalls: Text4hFreeformWallSegment[],
  typicalThickness: number,
) => {
...
export const resolveText4hFramedInsetPixelsForTest = (
  gap: PixelGap,
...
export const recoverText4hMissingFacadeWallRunsForTest = recoverMissingFacadeWallRuns;
...
export const recoverText4hInteriorPartitionsForTest = recoverRasterSupportedInteriorPartitions;

interface Text4hTopologyRecoveryCandidate extends Text4hFreeformWallSegment {
  source: 'axis' | 'freeform';
}

const text4hRecoverySegmentLength = (segment: Pick<Text4hFreeformWallSegment, 'p1' | 'p2'>) =>
  Math.hypot(segment.p2.x - segment.p1.x, segment.p2.y - segment.p1.y);

const text4hNearestPointOnSegment = (
  point: Text4hPixelPoint,
  segment: Pick<Text4hFreeformWallSegment, 'p1' | 'p2'>,
) => {
  const dx = segment.p2.x - segment.p1.x, dy = segment.p2.y - segment.p1.y;
...
const text4hRecoverySegmentsOverlap = (
  candidate: Pick<Text4hFreeformWallSegment, 'p1' | 'p2'>,
  retained: Pick<Text4hFreeformWallSegment, 'p1' | 'p2'>,
  typicalThickness: number,
) => {
  const candidateLength = text4hRecoverySegmentLength(candidate);
  const retainedLength = text4hRecoverySegmentLength(retained);
  ...
  if (text4hNearestPointOnSegment(candidate.p1, retained).distance > lineTolerance
    && text4hNearestPointOnSegment(candidate.p2, retained).distance > lineTolerance) return false;
  const project = (point: Text4hPixelPoint) =>
...
const text4hRecoveryRasterSupport = (
  segment: Pick<Text4hFreeformWallSegment, 'p1' | 'p2' | 'thickness'>,
  mask: Uint8Array,
  width: number,
  height: number,
...
const text4hTagsMayShareOpenPlan = (first: RoomTextTag, second: RoomTextTag) => {
...
const text4hTaggedTopologyUtility = (
  flood: FloodResult,
  roomTextTags: RoomTextTag[],
  width: number,
  height: number,
  ...
  const regionIds = roomTextTags.map(tag => text4hTagRegionId(
  ...
  for (let first = 0; first < roomTextTags.length; first++) {
    for (let second = first + 1; second < roomTextTags.length; second++) {
      if (text4hTagsMayShareOpenPlan(roomTextTags[first], roomTextTags[second])) continue;
...
const text4hRoomTopologyAudit = (
  ...
  for (let first = 0; first < roomTextTags.length; first++) {
    for (let second = first + 1; second < roomTextTags.length; second++) {
      if (text4hTagsMayShareOpenPlan(roomTextTags[first], roomTextTags[second])) {
        union(first, second);
        continue;
      }
  ...
  const observedRegions = new Set(roomTextTags.map(tag => text4hTagRegionId(
    flood, tag, width, height, Math.max(2, Math.round(typicalThickness * 1.5)),
  )).filter(Boolean)).size;
...
const recoverTopologyValidatedInteriorPartitions = (
  ...
  freeformCandidates: Text4hFreeformWallSegment[],
  mask: Uint8Array,
  width: number,
  height: number,
  typicalThickness: number,
  ...
  const asFreeform = (segment: PixelSegment): Text4hTopologyRecoveryCandidate => segment.axis === 'horizontal'
    ? {
  ...
  const retained: Text4hTopologyRecoveryCandidate[] = [
  ...
  const rawCandidates: Text4hTopologyRecoveryCandidate[] = [
    ...axisCandidates.map(asFreeform),
    ...freeformCandidates.map(segment => ({ ...segment, source: 'freeform' as const })),
  ].filter(candidate => {
    const length = text4hRecoverySegmentLength(candidate);
    ...
    if (retained.some(segment => text4hRecoverySegmentsOverlap(candidate, segment, typicalThickness))) return false;
    const support = text4hRecoveryRasterSupport(candidate, mask, width, height, typicalThickness);
    return support.supportedRatio >= 0.68 && support.inkRatio >= 0.24;
  }).sort((a, b) => text4hRecoverySegmentLength(b) - text4hRecoverySegmentLength(a))
    ...
    .slice(0, 180);
  const candidates: Text4hTopologyRecoveryCandidate[] = [];
  rawCandidates.forEach(candidate => {
    if (!candidates.some(existing => text4hRecoverySegmentsOverlap(candidate, existing, typicalThickness))) {
  ...
  const existingAnchor = (point: Text4hPixelPoint) => retained
    .map(segment => ({ segment, ...text4hNearestPointOnSegment(point, segment) }))
    .filter(anchor => anchor.distance <= anchorTolerance)
    .sort((a, b) => a.distance - b.distance)[0];
  const candidateAnchor = (point: Text4hPixelPoint, ownIndex: number) => candidates
    .flatMap((segment, index) => index === ownIndex
      ? []
      : [{ index, segment, ...text4hNearestPointOnSegment(point, segment) }])
    .filter(anchor => anchor.distance <= anchorTolerance)
    .sort((a, b) => a.distance - b.distance)[0];
  ...
  const separatesIndependentRoomTags = (candidate: Text4hTopologyRecoveryCandidate) => {
    ...
    return taggedSides.some(first => taggedSides.some(second =>
      first.side !== second.side && !text4hTagsMayShareOpenPlan(first.tag, second.tag)));
  };
  ...
  const viableComponents = components.filter(component => {
    ...
    .sort((a, b) =>
      b.reduce((sum, segment) => sum + text4hRecoverySegmentLength(segment), 0)
        - a.reduce((sum, segment) => sum + text4hRecoverySegmentLength(segment), 0))
    .slice(0, 32);
  ...
  snapCandidates.forEach((candidate, ownIndex) => {
    (['p1', 'p2'] as const).forEach(endpoint => {
      ...
      const anchors = [
        ...retained.map(segment => ({ ...text4hNearestPointOnSegment(sourcePoint, segment), segment })),
        ...snapCandidates.flatMap((segment, index) => index === ownIndex
          ? []
          : [{ ...text4hNearestPointOnSegment(sourcePoint, segment), segment }]),
      ].filter(anchor => anchor.distance <= anchorTolerance)
        .sort((a, b) => a.distance - b.distance);
      ...
      const support = text4hRecoveryRasterSupport(extension, mask, width, height, typicalThickness);
  ...
  if (targetUtility > baseUtility) {
    removalOrder.forEach(component => {
      ...
      const utility = text4hTaggedTopologyUtility(
  ...
  const recoveredFreeform: Text4hFreeformWallSegment[] = [];
...
export const recoverText4hTopologyValidatedPartitionsForTest = recoverTopologyValidatedInteriorPartitions;

export const text4hInteriorArchitecturalCurveEvidenceForTest = (
  arc: Text4hNativeArcRun,
  sourceSegments: Text4hFreeformWallSegment[],
  curveSourceSegments: Text4hFreeformWallSegment[],
  axisSegments: PixelSegment[],
  width: number,
  height: number,
  typicalThickness: number,
  active: boolean,
  nestCenter?: Text4hPixelPoint,
  ...
  const rotation = arc.ellipseRotation || 0;
  const cosR = Math.cos(rotation), sinR = Math.sin(rotation);
  const parameter = (point: Text4hPixelPoint) => {
  ...
  const curveResidual = (point: Text4hPixelPoint) => {
    const dx = point.x - arc.center.x, dy = point.y - arc.center.y;
    const localX = dx * cosR + dy * sinR;
    const localY = -dx * sinR + dy * cosR;
...
export interface Text4hConcentricInteriorRingEvidence {
  center: Text4hPixelPoint;
  radius: number;
  support: number;
...
export const text4hShouldRetainHybridCurveModeForTest = (
  geometryMode: Text4hGeometryMode,
...
export const text4hOpenShellExteriorCurveFamilyEvidenceForTest = (
  arc: Text4hNativeArcRun,
  sourceSegments: Text4hFreeformWallSegment[],
 ...
 */
export const text4hDominantEllipseGapHasRasterEvidenceForTest = (
 ...
 */
export const recoverText4hConcentricInteriorRingForTest = (
  darkMask: Uint8Array,
  width: number,
  height: number,
  typicalThickness: number,
  ...
  active: boolean,
): Text4hConcentricInteriorRingEvidence | undefined => {
...
/** Pure raster-to-geometry rule engine used by Text 4.0 H. */
export const classifyText4hRasterGeometry = (
  image: RasterImageData,
  ...
  return analyzeText4hGeometryMode(
    darkMask,
    width,
    height,
...
export const extractGeometryFromImageData = (
  ...
  let geometryModeAnalysis = analyzeText4hGeometryMode(
    darkMask,
    width,
    height,
    estimateTypicalWallThickness(rawSegments),
  ...
  if (textObservations.length && !roomTextTags.length && geometryModeAnalysis.mode !== 'orthogonal') {
    ...
    const unstrippedModeAnalysis = analyzeText4hGeometryMode(
      originalDarkMask,
      width,
      height,
  ...
  let geometryMode: Text4hGeometryMode = geometryModeAnalysis.mode;
  ...
  const faintAngularWindowGaps = new Set<Text4hFreeformWallGap>();
  ...
  if (geometryMode === 'hybrid') {
    const recoveredAngularWindows = recoverText4hFaintAngularWindowHosts(
      originalDarkMask,
      originalDoorEvidenceMask,
      width,
      height,
  ...
  const ordinaryPixelColumns = componentSheetMode
    ...
    : geometryMode === 'orthogonal'
      ...
      : [];
  const curvilinearColumnSegments: Text4hFreeformWallSegment[] = smoothCurvilinearCandidate
    ? [
      ...freeformSegments,
  ...
  const supplementalAngularStairs = (sparseHybridSymbols || smoothCurvilinearCandidate) && !ordinaryPixelStairs.length
    ? detectText4hSparseAngularStairs(originalDoorEvidenceMask, originalDarkMask, width, height, typicalThickness)
    : [];
  ...
  const detectedAngularRailings = sparseHybridSymbols || smoothCurvilinearCandidate
    ? detectText4hSparseAngularRailings(createCoolGrayRailingMask(image), width, height, typicalThickness)
    : [];
  ...
  if (sparseAngularComponentSheetMode) {
    const recoveredAngularDoors = recoverText4hSupplementalAngularDoorHosts(
      originalDarkMask,
      originalDoorEvidenceMask,
      width,
      height,
  ...
  const pointSegmentDistance = (point: Text4hPixelPoint, first: PixelPoint, second: PixelPoint) => {
  ...
  const segmentOnFootprintEdge = (segment: Text4hFreeformWallSegment, toleranceFactor = 2.2) => {
  ...
  const segmentHasSeparatedGlazingBands = (segment: Text4hFreeformWallSegment) => {
    const dx = segment.p2.x - segment.p1.x, dy = segment.p2.y - segment.p1.y;
    const length = Math.hypot(dx, dy);
  ...
  const curvedExteriorNeighbourCount = (segment: Text4hFreeformWallSegment, maximumAngleDelta: number) => {
  ...
  const nativeCurveMode: Text4hGeometryMode = text4hShouldRetainHybridCurveModeForTest(
    geometryMode,
  ...
  const interiorArchitecturalCurveEvidence = (
    ...
    return text4hInteriorArchitecturalCurveEvidenceForTest(
      arc,
      sourceSegments,
      curveSourceSegments,
      outputWallSegments,
  ...
  const openShellExteriorCurveEvidence = (
    ...
    return text4hOpenShellExteriorCurveFamilyEvidenceForTest(
      arc,
      sourceSegments,
      outsideSourceCount,
      width,
  ...
  const consolidatedFreeform = consolidateText4hCurveArcs(curveSourceSegments, {
  ...
  const recoverNestedInteriorCurve = () => {
    ...
    if (dominantOuterEllipse) {
      ...
      if (nestedSegments.length >= 4) {
        const nestedConsolidated = consolidateText4hCurveArcs(nestedSegments, {
          mode: 'curved',
          structuralMask: originalDarkMask,
          width,
          height,
        ...
        const acceptedNested = nestedConsolidated.arcs.filter(arc => {
          ...
          const sources = familySourceIndices
            ...
            .filter((segment): segment is Text4hFreeformWallSegment => !!segment);
          return text4hInteriorArchitecturalCurveEvidenceForTest(
            arc,
            sources,
            curveSourceSegments,
        ...
        if (acceptedNested.length) {
          const acceptedSources = new Set<Text4hFreeformWallSegment>();
      ...
      if (!hasNestedFamily && nestedSegments.length >= 4) {
        const ring = recoverText4hConcentricInteriorRingForTest(
          originalDarkMask,
          width,
          height,
          typicalThickness,
  ...
  if (curveEvidenceMode && consolidatedFreeform.arcs.length >= 2) {
    ...
    if (ellipseArcs.length) {
      const ellipseParameter = (ellipse: ConsolidatedArc, point: Text4hPixelPoint) => {
      ...
      const ellipsePointAt = (ellipse: ConsolidatedArc, angle: number): Text4hPixelPoint => {
        const radiusX = ellipse.ellipseRadiusX!;
        const radiusY = ellipse.ellipseRadiusY!;
        const rotation = ellipse.ellipseRotation || 0;
        const cosR = Math.cos(rotation), sinR = Math.sin(rotation);
  ...
  if (curveEvidenceMode && consolidatedFreeform.arcs.length >= 1) {
    ...
    families.forEach(family => {
      ...
      const acceptCurveGap = (
        ...
        const gap: Text4hFreeformWallGap = { p1, p2, thickness: typicalThickness, confidence: 0.82 };
        const evidence = inspectText4hFreeformGap(gap, originalDarkMask, originalDoorEvidenceMask, width, height);
      ...
      const hasStructuralInk = (point: Text4hPixelPoint) => {
    ...
    canonicalEllipseFamilies.forEach(family => {
      ...
      const pointAt = (angle: number): Text4hPixelPoint => {
        const cosR = Math.cos(rotation), sinR = Math.sin(rotation);
        const localX = Math.cos(angle) * radiusX;
        const localY = Math.sin(angle) * radiusY;
        return {
    ...
    for (let firstIndex = 0; firstIndex < curveEndpoints.length; firstIndex++) {
      for (let secondIndex = firstIndex + 1; secondIndex < curveEndpoints.length; secondIndex++) {
        ...
        const gap: Text4hFreeformWallGap = {
  ...
  if (curveEvidenceMode && consolidatedFreeform.arcs.length) {
    ...
    const curveParameterDistance = (arc: (typeof consolidatedFreeform.arcs)[number], point: Text4hPixelPoint) => {
    ...
    const curvePointAt = (arc: (typeof consolidatedFreeform.arcs)[number], angle: number): Text4hPixelPoint => {
      const radiusX = arc.ellipseRadiusX ?? arc.radius;
      const radiusY = arc.ellipseRadiusY ?? arc.radius;
      const rotation = arc.ellipseRotation || 0;
      const cosR = Math.cos(rotation), sinR = Math.sin(rotation);
    ...
    const curveTangentAt = (arc: (typeof consolidatedFreeform.arcs)[number], angle: number): Text4hPixelPoint => {
      const radiusX = arc.ellipseRadiusX ?? arc.radius;
      const radiusY = arc.ellipseRadiusY ?? arc.radius;
      const rotation = arc.ellipseRotation || 0;
      const cosR = Math.cos(rotation), sinR = Math.sin(rotation);
    ...
    if (nativeCurveMode === 'hybrid') {
      ...
      if (dominantEllipse) {
        ...
        const supported = Array.from({ length: sampleCount }, (_, index) => {
          ...
          return text4hDominantEllipseGapHasRasterEvidenceForTest(
            structuralSupport,
            symbolSupport,
          );
        });
    ...
    if (nativeCurveMode === 'hybrid') {
      const rejectedSources = new Set<Text4hFreeformWallSegment>();
      const coherentArcs = consolidatedFreeform.arcs.filter(arc => {
        const sources = lateCurveFamilySources(arc);
        const coherent = text4hCurveArcSourceEnvelopeCoherentForTest(
          arc,
          sources,
          width,
          height,
    ...
    recoverNestedInteriorCurve();
    rasterizeNativeText4hArcs(barrier, consolidatedFreeform.arcs, width, height, typicalThickness);
  ...
  const cleanedAngularWalls = cleanupText4hAngularWalls(
    consolidatedFreeform.retainedSegments,
    outputWallSegments,
    {
      mode: geometryMode,
  ...
  const curveApertureChord = (segment: Text4hFreeformWallSegment) => {
    ...
    return consolidatedFreeform.arcs.some(arc => {
      const radiusX = arc.ellipseRadiusX ?? arc.radius;
      const radiusY = arc.ellipseRadiusY ?? arc.radius;
      const endpointResidual = (point: Text4hPixelPoint) => Math.abs(Math.hypot(
  ...
  const duplicatesAxisWall = (segment: Text4hFreeformWallSegment) => {
    if (!curveEvidenceMode) return false;
    const dx = segment.p2.x - segment.p1.x, dy = segment.p2.y - segment.p1.y;
  ...
  const freeformStructuralCoreRatio = (segment: Text4hFreeformWallSegment) => {
    const dx = segment.p2.x - segment.p1.x, dy = segment.p2.y - segment.p1.y;
    const length = Math.max(1e-6, Math.hypot(dx, dy));
    const normal = { x: -dy / length, y: dx / length };
  ...
  const recoverRasterRadialPartitions = () => {
    if (!curveEvidenceMode || !consolidatedFreeform.arcs.length) return [] as Text4hFreeformWallSegment[];
    ...
    if (outerRadius < Math.min(width, height) * 0.22) return [] as Text4hFreeformWallSegment[];
    ...
    if (!detections.length) return [] as Text4hFreeformWallSegment[];
  ...
  const supportedCurvilinearStraightWall = (segment: Text4hFreeformWallSegment) => {
  ...
  const ordinaryExportFreeformSegments = collapseText4hExteriorAngularChamfers(
  ...
  const protectedRadialIsCovered = (segment: Text4hFreeformWallSegment) => ordinaryExportFreeformSegments.some(other => {
    const dx = segment.p2.x - segment.p1.x, dy = segment.p2.y - segment.p1.y;
  ...
  const isTinyAngularConnector = (segment: (typeof cleanedAngularWalls.freeformSegments)[number]) => {
    ...
    const connectedToLongWall = (point: Text4hPixelPoint) => exportFreeformSegments.some(other => {
      if (other === segment) return false;
  ...
  const pointIsHostedByAxisWall = (point: Text4hPixelPoint) => outputWallSegments.some(axisSegment => {
  ...
  const freeformOnFootprintEdge = (segment: Text4hFreeformWallSegment) => {
        ...
        evidence: {
          ...
          notes: [segment.bridge
            ...
            : segment.cleanup
              ...
              : `${geometryMode[0].toUpperCase()}${geometryMode.slice(1)} wall chord retained by the Text 4.0 H geometry detector.`],
        },
      };
    });
  ...
  const gapRecords = architecturalHostedGaps.map(gap => {
    ...
    const outputGap = clampText4hRecoveredSwingGapToJambFacesForTest(
      gap,
      outputWallSegments,
      freeformSegments,
      typicalThickness,
    ...
    const hostedNonDoorKind = resolveText4hHostedNonDoorRasterKindForTest({
      exterior,
    ...
    const framedInsetPixels = resolveText4hFramedInsetPixelsForTest(
      outputGap,
  ...
  freeformGaps.forEach((gap: Text4hFreeformWallGap) => {
    ...
    const evidence = inspectText4hFreeformGap(
      gap,
      originalDarkMask,
      originalDoorEvidenceMask,
      width,
    ...
    if (isDoor) {
      ...
      // inspectText4hFreeformGap reports hinge/side in the gap's own p1->p2
  ...
  openings.splice(0, openings.length, ...arbitrateText4hHostedOpenings(doors, openings));
  windows.splice(0, windows.length, ...arbitrateText4hHostedWindows(doors, windows));
  ...
  openings.splice(0, openings.length, ...openings.filter(opening =>
    text4hOpeningHasHost(opening, walls, hostTolerance)));
  ...
  const roomTopologyAudit = text4hRoomTopologyAudit(
    finalFlood,
    roomTextTags,
    width,
    height,
  ...
  if (geometryMode !== 'orthogonal') {
    ...
    warnings.push(
      `${geometryMode[0].toUpperCase()}${geometryMode.slice(1)} geometry detector retained ${representations.join(' and ')}; scaling, OCR, enclosure recovery, and JSON mapping used the common Text 4.0 H pipeline.`,
    );
  }
