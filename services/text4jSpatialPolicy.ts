...
export interface Text4HraphRoom {
  id: string;
  label: string;
}

export interface Text4HraphEdge {
  a: string;
  b: string;
  access: 'door' | 'opening' | 'window';
}

export interface Text4HraphIssue {
  code: string;
...
export const auditText4AccessGraph = (
  rooms: Text4HraphRoom[],
  edges: Text4HraphEdge[],
  entryRoomId?: string,
): Text4HraphIssue[] => {
  const issues: Text4HraphIssue[] = [];
  const byId = new Map(rooms.map(room => [room.id, room]));
  const adjacency = new Map<string, Text4HraphEdge[]>();
