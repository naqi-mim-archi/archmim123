...
export type Text4eDirectUploadUnitSystem = 'imperial' | 'metric';

export interface Text4eDirectUploadScaleInput {
  unitSystem: Text4eDirectUploadUnitSystem;
  width: string;
  depth: string;
  area: string;
}

export interface Text4eDirectUploadValidation {
  valid: boolean;
  errors: string[];
...
const propertyArea = (value: string, unitSystem: Text4eDirectUploadUnitSystem): number | undefined => {
  const normalized = normalizeMeasurement(value);
  if (!normalized) return undefined;
...
export const validateText4eDirectUploadScale = (
  input: Text4eDirectUploadScaleInput,
): Text4eDirectUploadValidation => {
  const errors: string[] = [];
...
export const prepareText4eDirectUpload = (
  sourceBrief: ConfirmedText4eBrief,
  input: Text4eDirectUploadScaleInput,
): PreparedText4eDirectUpload => {
  const validation = validateText4eDirectUploadScale(input);
