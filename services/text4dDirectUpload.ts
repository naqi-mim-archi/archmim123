...
export type Text4dDirectUploadUnitSystem = 'imperial' | 'metric';

export interface Text4dDirectUploadScaleInput {
  unitSystem: Text4dDirectUploadUnitSystem;
  width: string;
  depth: string;
  area: string;
}

export interface Text4dDirectUploadValidation {
  valid: boolean;
  errors: string[];
...
const propertyArea = (value: string, unitSystem: Text4dDirectUploadUnitSystem): number | undefined => {
  const normalized = normalizeMeasurement(value);
  if (!normalized) return undefined;
...
export const validateText4dDirectUploadScale = (
  input: Text4dDirectUploadScaleInput,
): Text4dDirectUploadValidation => {
  const errors: string[] = [];
...
export const prepareText4dDirectUpload = (
  sourceBrief: ConfirmedText4dBrief,
  input: Text4dDirectUploadScaleInput,
): PreparedText4dDirectUpload => {
  const validation = validateText4dDirectUploadScale(input);
