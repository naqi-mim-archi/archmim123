      ...
      const worker = await createWorker('eng', OEM.LSTM_ONLY, {
        logger: message => {
          if (message.status === 'recognizing text' && message.progress >= 0.99) {
            console.info('[Text 4.0 F] Local OCR complete.');
          }
        },
      });
      try {
 ...
 */
export const warmLocalFloorplanOcr4f = async (): Promise<boolean> => {
  const startedAt = Date.now();
  try {
    await getWorker();
    console.info(`[Text 4.0 F] Local OCR ready in ${Date.now() - startedAt}ms.`);
    return true;
  } catch (error) {
    console.warn(`[Text 4.0 F] Local OCR warm-up failed after ${Date.now() - startedAt}ms; extraction will retry or continue without OCR.`, error);
    return false;
  }
};
 ...
 */
export const recognizeFloorplanText4f = async (
