...
export const TEXT4J_RENDERING_PROFILE_ID = 'text4j-digitizable-floorplan-v1' as const;

export type Text4jAreaUnit = 'sq_ft' | 'sq_m';
export type Text4jLengthUnit = 'ft' | 'm';
export type Text4jValueSource = 'user_confirmed' | 'model_inferred' | 'calculated' | 'application_default';
export type Text4jEnvelopeScope = 'building_footprint' | 'enclosed_plan_envelope';
export type Text4jBriefSection = 'project' | 'dimensions' | 'rooms' | 'adjacency' | 'planning' | 'floors';
export type Text4jRectangularBoundarySource = 'automatic_area_match' | 'explicit_user_request' | 'user_confirmed' | 'ineligible';

export type Text4jImageModel = 'gemini-3.1-flash-lite-image' | 'imagen-3.0-generate-001' | 'imagen-3.0-fast-generate-001';

export interface ConfirmedText4jBrief {
  version: '1';
  project: {
    category: string;
    type: string;
  ...
  dimensions: {
    area: { value: number; unit: Text4jAreaUnit; source: Text4jValueSource };
    envelope: {
      width: number;
      depth: number;
      unit: Text4jLengthUnit;
      scope: Text4jEnvelopeScope;
      source: Text4jValueSource;
    };
    rectangularBoundary: {
      locked: boolean;
      source: Text4jRectangularBoundarySource;
    };
  };
  rooms: Array<{ id: string; name: string; count: number; details?: string; source: Text4jValueSource }>;
  adjacency: Array<{
    id: string;
    description: string;
  ...
  renderingProfileId: typeof TEXT4J_RENDERING_PROFILE_ID;
  imageModel?: Text4jImageModel;
  provenance: {
    draftSource: 'design_copilot' | 'legacy_summary';
    confirmed: boolean;
    lastEditedSection?: Text4jBriefSection;
  };
}

export interface Text4jBriefValidation {
  valid: boolean;
  errors: string[];
  warnings: string[];
}

export interface Text4jRectangularBoundaryEligibility {
  eligible: boolean;
  coverageRatio: number;
  mismatchRatio: number;
}

export const getText4jRectangularBoundaryEligibility = (
  brief: Pick<ConfirmedText4jBrief, 'project' | 'dimensions'>,
): Text4jRectangularBoundaryEligibility => {
...
export const text4jHasExplicitRectangularRequest = (text: string): boolean => {
...
export const applyText4jRectangularBoundaryPolicy = (
  brief: ConfirmedText4jBrief,
  explicitRequestText = '',
): ConfirmedText4jBrief => {
  const eligibility = getText4jRectangularBoundaryEligibility(brief);
  const current = brief.dimensions.rectangularBoundary;
  const rectangularBoundary: ConfirmedText4jBrief['dimensions']['rectangularBoundary'] = !eligibility.eligible
    ? { locked: false, source: 'ineligible' }
    : text4jHasExplicitRectangularRequest(explicitRequestText)
...
export const setText4jRectangularBoundaryLock = (
  brief: ConfirmedText4jBrief,
  locked: boolean,
): ConfirmedText4jBrief => {
  const eligibility = getText4jRectangularBoundaryEligibility(brief);
...
const parseRooms = (value: string | null, source: Text4jValueSource): ConfirmedText4jBrief['rooms'] => {
...
const parseAdjacency = (value: string | null, source: Text4jValueSource): ConfirmedText4jBrief['adjacency'] => {
  if (value === null) return [];
  return value
    .split(/[\n\r]+|\s+[•*]\s+/)
    .map(line => line.trim().replace(/^[-*•\s]+/, '').trim())
...
const parseProject = (purposeText: string | null, previous?: ConfirmedText4jBrief) => {
...
export const createEmptyText4jBrief = (): ConfirmedText4jBrief => ({
  version: '1',
  ...
  renderingProfileId: TEXT4J_RENDERING_PROFILE_ID,
  provenance: { draftSource: 'design_copilot', confirmed: false },
});

export const legacySummaryToText4jBrief = (
  summary: string,
  previous?: ConfirmedText4jBrief,
  source: Text4jValueSource = 'model_inferred',
): ConfirmedText4jBrief => {
  ...
  const areaUnit: Text4jAreaUnit = metric ? 'sq_m' : 'sq_ft';
  const lengthUnit: Text4jLengthUnit = areaUnit === 'sq_m' ? 'm' : 'ft';
  ...
  const nextBrief: ConfirmedText4jBrief = {
    version: '1',
  ...
  return applyText4jRectangularBoundaryPolicy(nextBrief, summary);
};

export const updateText4jBriefFromSummary = (
  summary: string,
  previous: ConfirmedText4jBrief,
  editedSection: Text4jBriefSection,
): ConfirmedText4jBrief => {
  const next = legacySummaryToText4jBrief(summary, previous, 'model_inferred');
  ...
  return applyText4jRectangularBoundaryPolicy(next);
};
...
export const text4jBriefToDesignSummary = (brief: ConfirmedText4jBrief): string => {
...
export const validateText4jBrief = (brief: ConfirmedText4jBrief): Text4jBriefValidation => {
  const errors: string[] = [];
...
export const text4jBriefBoundaryMeters = (brief: ConfirmedText4jBrief): Point[] | undefined => {
