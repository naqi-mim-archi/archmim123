...
export interface Text4fAxisWallSegment {
...
export interface Text4fCleanAngularWallSegment extends Text4fFreeformWallSegment {
...
interface ExteriorAngularChamferCleanupOptions {
  ...
  isExterior: (segment: Pick<Text4fFreeformWallSegment, 'p1' | 'p2' | 'thickness'>) => boolean;
}
...
interface CleanupOptions {
  ...
  height: number;
  typicalThickness: number;
  protectedGaps?: Text4fFreeformWallGap[];
}
...
const distance = (a: Text4fPixelPoint, b: Text4fPixelPoint) =>
...
const isAxisAligned = (segment: Pick<Text4fFreeformWallSegment, 'p1' | 'p2'>) => {
...
const nearestEndpoint = (point: Text4fPixelPoint, segment: Text4fFreeformWallSegment) => {
...
const alignCanonicalAngularFamilies = (
  sourceSegments: Text4fCleanAngularWallSegment[],
...
const absorbAxisAlignedFreeformBridges = (
  sourceFreeform: Text4fCleanAngularWallSegment[],
  sourceAxis: Text4fAxisWallSegment[],
  typicalThickness: number,
) => {
  const axis = sourceAxis.map(segment => ({ ...segment }));
  const retained: Text4fCleanAngularWallSegment[] = [];
...
const asGenericSegments = (
  freeform: Text4fCleanAngularWallSegment[],
  axis: Text4fAxisWallSegment[],
): GenericSegment[] => [
  ...
  while (changed) {
    ...
    for (let first = 0; first < segments.length; first++) {
      for (let second = first + 1; second < segments.length; second++) {
        ...
        let farthest: [Text4fPixelPoint, Text4fPixelPoint] = [allPoints[0], allPoints[1]];
    ...
    let endpoints: [Text4fPixelPoint, Text4fPixelPoint] = [points[0], points[1]];
    let span = 0;
    ...
    const merged: Text4fCleanAngularWallSegment = {
      p1: { ...endpoints[0] },
      p2: { ...endpoints[1] },
...
const removeCoveredDuplicates = (
  ...
  return !sourceSegments.some((host, hostIndex) => {
    if (hostIndex === candidateIndex || distance(host.p1, host.p2) <= candidateLength * 1.35) return false;
    const angle = undirectedAngleDifference(
      { ...candidate, source: 'freeform', sourceIndex: candidateIndex },
      { ...host, source: 'freeform', sourceIndex: hostIndex },
    );
    if (angle > 3 * Math.PI / 180) return false;
    const tolerance = Math.max(1, typicalThickness * 0.22);
    return pointToSegmentDistance(candidate.p1, host.p1, host.p2) <= tolerance
      && pointToSegmentDistance(candidate.p2, host.p1, host.p2) <= tolerance;
  });
});
 ...
 */
export const collapseText4fExteriorAngularChamfers = (
  sourceSegments: Text4fFreeformWallSegment[],
  ...
  const segments = sourceSegments.map(segment => ({
    ...
    p1: { ...segment.p1 },
    p2: { ...segment.p2 },
  } as Text4fCleanAngularWallSegment));
...
const collapseMicroConnectors = (
  ...
  while (changed) {
    ...
    for (const connector of connectors) {
      ...
      let repair: { first: GenericSegment; second: GenericSegment; point: Text4fPixelPoint; score: number } | undefined;
 ...
 */
export const cleanupText4fAngularWalls = (
  freeformSegments: Text4fFreeformWallSegment[],
  axisSegments: Text4fAxisWallSegment[],
  options: CleanupOptions,
) => {
