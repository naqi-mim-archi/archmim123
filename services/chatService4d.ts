...
import { ai } from "./aiClient4d";
import { getText4dConversationProfile } from "./text4dConversationVariation";

export const refineDesignRequirements4d = async (
  chatHistory: { role: 'user' | 'model'; text: string }[],
  variationIndex = 0,
): Promise<{ reply: string; isReady: boolean; summary: string }> => {
  const profile = getText4dConversationProfile(variationIndex);
