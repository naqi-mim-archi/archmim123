...
type LengthUnit = ConfirmedText4gBrief['dimensions']['envelope']['unit'];
...
export interface Text4gRoomCrossDimensionLabel {
  label: string;
...
export const formatText4gRoomCrossDimensions = (
  widthMeters: number,
  depthMeters: number,
  unit: LengthUnit,
): string => unit === 'ft'
...
export const buildText4gRoomCrossDimensionLabels = (
  geometry: GeneratedData,
  unit: LengthUnit,
): Text4gRoomCrossDimensionLabel[] => {
  ...
  return (geometry.rooms || []).flatMap(room => {
    ...
    return [{
      ...
      widthMeters,
      depthMeters,
      dimensionText: formatText4gRoomCrossDimensions(widthMeters, depthMeters, unit),
    }];
  });
};
...
const loadImage = (dataUrl: string): Promise<HTMLImageElement> => new Promise((resolve, reject) => {
  ...
  image.onerror = () => reject(new Error('Generated Text 4.0 G image could not be decoded for room annotation.'));
  image.src = dataUrl;
});
 ...
 */
export const annotateText4gGeneratedImageRooms = async (
  dataUrl: string,
  geometry: GeneratedData,
  ...
  const labels = buildText4gRoomCrossDimensionLabels(geometry, brief.dimensions.envelope.unit);
