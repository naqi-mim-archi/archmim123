...
type LengthUnit = ConfirmedText4jBrief['dimensions']['envelope']['unit'];
...
export interface Text4jRoomCrossDimensionLabel {
  label: string;
...
export const formatText4jRoomCrossDimensions = (
  widthMeters: number,
  depthMeters: number,
  unit: LengthUnit,
): string => unit === 'ft'
...
export const buildText4jRoomCrossDimensionLabels = (
  geometry: GeneratedData,
  unit: LengthUnit,
): Text4jRoomCrossDimensionLabel[] => {
  ...
  return (geometry.rooms || []).flatMap(room => {
    ...
    return [{
      ...
      widthMeters,
      depthMeters,
      dimensionText: formatText4jRoomCrossDimensions(widthMeters, depthMeters, unit),
    }];
  });
};
...
const loadImage = (dataUrl: string): Promise<HTMLImageElement> => new Promise((resolve, reject) => {
  ...
  image.onerror = () => reject(new Error('Generated Text 4.0 J image could not be decoded for room annotation.'));
  image.src = dataUrl;
});
 ...
 */
export const annotateText4jGeneratedImageRooms = async (
  dataUrl: string,
  geometry: GeneratedData,
  ...
  const labels = buildText4jRoomCrossDimensionLabels(geometry, brief.dimensions.envelope.unit);
