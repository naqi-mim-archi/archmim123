...
import { recognizeFloorplanText4d, type FloorplanTextObservation } from './localFloorplanOcr4d';
...
export interface LocalImageExtractionOptions {
  ...
  /** Text 4.0 D uses its confirmed width and depth as independent, authoritative axes. */
    ...
    image.onload = async () => {
      try {
        ...
        if (!textObservations && !options.disableOcr) {
          ...
          const ocrTask = recognizeFloorplanText4d(canvas);
          if (options.onGeometryReady) {
            try {
              ...
              console.warn('[Text 4.0 D] Early raster preview was unavailable; continuing to final digitization.', error);
            }
          }
          try {
            ...
            if (result === timeout) {
              console.warn(`[Text 4.0 D] Local OCR exceeded its ${budgetMs}ms budget; continuing with raster and design-brief signals.`);
              textObservations = [];
              ocrStatus = 'timed-out';
            } else {
            ...
            ocrDurationMs = performance.now() - ocrStartedAt;
            console.warn('[Text 4.0 D] Local OCR was unavailable; continuing with raster and design-brief signals.', error);
            textObservations = [];
...
const detectDoorSwing = (
  ...
  const endpointRuns: Array<{ endpoint: 'start' | 'end'; normalDirection: -1 | 1; length: number }> = [];
      ...
      for (let y = y1; y <= y2 + 1; y++) {
        ...
        if ((!dark || y > y2) && runStart >= 0) {
          ...
          if (runStart <= gap.line + gap.thickness && runEnd >= gap.line - gap.thickness && length >= minimumStroke && length <= maximumStroke) {
            ...
            const candidate = { endpoint, normalDirection: (positiveReach >= negativeReach ? 1 : -1) as -1 | 1, length };
...
const detectBestDoorSwing = (
  ...
  return leaf.confidence >= arc.confidence ? leaf : arc;
};
...
const detectGapSymbolEvidence = (
  ...
  return {
    doorSwing: originalSwing.confidence > cleanSwing.confidence ? originalSwing : cleanSwing,
...
/** Pure raster-to-geometry rule engine used by Text 4.0 D. */
export const extractGeometryFromImageData = (
  ...
  const preRetentionSymbolGaps = recoverSymbolSupportedGaps(
    ...
    if (record.exterior) {
      ...
      doors.push({
        ...
        evidence: {
          ...
          ...(record.doorSwing.arcOnly ? { notes: ['Door swing recovered from a jamb-centred quarter-circle arc.'] } : {}),
        },
      });
    }
  }
