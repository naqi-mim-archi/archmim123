...
export const TEXT4F_RENDERING_PROFILE_ID = 'text4f-digitizable-floorplan-v1' as const;

export type Text4fAreaUnit = 'sq_ft' | 'sq_m';
export type Text4fLengthUnit = 'ft' | 'm';
export type Text4fValueSource = 'user_confirmed' | 'model_inferred' | 'calculated' | 'application_default';
export type Text4fEnvelopeScope = 'building_footprint' | 'enclosed_plan_envelope';
export type Text4fBriefSection = 'project' | 'dimensions' | 'rooms' | 'adjacency' | 'planning' | 'floors';
export type Text4fRectangularBoundarySource = 'automatic_area_match' | 'explicit_user_request' | 'user_confirmed' | 'ineligible';

export interface ConfirmedText4fBrief {
  version: '1';
  project: {
    category: string;
    type: string;
  ...
  dimensions: {
    area: { value: number; unit: Text4fAreaUnit; source: Text4fValueSource };
    envelope: {
      width: number;
      depth: number;
      unit: Text4fLengthUnit;
      scope: Text4fEnvelopeScope;
      source: Text4fValueSource;
    };
    rectangularBoundary: {
      locked: boolean;
      source: Text4fRectangularBoundarySource;
    };
  };
  rooms: Array<{ id: string; name: string; count: number; details?: string; source: Text4fValueSource }>;
  adjacency: Array<{
    id: string;
    description: string;
  ...
  renderingProfileId: typeof TEXT4F_RENDERING_PROFILE_ID;
  provenance: {
    draftSource: 'design_copilot' | 'legacy_summary';
    confirmed: boolean;
    lastEditedSection?: Text4fBriefSection;
  };
}

export interface Text4fBriefValidation {
  valid: boolean;
  errors: string[];
  warnings: string[];
}

export interface Text4fRectangularBoundaryEligibility {
  eligible: boolean;
  coverageRatio: number;
  mismatchRatio: number;
}

export const getText4fRectangularBoundaryEligibility = (
  brief: Pick<ConfirmedText4fBrief, 'project' | 'dimensions'>,
): Text4fRectangularBoundaryEligibility => {
...
export const text4fHasExplicitRectangularRequest = (text: string): boolean => {
...
export const applyText4fRectangularBoundaryPolicy = (
  brief: ConfirmedText4fBrief,
  explicitRequestText = '',
): ConfirmedText4fBrief => {
  const eligibility = getText4fRectangularBoundaryEligibility(brief);
  const current = brief.dimensions.rectangularBoundary;
  const rectangularBoundary: ConfirmedText4fBrief['dimensions']['rectangularBoundary'] = !eligibility.eligible
    ? { locked: false, source: 'ineligible' }
    : text4fHasExplicitRectangularRequest(explicitRequestText)
...
export const setText4fRectangularBoundaryLock = (
  brief: ConfirmedText4fBrief,
  locked: boolean,
): ConfirmedText4fBrief => {
  const eligibility = getText4fRectangularBoundaryEligibility(brief);
...
const parseRooms = (value: string | null, source: Text4fValueSource): ConfirmedText4fBrief['rooms'] => {
...
const parseAdjacency = (value: string | null, source: Text4fValueSource): ConfirmedText4fBrief['adjacency'] => {
  if (value === null) return [];
  return value
    .split(/[\n\r]+|\s+[•*]\s+/)
    .map(line => line.trim().replace(/^[-*•\s]+/, '').trim())
...
const parseProject = (purposeText: string | null, previous?: ConfirmedText4fBrief) => {
...
export const createEmptyText4fBrief = (): ConfirmedText4fBrief => ({
  version: '1',
  ...
  renderingProfileId: TEXT4F_RENDERING_PROFILE_ID,
  provenance: { draftSource: 'design_copilot', confirmed: false },
});

export const legacySummaryToText4fBrief = (
  summary: string,
  previous?: ConfirmedText4fBrief,
  source: Text4fValueSource = 'model_inferred',
): ConfirmedText4fBrief => {
  ...
  const areaUnit: Text4fAreaUnit = metric ? 'sq_m' : 'sq_ft';
  const lengthUnit: Text4fLengthUnit = areaUnit === 'sq_m' ? 'm' : 'ft';
  ...
  const nextBrief: ConfirmedText4fBrief = {
    version: '1',
  ...
  return applyText4fRectangularBoundaryPolicy(nextBrief, summary);
};

export const updateText4fBriefFromSummary = (
  summary: string,
  previous: ConfirmedText4fBrief,
  editedSection: Text4fBriefSection,
): ConfirmedText4fBrief => {
  const next = legacySummaryToText4fBrief(summary, previous, 'model_inferred');
  ...
  return applyText4fRectangularBoundaryPolicy(next);
};
...
export const text4fBriefToDesignSummary = (brief: ConfirmedText4fBrief): string => {
...
export const validateText4fBrief = (brief: ConfirmedText4fBrief): Text4fBriefValidation => {
  const errors: string[] = [];
...
export const text4fBriefBoundaryMeters = (brief: ConfirmedText4fBrief): Point[] | undefined => {
