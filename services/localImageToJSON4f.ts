...
import { recognizeFloorplanText4f, type FloorplanTextObservation } from './localFloorplanOcr4f';
import {
  analyzeText4fGeometryMode,
  detectText4fSparseAngularColumns,
  detectText4fSparseAngularRailings,
  detectText4fSparseAngularStairs,
  inspectText4fFreeformGap,
  recoverText4fFaintAngularWindowHosts,
  recoverText4fSupplementalAngularDoorHosts,
  type Text4fFreeformWallGap,
  type Text4fFreeformWallSegment,
  type Text4fGeometryMode,
  type Text4fPixelPoint,
} from './text4fGeometryModes';
import { consolidateText4fCurveArcs } from './text4fCurveArcs';
import {
  cleanupText4fAngularWalls,
  collapseText4fExteriorAngularChamfers,
} from './text4fAngularCleanup';
...
export interface LocalImageExtractionOptions {
  ...
  /** Text 4.0 F uses its confirmed width and depth as independent, authoritative axes. */
    ...
    image.onload = async () => {
      try {
        ...
        if (!textObservations && !options.disableOcr) {
          ...
          const ocrTask = recognizeFloorplanText4f(canvas);
          if (options.onGeometryReady) {
            try {
              ...
              console.warn('[Text 4.0 F] Early raster preview was unavailable; continuing to final digitization.', error);
            }
          }
          try {
            ...
            if (result === timeout) {
              console.warn(`[Text 4.0 F] Local OCR exceeded its ${budgetMs}ms budget; continuing with raster and design-brief signals.`);
              textObservations = [];
              ocrStatus = 'timed-out';
            } else {
            ...
            ocrDurationMs = performance.now() - ocrStartedAt;
            console.warn('[Text 4.0 F] Local OCR was unavailable; continuing with raster and design-brief signals.', error);
            textObservations = [];
...
const rasterizeFreeformWalls = (
  barrier: Uint8Array,
  segments: Text4fFreeformWallSegment[],
  width: number,
  height: number,
) => {
...
const rasterizeNativeText4fArcs = (
  barrier: Uint8Array,
  arcs: Array<{
    center: Text4fPixelPoint;
    radius: number;
...
const freeformBounds = (segments: Text4fFreeformWallSegment[]): PixelBounds | undefined => {
...
const freeformCoveredByAxisWall = (
  segment: Text4fFreeformWallSegment,
...
const isFreeformExterior = (
  segment: Pick<Text4fFreeformWallSegment, 'p1' | 'p2' | 'thickness'>,
  outside: Uint8Array,
  width: number,
  height: number,
) => {
...
/** Pure raster-to-geometry rule engine used by Text 4.0 F. */
export const extractGeometryFromImageData = (
  ...
  let geometryModeAnalysis = analyzeText4fGeometryMode(
    darkMask,
    width,
    height,
  ...
  if (textObservations.length && !roomTextTags.length && geometryModeAnalysis.mode !== 'orthogonal') {
    ...
    const unstrippedModeAnalysis = analyzeText4fGeometryMode(
      originalDarkMask,
      width,
      height,
  ...
  let geometryMode: Text4fGeometryMode = geometryModeAnalysis.mode;
  ...
  const faintAngularWindowGaps = new Set<Text4fFreeformWallGap>();
  ...
  if (geometryMode === 'hybrid') {
    const recoveredAngularWindows = recoverText4fFaintAngularWindowHosts(
      originalDarkMask,
      originalDoorEvidenceMask,
      width,
      height,
  ...
  const ordinaryPixelColumns = componentSheetMode
    ...
    : geometryMode === 'orthogonal'
      ...
      : [];
  const curvilinearColumnSegments: Text4fFreeformWallSegment[] = smoothCurvilinearCandidate
    ? [
      ...freeformSegments,
  ...
  const supplementalAngularStairs = (sparseHybridSymbols || smoothCurvilinearCandidate) && !ordinaryPixelStairs.length
    ? detectText4fSparseAngularStairs(originalDoorEvidenceMask, originalDarkMask, width, height, typicalThickness)
    : [];
  ...
  const detectedAngularRailings = sparseHybridSymbols || smoothCurvilinearCandidate
    ? detectText4fSparseAngularRailings(createCoolGrayRailingMask(image), width, height, typicalThickness)
    : [];
  ...
  if (sparseAngularComponentSheetMode) {
    const recoveredAngularDoors = recoverText4fSupplementalAngularDoorHosts(
      originalDarkMask,
      originalDoorEvidenceMask,
      width,
      height,
  ...
  const pointSegmentDistance = (point: Text4fPixelPoint, first: PixelPoint, second: PixelPoint) => {
  ...
  const segmentOnFootprintEdge = (segment: Text4fFreeformWallSegment, toleranceFactor = 2.2) => {
  ...
  const smoothCurvedExteriorEvidence = retainedFreeformSegments
    .filter(segment => {
      const absoluteAngle = (Math.atan2(segment.p2.y - segment.p1.y, segment.p2.x - segment.p1.x) * 180 / Math.PI + 180) % 180;
      if (Math.min(absoluteAngle % 90, 90 - (absoluteAngle % 90)) < 10
        || !isFreeformExterior(segment, finalFlood.outside, width, height)) return false;
      const length = Math.max(1e-6, Math.hypot(segment.p2.x - segment.p1.x, segment.p2.y - segment.p1.y));
      const neighbouringCount = retainedFreeformSegments.filter(other => {
        ...
        return joins && angleDelta >= 2 && angleDelta <= 22;
      }).length;
      return neighbouringCount >= 2 && length >= typicalThickness * 0.65;
    })
    .reduce((sum, segment) => sum + Math.hypot(segment.p2.x - segment.p1.x, segment.p2.y - segment.p1.y), 0);
  const nativeCurveMode: Text4fGeometryMode = geometryMode === 'hybrid'
    && (nonAxisExteriorEvidence < Math.max(typicalThickness * 8, Math.min(width, height) * 0.12)
      || smoothCurvedExteriorEvidence < Math.max(typicalThickness * 8, Math.min(width, height) * 0.16))
    ? 'angular'
    : geometryMode;
  ...
  const consolidatedFreeform = consolidateText4fCurveArcs(curveSourceSegments, {
  ...
  if (curveEvidenceMode && consolidatedFreeform.arcs.length >= 2) {
    ...
    families.forEach(family => {
      ...
      const acceptCurveGap = (
        ...
        const gap: Text4fFreeformWallGap = { p1, p2, thickness: typicalThickness, confidence: 0.82 };
        const evidence = inspectText4fFreeformGap(gap, originalDarkMask, originalDoorEvidenceMask, width, height);
      ...
      const angularGaps = merged.length >= 2 ? merged.map((interval, index) => {
      ...
      const hasStructuralInk = (point: Text4fPixelPoint) => {
    ...
    for (let firstIndex = 0; firstIndex < curveEndpoints.length; firstIndex++) {
      for (let secondIndex = firstIndex + 1; secondIndex < curveEndpoints.length; secondIndex++) {
        ...
        const gap: Text4fFreeformWallGap = {
  ...
  if (curveEvidenceMode && consolidatedFreeform.arcs.length) {
    ...
    const curveParameterDistance = (arc: (typeof consolidatedFreeform.arcs)[number], point: Text4fPixelPoint) => {
    ...
    const curvePointAt = (arc: (typeof consolidatedFreeform.arcs)[number], angle: number): Text4fPixelPoint => {
      const radiusX = arc.ellipseRadiusX ?? arc.radius;
      const radiusY = arc.ellipseRadiusY ?? arc.radius;
      const rotation = arc.ellipseRotation || 0;
      const cosR = Math.cos(rotation), sinR = Math.sin(rotation);
    ...
    const curveTangentAt = (arc: (typeof consolidatedFreeform.arcs)[number], angle: number): Text4fPixelPoint => {
      const radiusX = arc.ellipseRadiusX ?? arc.radius;
      const radiusY = arc.ellipseRadiusY ?? arc.radius;
      const rotation = arc.ellipseRotation || 0;
      const cosR = Math.cos(rotation), sinR = Math.sin(rotation);
    ...
    rasterizeNativeText4fArcs(barrier, consolidatedFreeform.arcs, width, height, typicalThickness);
  ...
  const cleanedAngularWalls = cleanupText4fAngularWalls(
    consolidatedFreeform.retainedSegments,
    outputWallSegments,
    {
      mode: geometryMode,
  ...
  const curveApertureChord = (segment: Text4fFreeformWallSegment) => {
    ...
    return consolidatedFreeform.arcs.some(arc => {
      const radiusX = arc.ellipseRadiusX ?? arc.radius;
      const radiusY = arc.ellipseRadiusY ?? arc.radius;
      const endpointResidual = (point: Text4fPixelPoint) => Math.abs(Math.hypot(
  ...
  const duplicatesAxisWall = (segment: Text4fFreeformWallSegment) => {
    if (!curveEvidenceMode) return false;
    const dx = segment.p2.x - segment.p1.x, dy = segment.p2.y - segment.p1.y;
  ...
  const freeformStructuralCoreRatio = (segment: Text4fFreeformWallSegment) => {
    const dx = segment.p2.x - segment.p1.x, dy = segment.p2.y - segment.p1.y;
    const length = Math.max(1e-6, Math.hypot(dx, dy));
    const normal = { x: -dy / length, y: dx / length };
  ...
  const supportedCurvilinearStraightWall = (segment: Text4fFreeformWallSegment) => {
  ...
  const exportFreeformSegments = collapseText4fExteriorAngularChamfers(
  ...
  const isTinyAngularConnector = (segment: (typeof cleanedAngularWalls.freeformSegments)[number]) => {
    ...
    const connectedToLongWall = (point: Text4fPixelPoint) => exportFreeformSegments.some(other => {
      if (other === segment) return false;
  ...
  const pointIsHostedByAxisWall = (point: Text4fPixelPoint) => outputWallSegments.some(axisSegment => {
  ...
  const freeformOnFootprintEdge = (segment: Text4fFreeformWallSegment) => {
        ...
        evidence: {
          ...
          notes: [segment.bridge
            ...
            : segment.cleanup
              ...
              : `${geometryMode[0].toUpperCase()}${geometryMode.slice(1)} wall chord retained by the Text 4.0 F geometry detector.`],
        },
      };
    });
  ...
  const nativeArcWalls: NonNullable<GeneratedData['walls']> = consolidatedFreeform.arcs.map(arc => {
    const sourceSegments = arc.sourceIndices
      ...
      .filter((segment): segment is Text4fFreeformWallSegment => !!segment);
  ...
  const gapRecords = architecturalHostedGaps.map(gap => {
    ...
    // ink gets one additional, hybrid-only read at final classification: two
    // independently evidenced jamb leaves are a conservative double-door
    // signature, while staggered parallel panels retain a sliding door. This
    // does not alter the ordinary orthogonal route or wall topology.
    const faintHybridSwing = geometryMode === 'hybrid' && !symbolEvidence.doorSwing.detected
      ...
      : undefined;
    const faintHybridDouble = faintHybridSwing?.endpointCount && faintHybridSwing.endpointCount >= 2
      && Math.abs(faintHybridSwing.hingeOffset || 0) <= Math.max(2, gap.thickness * 1.25)
      ? { ...faintHybridSwing, confidence: Math.min(0.88, faintHybridSwing.confidence) }
      : undefined;
    ...
    const faintCurvilinearSingle = curveEvidenceMode
      ...
      : undefined;
    const doorSwing = faintHybridDouble || faintCurvilinearSingle || symbolEvidence.doorSwing;
    ...
    const windowFrame = symbolEvidence.windowFrame || faintHybridWindowFrame;
    ...
    const slidingEvidence = faintHybridSliding?.detected
  ...
  }).filter(record => !record.intactThinWall && record.widthMeters >= 0.45 && record.widthMeters <= 4.5);
  ...
  for (const record of gapRecords) {
    ...
    if (record.exterior) {
      const exteriorSlidingDoor = effectiveSlidingDoor
        && (record.curvilinearAxisSliding || !exteriorHasSwingEntry || record === entryRecord);
      ...
      if (isDoor) {
        ...
        const exportedWidth = clamp(
          ...
          geometryMode === 'hybrid' ? doorType === 'single' ? 1.0 : 1.55 : 2.4,
        );
        doors.push({
          levelIndex: 0,
          pos: record.position,
      ...
      const exportedWidth = clamp(
        record.widthMeters,
        doorType === 'single' ? 0.6 : 0.7,
        geometryMode === 'hybrid' ? doorType === 'single' ? 1.0 : 1.55 : doorType === 'single' ? 1.2 : 2.4,
      );
      doors.push({
  ...
  freeformGaps.forEach((gap: Text4fFreeformWallGap) => {
    ...
    const evidence = inspectText4fFreeformGap(
      gap,
      originalDarkMask,
      originalDoorEvidenceMask,
      width,
    ...
    const authoritativeFreeformWindow = evidence.windowFrame || curvedWindowFrame;
    ...
    const strongFreeformDoorEvidence = freeformDoorSized && (orientedFoldingDoor || orientedSlidingDoor || ((evidence.doorLeaf || supportedSwingLeaf)
      ...
      && !windowLikeFreeform));
    const strongLinearLeaf = strongFreeformDoorEvidence && (evidence.doorLeafSupport || 0) >= 0.62;
    const isDoor = strongLinearLeaf
      || (!recoveredFaintAngularWindow
        && !authoritativeFreeformWindow
        && (strongFreeformDoorEvidence || (exterior && nearEntryTag && doorSized)));
    ...
    if (isDoor) {
      ...
      // inspectText4fFreeformGap reports hinge/side in the gap's own p1->p2
  ...
  if (curveEvidenceMode) {
      ...
      for (let index = 0; index <= marked.length; index++) {
        ...
        if ((index === marked.length || !marked[index].marked) && runStart >= 0) {
          ...
          if (runLength >= 2) {
            ...
            if (widthMeters >= 0.65 && widthMeters <= 2.4) {
    ...
    const planDistance = (first: [number, number], second: [number, number]) =>
  ...
  const slabs: NonNullable<GeneratedData['slabs']> = componentSheetMode || sparseAngularComponentSheetMode
    ? []
  ...
  if (geometryMode !== 'orthogonal') {
    ...
    warnings.push(
      `${geometryMode[0].toUpperCase()}${geometryMode.slice(1)} geometry detector retained ${representations.join(' and ')}; scaling, OCR, enclosure recovery, and JSON mapping used the common Text 4.0 F pipeline.`,
    );
  }
