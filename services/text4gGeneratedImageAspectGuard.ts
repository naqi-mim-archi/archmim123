...
import { getText4gRectangularBoundaryEligibility } from './text4gBrief';

export interface Text4gAspectRaster {
  width: number;
  height: number;
  data: ArrayLike<number>;
}

export interface Text4gStructuralFrame {
  minX: number;
  maxX: number;
  minY: number;
  maxY: number;
...
export interface Text4gAspectCorrectionPlan {
  applied: boolean;
  targetRatio: number;
  sourceRatio: number;
...
export interface Text4gGeneratedImageAspectResult {
  dataUrl: string;
  applied: boolean;
  targetRatio?: number;
  sourceRatio?: number;
  correctedRatio?: number;
  relativeError?: number;
  reason: 'not-rectangle-locked' | 'frame-not-detected' | Text4gAspectCorrectionPlan['reason'];
}
 ...
 */
export const detectText4gLockedStructuralFrame = (
  raster: Text4gAspectRaster,
): Text4gStructuralFrame | undefined => {
  ...
  let bestFrame: Text4gStructuralFrame | undefined;
  let bestScore = -1;
...
export const planText4gLockedAspectCorrection = (
  frame: Pick<Text4gStructuralFrame, 'ratio'>,
  targetRatio: number,
  tolerance = 0.02,
): Text4gAspectCorrectionPlan => {
...
export const getText4gLockedImageAspectTarget = (
...
const loadImage = (dataUrl: string): Promise<HTMLImageElement> => new Promise((resolve, reject) => {
  ...
  image.onerror = () => reject(new Error('Generated Text 4.0 G image could not be decoded for aspect validation.'));
  image.src = dataUrl;
});
 ...
 */
export const enforceText4gGeneratedImageAspect = async (
  dataUrl: string,
  brief: ConfirmedText4gBrief,
): Promise<Text4gGeneratedImageAspectResult> => {
  const targetRatio = getText4gLockedImageAspectTarget(brief);
  ...
  const frame = detectText4gLockedStructuralFrame(sourceContext.getImageData(0, 0, width, height));
  if (!frame) return { dataUrl, applied: false, targetRatio, reason: 'frame-not-detected' };
  const plan = planText4gLockedAspectCorrection(frame, targetRatio);
  if (!plan.applied) {
    return {
      dataUrl,
      applied: false,
