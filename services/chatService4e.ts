...
import { ai } from "./aiClient4e";
import { getText4eConversationProfile } from "./text4eConversationVariation";

export const refineDesignRequirements4e = async (
  chatHistory: { role: 'user' | 'model'; text: string }[],
  variationIndex = 0,
): Promise<{ reply: string; isReady: boolean; summary: string }> => {
  const profile = getText4eConversationProfile(variationIndex);
