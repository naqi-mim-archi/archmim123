...
export const TEXT4E_RENDERING_PROFILE_ID = 'text4e-digitizable-floorplan-v1' as const;

export type Text4eAreaUnit = 'sq_ft' | 'sq_m';
export type Text4eLengthUnit = 'ft' | 'm';
export type Text4eValueSource = 'user_confirmed' | 'model_inferred' | 'calculated' | 'application_default';
export type Text4eEnvelopeScope = 'building_footprint' | 'enclosed_plan_envelope';
export type Text4eBriefSection = 'project' | 'dimensions' | 'rooms' | 'adjacency' | 'planning' | 'floors';
export type Text4eRectangularBoundarySource = 'automatic_area_match' | 'explicit_user_request' | 'user_confirmed' | 'ineligible';

export interface ConfirmedText4eBrief {
  version: '1';
  project: {
    category: string;
    type: string;
  ...
  dimensions: {
    area: { value: number; unit: Text4eAreaUnit; source: Text4eValueSource };
    envelope: {
      width: number;
      depth: number;
      unit: Text4eLengthUnit;
      scope: Text4eEnvelopeScope;
      source: Text4eValueSource;
    };
    rectangularBoundary: {
      locked: boolean;
      source: Text4eRectangularBoundarySource;
    };
  };
  rooms: Array<{ id: string; name: string; count: number; details?: string; source: Text4eValueSource }>;
  adjacency: Array<{
    id: string;
    description: string;
  ...
  renderingProfileId: typeof TEXT4E_RENDERING_PROFILE_ID;
  provenance: {
    draftSource: 'design_copilot' | 'legacy_summary';
    confirmed: boolean;
    lastEditedSection?: Text4eBriefSection;
  };
}

export interface Text4eBriefValidation {
  valid: boolean;
  errors: string[];
  warnings: string[];
}

export interface Text4eRectangularBoundaryEligibility {
  eligible: boolean;
  coverageRatio: number;
  mismatchRatio: number;
}

export const getText4eRectangularBoundaryEligibility = (
  brief: Pick<ConfirmedText4eBrief, 'project' | 'dimensions'>,
): Text4eRectangularBoundaryEligibility => {
...
export const text4eHasExplicitRectangularRequest = (text: string): boolean => {
...
export const applyText4eRectangularBoundaryPolicy = (
  brief: ConfirmedText4eBrief,
  explicitRequestText = '',
): ConfirmedText4eBrief => {
  const eligibility = getText4eRectangularBoundaryEligibility(brief);
  const current = brief.dimensions.rectangularBoundary;
  const rectangularBoundary: ConfirmedText4eBrief['dimensions']['rectangularBoundary'] = !eligibility.eligible
    ? { locked: false, source: 'ineligible' }
    : text4eHasExplicitRectangularRequest(explicitRequestText)
...
export const setText4eRectangularBoundaryLock = (
  brief: ConfirmedText4eBrief,
  locked: boolean,
): ConfirmedText4eBrief => {
  const eligibility = getText4eRectangularBoundaryEligibility(brief);
...
const parseRooms = (value: string | null, source: Text4eValueSource): ConfirmedText4eBrief['rooms'] => {
...
const parseAdjacency = (value: string | null, source: Text4eValueSource): ConfirmedText4eBrief['adjacency'] => {
  if (value === null) return [];
  return value
    .split(/[\n\r]+|\s+[•*]\s+/)
    .map(line => line.trim().replace(/^[-*•\s]+/, '').trim())
...
const parseProject = (purposeText: string | null, previous?: ConfirmedText4eBrief) => {
...
export const createEmptyText4eBrief = (): ConfirmedText4eBrief => ({
  version: '1',
  ...
  renderingProfileId: TEXT4E_RENDERING_PROFILE_ID,
  provenance: { draftSource: 'design_copilot', confirmed: false },
});

export const legacySummaryToText4eBrief = (
  summary: string,
  previous?: ConfirmedText4eBrief,
  source: Text4eValueSource = 'model_inferred',
): ConfirmedText4eBrief => {
  ...
  const areaUnit: Text4eAreaUnit = metric ? 'sq_m' : 'sq_ft';
  const lengthUnit: Text4eLengthUnit = areaUnit === 'sq_m' ? 'm' : 'ft';
  ...
  const nextBrief: ConfirmedText4eBrief = {
    version: '1',
  ...
  return applyText4eRectangularBoundaryPolicy(nextBrief, summary);
};

export const updateText4eBriefFromSummary = (
  summary: string,
  previous: ConfirmedText4eBrief,
  editedSection: Text4eBriefSection,
): ConfirmedText4eBrief => {
  const next = legacySummaryToText4eBrief(summary, previous, 'model_inferred');
  ...
  return applyText4eRectangularBoundaryPolicy(next);
};
...
export const text4eBriefToDesignSummary = (brief: ConfirmedText4eBrief): string => {
...
export const validateText4eBrief = (brief: ConfirmedText4eBrief): Text4eBriefValidation => {
  const errors: string[] = [];
...
export const text4eBriefBoundaryMeters = (brief: ConfirmedText4eBrief): Point[] | undefined => {
