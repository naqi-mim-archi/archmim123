import {
  type ConfirmedText4jBrief,
  getText4jRectangularBoundaryEligibility,
  TEXT4J_RENDERING_PROFILE_ID,
} from './text4jBrief';
...
const TEXT4J_IMAGE_CANVAS_PIXELS = 1024;
const TEXT4J_FOOTPRINT_MAX_SPAN_PERCENT = 72;
...
const roomText = (brief: ConfirmedText4jBrief): string => brief.rooms
  .map(room => `${room.count} x ${clean(room.name)}${room.details ? ` (${clean(room.details)})` : ''}`)
  .join('; ');

const curvilinearRequested = (brief: ConfirmedText4jBrief): boolean => {
  const text = [
...
const relevantPlanningRules = (brief: ConfirmedText4jBrief): string[] => {
...
export const buildText4jImagePrompt = (brief: ConfirmedText4jBrief): string => {
  ...
  const rectangularEligibility = getText4jRectangularBoundaryEligibility(brief);
  ...
  const frameWidth = TEXT4J_FOOTPRINT_MAX_SPAN_PERCENT * brief.dimensions.envelope.width / longestSide;
  const frameDepth = TEXT4J_FOOTPRINT_MAX_SPAN_PERCENT * brief.dimensions.envelope.depth / longestSide;
  const frameWidthPixels = Math.round(TEXT4J_IMAGE_CANVAS_PIXELS * frameWidth / 100);
  const frameDepthPixels = Math.round(TEXT4J_IMAGE_CANVAS_PIXELS * frameDepth / 100);
