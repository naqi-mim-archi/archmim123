/**
 * Conservative preset resolution for Text 4.0 J digitization.
 *
...
export const TEXT4J_WALL_PRESETS = [
...
export const TEXT4J_DOOR_PRESETS = [
...
export const TEXT4J_WINDOW_PRESETS = [
...
export type Text4jEvidenceStrength = 'strong' | 'medium' | 'weak' | 'none';
export type Text4jResolutionConfidence = 'high' | 'medium' | 'low' | 'unresolved';
export type Text4jResolutionProvenance =
  | 'measured-preset'
  | 'measured-custom'
  | 'role-default'
  | 'semantic-default'
  | 'unresolved';
export type Text4jWallRole = 'exterior' | 'interior' | 'structural-interior' | 'partition' | 'glass';

export interface Text4jMeasuredValues {
  widthM: number | null;
  heightM: number | null;
  thicknessM: number | null;
}

export interface Text4jPresetResolution {
  kind: 'wall' | 'door' | 'window';
  presetId: string | null;
  presetLabel: string | null;
  subtype: string;
  measuredValues: Text4jMeasuredValues;
  resolvedValues: Text4jMeasuredValues;
  confidence: Text4jResolutionConfidence;
  provenance: Text4jResolutionProvenance;
...
export interface ResolveText4jWallInput {
  semanticRole: Text4jWallRole;
  projectClass?: string | null;
  measuredThicknessM?: number | null;
  evidenceStrength?: Text4jEvidenceStrength;
  /** Required for measured-to-preset snapping. No implicit tolerance is applied. */
  toleranceM?: number | null;
}

export interface ResolveText4jDoorInput {
...
export interface ResolveText4jWindowInput {
  measuredWidthM?: number | null;
  measuredHeightM?: number | null;
  detectedSubtype?: string | null;
  evidenceStrength?: Text4jEvidenceStrength;
...
const evidenceConfidence = (
  strength: Text4jEvidenceStrength,
  unresolved = false,
): Text4jResolutionConfidence => {
...
const emptyValues = (): Text4jMeasuredValues => ({ widthM: null, heightM: null, thicknessM: null });
...
const wallCandidates = (role: Text4jWallRole): readonly (typeof TEXT4J_WALL_PRESETS)[number][] => {
  if (role === 'exterior') return TEXT4J_WALL_PRESETS.filter(preset => preset.subtype === 'exterior');
  return TEXT4J_WALL_PRESETS.filter(preset => preset.subtype === role);
};

const semanticWallPresetId = (role: Text4jWallRole, projectClass?: string | null): string => {
...
export const resolveText4jWallPreset = (input: ResolveText4jWallInput): Text4jPresetResolution => {
  ...
  const measuredValues: Text4jMeasuredValues = { ...emptyValues(), thicknessM: measuredThicknessM };
  ...
  const preset = TEXT4J_WALL_PRESETS.find(candidate => candidate.id === presetId)!;
...
export const resolveText4jDoorPreset = (input: ResolveText4jDoorInput): Text4jPresetResolution => {
  ...
  const measuredValues: Text4jMeasuredValues = {
    ...emptyValues(),
  ...
  if (measuredWidthM !== null) {
    const knownSubtype = TEXT4J_DOOR_PRESETS.some(preset => preset.subtype === detectedSubtype);
    const candidates = knownSubtype
      ? TEXT4J_DOOR_PRESETS.filter(preset => preset.subtype === detectedSubtype)
      : detectedSubtype ? [] : [...TEXT4J_DOOR_PRESETS];
  ...
  const rolePreset = rolePresetId
    ? TEXT4J_DOOR_PRESETS.find(candidate => candidate.id === rolePresetId) || null
    : null;
  if (rolePreset) {
    return {
      kind: 'door',
...
export const resolveText4jWindowPreset = (input: ResolveText4jWindowInput): Text4jPresetResolution => {
  const measuredWidthM = positiveMeasurement(input.measuredWidthM);
  const measuredHeightM = positiveMeasurement(input.measuredHeightM);
  const toleranceM = explicitTolerance(input.toleranceM);
  const evidence = input.evidenceStrength || 'none';
  ...
  if (measuredWidthM !== null) {
    const knownSubtype = TEXT4J_WINDOW_PRESETS.some(preset => preset.subtype === detectedSubtype);
    ...
    const candidates = knownSubtype
      ? TEXT4J_WINDOW_PRESETS.filter(preset => preset.subtype === detectedSubtype)
      : detectedSubtype ? [] : TEXT4J_WINDOW_PRESETS.filter(preset => preset.subtype === 'regular');
    const nearest = candidates
      .map(preset => {
