...
type LengthUnit = ConfirmedText4hBrief['dimensions']['envelope']['unit'];
...
export interface Text4hRoomCrossDimensionLabel {
  label: string;
...
export const formatText4hRoomCrossDimensions = (
  widthMeters: number,
  depthMeters: number,
  unit: LengthUnit,
): string => unit === 'ft'
...
export const buildText4hRoomCrossDimensionLabels = (
  geometry: GeneratedData,
  unit: LengthUnit,
): Text4hRoomCrossDimensionLabel[] => {
  ...
  return (geometry.rooms || []).flatMap(room => {
    ...
    return [{
      ...
      widthMeters,
      depthMeters,
      dimensionText: formatText4hRoomCrossDimensions(widthMeters, depthMeters, unit),
    }];
  });
};
...
const loadImage = (dataUrl: string): Promise<HTMLImageElement> => new Promise((resolve, reject) => {
  ...
  image.onerror = () => reject(new Error('Generated Text 4.0 H image could not be decoded for room annotation.'));
  image.src = dataUrl;
});
 ...
 */
export const annotateText4hGeneratedImageRooms = async (
  dataUrl: string,
  geometry: GeneratedData,
  ...
  const labels = buildText4hRoomCrossDimensionLabels(geometry, brief.dimensions.envelope.unit);
