export type Text4hGeometryMode = 'orthogonal' | 'angular' | 'curved' | 'hybrid';

export interface Text4hPixelPoint {
  x: number;
  y: number;
}

export interface Text4hFreeformWallSegment {
  p1: Text4hPixelPoint;
  p2: Text4hPixelPoint;
  thickness: number;
  confidence: number;
...
export interface Text4hFreeformWallGap {
  p1: Text4hPixelPoint;
  p2: Text4hPixelPoint;
  thickness: number;
  confidence: number;
...
export interface Text4hGeometryModeAnalysis {
   ...
   */
  candidateSegments?: Text4hFreeformWallSegment[];
...
export interface Text4hFreeformGapEvidence {
  doorLeaf: boolean;
  windowFrame: boolean;
  confidence: number;
  hingeAtEnd: boolean;
...
export interface Text4hSparseAngularStair {
  p1: Text4hPixelPoint;
  p2: Text4hPixelPoint;
  widthPixels: number;
  stepCount: number;
...
export interface Text4hSparseAngularColumn {
  x: number;
  y: number;
  widthPixels: number;
  depthPixels: number;
...
const pointKey = (point: Text4hPixelPoint) => `${point.x},${point.y}`;
const direction = (a: Text4hPixelPoint, b: Text4hPixelPoint) => {
  const length = Math.max(1e-6, distance(a, b));
  return { x: (b.x - a.x) / length, y: (b.y - a.y) / length };
};
const dot = (a: Text4hPixelPoint, b: Text4hPixelPoint) => a.x * b.x + a.y * b.y;
const angleDistanceFromAxis = (segment: Pick<Text4hFreeformWallSegment, 'p1' | 'p2'>) => {
...
interface ThinSegment { p1: Text4hPixelPoint; p2: Text4hPixelPoint; }
...
export const detectText4hSparseAngularStairs = (
  ...
  const stairs: Text4hSparseAngularStair[] = [];
...
export const detectText4hSparseAngularRailings = (
...
export const detectText4hSparseAngularColumns = (
  structuralMask: Uint8Array,
  width: number,
  height: number,
  typicalThickness: number,
  ...
  const columns: Text4hSparseAngularColumn[] = [];
  unique.forEach(point => {
    ...
    const inspectCenter = (center: Text4hPixelPoint) => {
...
const rdp = (points: Text4hPixelPoint[], tolerance: number): Text4hPixelPoint[] => {
...
const simplifyPath = (points: Text4hPixelPoint[], tolerance: number, closed: boolean) => {
...
const traceSkeleton = (skeleton: Uint8Array, width: number, height: number) => {
  ...
  const toPoint = (index: number): Text4hPixelPoint => ({ x: index % width, y: Math.floor(index / width) });
...
const pathLength = (points: Text4hPixelPoint[]) => points.slice(1)
...
export const analyzeText4hGeometryMode = (
  sourceMask: Uint8Array,
  sourceWidth: number,
  sourceHeight: number,
  typicalThicknessHint: number,
): Text4hGeometryModeAnalysis => {
  ...
  const segments: Text4hFreeformWallSegment[] = [];
  const candidateSegments: Text4hFreeformWallSegment[] = [];
  interface PathEndpoint { point: Text4hPixelPoint; inner: Text4hPixelPoint; component: number; }
  ...
  const gaps: Text4hFreeformWallGap[] = [];
  ...
  const cross = (origin: Text4hPixelPoint, a: Text4hPixelPoint, b: Text4hPixelPoint) =>
    (a.x - origin.x) * (b.y - origin.y) - (a.y - origin.y) * (b.x - origin.x);
  const lower: Text4hPixelPoint[] = [];
  hullPoints.forEach(point => {
    while (lower.length >= 2 && cross(lower[lower.length - 2], lower[lower.length - 1], point) <= 0) lower.pop();
    lower.push(point);
  });
  const upper: Text4hPixelPoint[] = [];
  [...hullPoints].reverse().forEach(point => {
    while (upper.length >= 2 && cross(upper[upper.length - 2], upper[upper.length - 1], point) <= 0) upper.pop();
    upper.push(point);
  });
  const hull = [...lower.slice(0, -1), ...upper.slice(0, -1)];
  const pointToSegmentDistance = (point: Text4hPixelPoint, a: Text4hPixelPoint, b: Text4hPixelPoint) => {
  ...
  const mode: Text4hGeometryMode = nonOrthogonalRatio >= 0.76
...
export const inspectText4hFreeformGap = (
  gap: Text4hFreeformWallGap,
  structuralMask: Uint8Array,
  doorEvidenceMask: Uint8Array,
  width: number,
  height: number,
): Text4hFreeformGapEvidence => {
 ...
 */
export const recoverText4hSupplementalAngularDoorHosts = (
  structuralMask: Uint8Array,
  doorEvidenceMask: Uint8Array,
  width: number,
  height: number,
  typicalThickness: number,
  observedSegments: Text4hFreeformWallSegment[],
  observedGaps: Text4hFreeformWallGap[],
  ...
  for (let firstIndex = 0; firstIndex < endpoints.length; firstIndex++) {
    for (let secondIndex = firstIndex + 1; secondIndex < endpoints.length; secondIndex++) {
      ...
      const evidence = inspectText4hFreeformGap(gap, structuralMask, doorEvidenceMask, width, height);
  ...
  return {
    segments: selected.map(candidate => ({
      ...
      bridge: true,
    } satisfies Text4hFreeformWallSegment)),
 ...
 */
export const recoverText4hFaintAngularWindowHosts = (
  structuralMask: Uint8Array,
  ...
  const faintPixels: Text4hPixelPoint[] = [];
  ...
  const maskNear = (point: Text4hPixelPoint, radius: number) => {
  ...
  canonicalAngles.forEach(angle => {
    ...
    for (let first = 0; first < runs.length; first++) for (let second = first + 1; second < runs.length; second++) {
      ...
      const toPoint = (along: number): Text4hPixelPoint => ({
        x: tangent.x * along + normal.x * hostRho,
        y: tangent.y * along + normal.y * hostRho,
      });
