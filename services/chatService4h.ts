...
import { ai } from "./aiClient4h";
import { getText4hConversationProfile } from "./text4hConversationVariation";

export const refineDesignRequirements4h = async (
  chatHistory: { role: 'user' | 'model'; text: string }[],
  variationIndex = 0,
): Promise<{ reply: string; isReady: boolean; summary: string }> => {
  const profile = getText4hConversationProfile(variationIndex);
