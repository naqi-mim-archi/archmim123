import {
  type ConfirmedText4fBrief,
  getText4fRectangularBoundaryEligibility,
  TEXT4F_RENDERING_PROFILE_ID,
} from './text4fBrief';
...
const TEXT4F_IMAGE_CANVAS_PIXELS = 1024;
const TEXT4F_FOOTPRINT_MAX_SPAN_PERCENT = 72;
...
const roomText = (brief: ConfirmedText4fBrief): string => brief.rooms
  .map(room => `${room.count} x ${clean(room.name)}${room.details ? ` (${clean(room.details)})` : ''}`)
  .join('; ');

const relevantPlanningRules = (brief: ConfirmedText4fBrief): string[] => {
...
export const buildText4fImagePrompt = (brief: ConfirmedText4fBrief): string => {
  ...
  const rectangularEligibility = getText4fRectangularBoundaryEligibility(brief);
  ...
  const frameWidth = TEXT4F_FOOTPRINT_MAX_SPAN_PERCENT * brief.dimensions.envelope.width / longestSide;
  const frameDepth = TEXT4F_FOOTPRINT_MAX_SPAN_PERCENT * brief.dimensions.envelope.depth / longestSide;
  const frameWidthPixels = Math.round(TEXT4F_IMAGE_CANVAS_PIXELS * frameWidth / 100);
  const frameDepthPixels = Math.round(TEXT4F_IMAGE_CANVAS_PIXELS * frameDepth / 100);
  const frameLeftPixels = Math.round((TEXT4F_IMAGE_CANVAS_PIXELS - frameWidthPixels) / 2);
  const frameTopPixels = Math.round((TEXT4F_IMAGE_CANVAS_PIXELS - frameDepthPixels) / 2);
  ...
  const frameGeometry = brief.project.floors > 1
    ...
    : `fixed-property bbox ${frameWidthPixels}x${frameDepthPixels}px at x=${frameLeftPixels}-${frameLeftPixels + frameWidthPixels}, y=${frameTopPixels}-${frameTopPixels + frameDepthPixels}`;
