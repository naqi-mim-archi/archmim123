/**
 * Conservative preset resolution for Text 4.0 F digitization.
 *
...
export const TEXT4F_WALL_PRESETS = [
...
export const TEXT4F_DOOR_PRESETS = [
...
export const TEXT4F_WINDOW_PRESETS = [
...
export type Text4fEvidenceStrength = 'strong' | 'medium' | 'weak' | 'none';
export type Text4fResolutionConfidence = 'high' | 'medium' | 'low' | 'unresolved';
export type Text4fResolutionProvenance =
  | 'measured-preset'
  | 'measured-custom'
  | 'role-default'
  | 'semantic-default'
  | 'unresolved';
export type Text4fWallRole = 'exterior' | 'interior' | 'structural-interior' | 'partition' | 'glass';

export interface Text4fMeasuredValues {
  widthM: number | null;
  heightM: number | null;
  thicknessM: number | null;
}

export interface Text4fPresetResolution {
  kind: 'wall' | 'door' | 'window';
  presetId: string | null;
  presetLabel: string | null;
  subtype: string;
  measuredValues: Text4fMeasuredValues;
  resolvedValues: Text4fMeasuredValues;
  confidence: Text4fResolutionConfidence;
  provenance: Text4fResolutionProvenance;
...
export interface ResolveText4fWallInput {
  semanticRole: Text4fWallRole;
  projectClass?: string | null;
  measuredThicknessM?: number | null;
  evidenceStrength?: Text4fEvidenceStrength;
  /** Required for measured-to-preset snapping. No implicit tolerance is applied. */
  toleranceM?: number | null;
}

export interface ResolveText4fDoorInput {
...
export interface ResolveText4fWindowInput {
  measuredWidthM?: number | null;
  measuredHeightM?: number | null;
  detectedSubtype?: string | null;
  evidenceStrength?: Text4fEvidenceStrength;
...
const evidenceConfidence = (
  strength: Text4fEvidenceStrength,
  unresolved = false,
): Text4fResolutionConfidence => {
...
const emptyValues = (): Text4fMeasuredValues => ({ widthM: null, heightM: null, thicknessM: null });
...
const wallCandidates = (role: Text4fWallRole): readonly (typeof TEXT4F_WALL_PRESETS)[number][] => {
  if (role === 'exterior') return TEXT4F_WALL_PRESETS.filter(preset => preset.subtype === 'exterior');
  return TEXT4F_WALL_PRESETS.filter(preset => preset.subtype === role);
};

const semanticWallPresetId = (role: Text4fWallRole, projectClass?: string | null): string => {
...
export const resolveText4fWallPreset = (input: ResolveText4fWallInput): Text4fPresetResolution => {
  ...
  const measuredValues: Text4fMeasuredValues = { ...emptyValues(), thicknessM: measuredThicknessM };
  ...
  const preset = TEXT4F_WALL_PRESETS.find(candidate => candidate.id === presetId)!;
...
export const resolveText4fDoorPreset = (input: ResolveText4fDoorInput): Text4fPresetResolution => {
  ...
  const measuredValues: Text4fMeasuredValues = {
    ...emptyValues(),
  ...
  if (measuredWidthM !== null) {
    const knownSubtype = TEXT4F_DOOR_PRESETS.some(preset => preset.subtype === detectedSubtype);
    const candidates = knownSubtype
      ? TEXT4F_DOOR_PRESETS.filter(preset => preset.subtype === detectedSubtype)
      : detectedSubtype ? [] : [...TEXT4F_DOOR_PRESETS];
  ...
  const rolePreset = rolePresetId
    ? TEXT4F_DOOR_PRESETS.find(candidate => candidate.id === rolePresetId) || null
    : null;
  if (rolePreset) {
    return {
      kind: 'door',
...
export const resolveText4fWindowPreset = (input: ResolveText4fWindowInput): Text4fPresetResolution => {
  const measuredWidthM = positiveMeasurement(input.measuredWidthM);
  const measuredHeightM = positiveMeasurement(input.measuredHeightM);
  const toleranceM = explicitTolerance(input.toleranceM);
  const evidence = input.evidenceStrength || 'none';
  ...
  if (measuredWidthM !== null) {
    const knownSubtype = TEXT4F_WINDOW_PRESETS.some(preset => preset.subtype === detectedSubtype);
    ...
    const candidates = knownSubtype
      ? TEXT4F_WINDOW_PRESETS.filter(preset => preset.subtype === detectedSubtype)
      : detectedSubtype ? [] : TEXT4F_WINDOW_PRESETS.filter(preset => preset.subtype === 'regular');
    const nearest = candidates
      .map(preset => {
