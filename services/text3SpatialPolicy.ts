...
export type Text3PrivacyZone = 'public' | 'semiPrivate' | 'private' | 'service' | 'outdoor';

export interface Text3RoomPolicy {
  zone: Text3PrivacyZone;
  preferredDepth: readonly [number, number];
  integration: 'high' | 'medium' | 'low' | 'private';
}

export const TEXT3_ROOM_POLICY: Record<Text3RoomKind, Text3RoomPolicy> = {
...
export const classifyText3Room = (label: string): Text3RoomKind => {
...
export const buildText3RelationshipMatrix = (summary: string): string[] => {
  const lines: string[] = [];
...
export interface Text3GraphRoom {
  id: string;
  label: string;
}

export interface Text3GraphEdge {
  a: string;
  b: string;
  access: 'door' | 'opening' | 'window';
}

export interface Text3GraphIssue {
  code: string;
...
export const auditText3AccessGraph = (
  rooms: Text3GraphRoom[],
  edges: Text3GraphEdge[],
  entryRoomId?: string,
): Text3GraphIssue[] => {
  const issues: Text3GraphIssue[] = [];
  const byId = new Map(rooms.map(room => [room.id, room]));
  const adjacency = new Map<string, Text3GraphEdge[]>();
  rooms.forEach(room => adjacency.set(room.id, []));
  edges.forEach(edge => {
    adjacency.get(edge.a)?.push(edge);
    adjacency.get(edge.b)?.push(edge);
    const aKind = classifyText3Room(byId.get(edge.a)?.label || '');
    const bKind = classifyText3Room(byId.get(edge.b)?.label || '');
  ...
  rooms.forEach(room => {
    if (!depths.has(room.id) && classifyText3Room(room.label) !== 'balcony') {
  ...
  while (publicQueue.length) {
    ...
    (adjacency.get(current) || []).forEach(edge => {
      const next = edge.a === current ? edge.b : edge.a;
      const nextKind = classifyText3Room(byId.get(next)?.label || '');
      if (nextKind === 'commonBathroom' || nextKind === 'ensuite' || publicDepths.has(next)) return;
      publicDepths.add(next);
      publicQueue.push(next);
    });
  }
  rooms.filter(room => classifyText3Room(room.label) === 'living').forEach(room => {
  ...
  const livingDepths = rooms.filter(r => classifyText3Room(r.label) === 'living').map(r => depths.get(r.id)).filter((d): d is number => d !== undefined);
  const bedroomDepths = rooms.filter(r => ['bedroom', 'masterBedroom'].includes(classifyText3Room(r.label))).map(r => depths.get(r.id)).filter((d): d is number => d !== undefined);
