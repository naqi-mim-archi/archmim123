...
import { getText4eRectangularBoundaryEligibility } from './text4eBrief';

export interface Text4eAspectRaster {
  width: number;
  height: number;
  data: ArrayLike<number>;
}

export interface Text4eStructuralFrame {
  minX: number;
  maxX: number;
  minY: number;
  maxY: number;
...
export interface Text4eAspectCorrectionPlan {
  applied: boolean;
  targetRatio: number;
  sourceRatio: number;
...
export interface Text4eGeneratedImageAspectResult {
  dataUrl: string;
  applied: boolean;
  targetRatio?: number;
  sourceRatio?: number;
  correctedRatio?: number;
  relativeError?: number;
  reason: 'not-rectangle-locked' | 'frame-not-detected' | Text4eAspectCorrectionPlan['reason'];
}
 ...
 */
export const detectText4eLockedStructuralFrame = (
  raster: Text4eAspectRaster,
): Text4eStructuralFrame | undefined => {
  ...
  let bestFrame: Text4eStructuralFrame | undefined;
  let bestScore = -1;
...
export const planText4eLockedAspectCorrection = (
  frame: Pick<Text4eStructuralFrame, 'ratio'>,
  targetRatio: number,
  tolerance = 0.02,
): Text4eAspectCorrectionPlan => {
...
export const getText4eLockedImageAspectTarget = (
...
const loadImage = (dataUrl: string): Promise<HTMLImageElement> => new Promise((resolve, reject) => {
  ...
  image.onerror = () => reject(new Error('Generated Text 4.0 E image could not be decoded for aspect validation.'));
  image.src = dataUrl;
});
 ...
 */
export const enforceText4eGeneratedImageAspect = async (
  dataUrl: string,
  brief: ConfirmedText4eBrief,
): Promise<Text4eGeneratedImageAspectResult> => {
  const targetRatio = getText4eLockedImageAspectTarget(brief);
  ...
  const frame = detectText4eLockedStructuralFrame(sourceContext.getImageData(0, 0, width, height));
  if (!frame) return { dataUrl, applied: false, targetRatio, reason: 'frame-not-detected' };
  const plan = planText4eLockedAspectCorrection(frame, targetRatio);
  if (!plan.applied) {
    return {
      dataUrl,
      applied: false,
