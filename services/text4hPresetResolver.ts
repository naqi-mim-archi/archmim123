/**
 * Conservative preset resolution for Text 4.0 H digitization.
 *
...
export const TEXT4H_WALL_PRESETS = [
...
export const TEXT4H_DOOR_PRESETS = [
...
export const TEXT4H_WINDOW_PRESETS = [
...
export type Text4hEvidenceStrength = 'strong' | 'medium' | 'weak' | 'none';
export type Text4hResolutionConfidence = 'high' | 'medium' | 'low' | 'unresolved';
export type Text4hResolutionProvenance =
  | 'measured-preset'
  | 'measured-custom'
  | 'role-default'
  | 'semantic-default'
  | 'unresolved';
export type Text4hWallRole = 'exterior' | 'interior' | 'structural-interior' | 'partition' | 'glass';

export interface Text4hMeasuredValues {
  widthM: number | null;
  heightM: number | null;
  thicknessM: number | null;
}

export interface Text4hPresetResolution {
  kind: 'wall' | 'door' | 'window';
  presetId: string | null;
  presetLabel: string | null;
  subtype: string;
  measuredValues: Text4hMeasuredValues;
  resolvedValues: Text4hMeasuredValues;
  confidence: Text4hResolutionConfidence;
  provenance: Text4hResolutionProvenance;
...
export interface ResolveText4hWallInput {
  semanticRole: Text4hWallRole;
  projectClass?: string | null;
  measuredThicknessM?: number | null;
  evidenceStrength?: Text4hEvidenceStrength;
  /** Required for measured-to-preset snapping. No implicit tolerance is applied. */
  toleranceM?: number | null;
}

export interface ResolveText4hDoorInput {
...
export interface ResolveText4hWindowInput {
  measuredWidthM?: number | null;
  measuredHeightM?: number | null;
  detectedSubtype?: string | null;
  evidenceStrength?: Text4hEvidenceStrength;
...
const evidenceConfidence = (
  strength: Text4hEvidenceStrength,
  unresolved = false,
): Text4hResolutionConfidence => {
...
const emptyValues = (): Text4hMeasuredValues => ({ widthM: null, heightM: null, thicknessM: null });
...
const wallCandidates = (role: Text4hWallRole): readonly (typeof TEXT4H_WALL_PRESETS)[number][] => {
  if (role === 'exterior') return TEXT4H_WALL_PRESETS.filter(preset => preset.subtype === 'exterior');
  return TEXT4H_WALL_PRESETS.filter(preset => preset.subtype === role);
};

const semanticWallPresetId = (role: Text4hWallRole, projectClass?: string | null): string => {
...
export const resolveText4hWallPreset = (input: ResolveText4hWallInput): Text4hPresetResolution => {
  ...
  const measuredValues: Text4hMeasuredValues = { ...emptyValues(), thicknessM: measuredThicknessM };
  ...
  const preset = TEXT4H_WALL_PRESETS.find(candidate => candidate.id === presetId)!;
...
export const resolveText4hDoorPreset = (input: ResolveText4hDoorInput): Text4hPresetResolution => {
  ...
  const measuredValues: Text4hMeasuredValues = {
    ...emptyValues(),
  ...
  if (measuredWidthM !== null) {
    const knownSubtype = TEXT4H_DOOR_PRESETS.some(preset => preset.subtype === detectedSubtype);
    const candidates = knownSubtype
      ? TEXT4H_DOOR_PRESETS.filter(preset => preset.subtype === detectedSubtype)
      : detectedSubtype ? [] : [...TEXT4H_DOOR_PRESETS];
  ...
  const rolePreset = rolePresetId
    ? TEXT4H_DOOR_PRESETS.find(candidate => candidate.id === rolePresetId) || null
    : null;
  if (rolePreset) {
    return {
      kind: 'door',
...
export const resolveText4hWindowPreset = (input: ResolveText4hWindowInput): Text4hPresetResolution => {
  const measuredWidthM = positiveMeasurement(input.measuredWidthM);
  const measuredHeightM = positiveMeasurement(input.measuredHeightM);
  const toleranceM = explicitTolerance(input.toleranceM);
  const evidence = input.evidenceStrength || 'none';
  ...
  if (measuredWidthM !== null) {
    const knownSubtype = TEXT4H_WINDOW_PRESETS.some(preset => preset.subtype === detectedSubtype);
    ...
    const candidates = knownSubtype
      ? TEXT4H_WINDOW_PRESETS.filter(preset => preset.subtype === detectedSubtype)
      : detectedSubtype ? [] : TEXT4H_WINDOW_PRESETS.filter(preset => preset.subtype === 'regular');
    const nearest = candidates
      .map(preset => {
