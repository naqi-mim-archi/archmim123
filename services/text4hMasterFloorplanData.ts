...
export type Text4hMasterFloorplanSourceKind = 'generated' | 'uploaded';
export type Text4hMasterThinkingLevel = 'minimal' | 'low' | 'medium' | 'high';

export interface Text4hMasterFloorplanRequest {
  imageBase64: string;
...
export interface Text4hMasterGraphValidation {
  valid: boolean;
  errors: string[];
...
export const buildText4hMasterFloorplanPrompt = (
  ...
  return `You are the Text 4.0 H master floorplan geometry transcriber.
...
export const validateText4hMasterFloorplanGraph = (rawData: any): Text4hMasterGraphValidation => {
  const errors: string[] = [];
...
const transformMasterGraph = (
  rawData: any,
  request: Pick<Text4hMasterFloorplanRequest, 'requestedBoundary' | 'requestedExtentsMeters'>,
...
const validWalls = (data: any): NonNullable<GeneratedData['walls']> => (Array.isArray(data?.walls) ? data.walls : [])
  .map((wall: any) => {
    ...
    const normalized: NonNullable<GeneratedData['walls']>[number] = {
      ...
      provenance: 'observed',
      evidence: { source: 'raster', confidence: asNumber(wall.confidence, curveType === 'line' ? 0.82 : 0.88), notes: ['Text 4.0 H Gemini geometry graph.'] },
    };
...
export const normalizeText4hMasterFloorplanData = (
  rawData: any,
  request: Pick<Text4hMasterFloorplanRequest, 'brief' | 'imageBase64' | 'sourceKind' | 'requestedBoundary' | 'requestedExtentsMeters'>,
): GeneratedData => {
  const graphValidation = validateText4hMasterFloorplanGraph(rawData);
    ...
    extractionDiagnostics: {
      ...
      warnings: [
        request.sourceKind === 'generated'
          ? 'Generated raster was transcribed through the independent Text 4.0 H Gemini geometry graph.'
          : 'Uploaded raster was transcribed through the independent Text 4.0 H Gemini geometry graph.',
...
export const isText4hMasterFloorplanDataUsable = (data: GeneratedData): boolean =>
  (data.walls?.length || 0) >= 2 && data.extractionDiagnostics?.canImport === true;

export async function transcribeText4hMasterFloorplanData(
  request: Text4hMasterFloorplanRequest,
): Promise<GeneratedData> {
  const { data, mimeType } = splitText4hDataUrl(request.imageBase64);
  const prompt = buildText4hMasterFloorplanPrompt(
    request.brief,
    request.sourceKind,
    request.requestedBoundary,
    request.requestedExtentsMeters,
  );
  const response = await fetch('/api/text4h/master-geometry', {
    method: 'POST',
  ...
  if (!response.ok) {
    ...
    throw new Error(`Text 4.0 H master floorplan data API error (${response.status}): ${errorText}`);
  }
  ...
  return normalizeText4hMasterFloorplanData(geometry, request);
}
