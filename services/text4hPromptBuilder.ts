import {
  type ConfirmedText4hBrief,
  getText4hRectangularBoundaryEligibility,
  TEXT4H_RENDERING_PROFILE_ID,
} from './text4hBrief';
...
const TEXT4H_IMAGE_CANVAS_PIXELS = 1024;
const TEXT4H_FOOTPRINT_MAX_SPAN_PERCENT = 72;
...
const roomText = (brief: ConfirmedText4hBrief): string => brief.rooms
  .map(room => `${room.count} x ${clean(room.name)}${room.details ? ` (${clean(room.details)})` : ''}`)
  .join('; ');

const curvilinearRequested = (brief: ConfirmedText4hBrief): boolean => {
  const text = [
...
const relevantPlanningRules = (brief: ConfirmedText4hBrief): string[] => {
...
export const buildText4hImagePrompt = (brief: ConfirmedText4hBrief): string => {
  ...
  const rectangularEligibility = getText4hRectangularBoundaryEligibility(brief);
  ...
  const frameWidth = TEXT4H_FOOTPRINT_MAX_SPAN_PERCENT * brief.dimensions.envelope.width / longestSide;
  const frameDepth = TEXT4H_FOOTPRINT_MAX_SPAN_PERCENT * brief.dimensions.envelope.depth / longestSide;
  const frameWidthPixels = Math.round(TEXT4H_IMAGE_CANVAS_PIXELS * frameWidth / 100);
  const frameDepthPixels = Math.round(TEXT4H_IMAGE_CANVAS_PIXELS * frameDepth / 100);
