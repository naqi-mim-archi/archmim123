...
import { buildText3RelationshipMatrix } from "./text3SpatialPolicy";
...
const normalizeRelationships = (designSummary: string): string => {
  ...
  normalized.push(...buildText3RelationshipMatrix(designSummary));
...
export const generateFloorplan3 = async (
  ...
  try {
    ...
    console.info(`[Text 3.0] Gemini generation ${(requestFinished - requestStarted).toFixed(0)} ms`);
