import {
  type ConfirmedText4eBrief,
  getText4eRectangularBoundaryEligibility,
  TEXT4E_RENDERING_PROFILE_ID,
} from './text4eBrief';
...
const TEXT4E_IMAGE_CANVAS_PIXELS = 1024;
const TEXT4E_FOOTPRINT_MAX_SPAN_PERCENT = 72;
...
const roomText = (brief: ConfirmedText4eBrief): string => brief.rooms
  .map(room => `${room.count} x ${clean(room.name)}${room.details ? ` (${clean(room.details)})` : ''}`)
  .join('; ');

const relevantPlanningRules = (brief: ConfirmedText4eBrief): string[] => {
...
export const buildText4eImagePrompt = (brief: ConfirmedText4eBrief): string => {
  ...
  const rectangularEligibility = getText4eRectangularBoundaryEligibility(brief);
  ...
  const frameWidth = TEXT4E_FOOTPRINT_MAX_SPAN_PERCENT * brief.dimensions.envelope.width / longestSide;
  const frameDepth = TEXT4E_FOOTPRINT_MAX_SPAN_PERCENT * brief.dimensions.envelope.depth / longestSide;
  const frameWidthPixels = Math.round(TEXT4E_IMAGE_CANVAS_PIXELS * frameWidth / 100);
  const frameDepthPixels = Math.round(TEXT4E_IMAGE_CANVAS_PIXELS * frameDepth / 100);
  const frameLeftPixels = Math.round((TEXT4E_IMAGE_CANVAS_PIXELS - frameWidthPixels) / 2);
  const frameTopPixels = Math.round((TEXT4E_IMAGE_CANVAS_PIXELS - frameDepthPixels) / 2);
