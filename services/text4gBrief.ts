...
export const TEXT4G_RENDERING_PROFILE_ID = 'text4g-digitizable-floorplan-v1' as const;

export type Text4gAreaUnit = 'sq_ft' | 'sq_m';
export type Text4gLengthUnit = 'ft' | 'm';
export type Text4gValueSource = 'user_confirmed' | 'model_inferred' | 'calculated' | 'application_default';
export type Text4gEnvelopeScope = 'building_footprint' | 'enclosed_plan_envelope';
export type Text4gBriefSection = 'project' | 'dimensions' | 'rooms' | 'adjacency' | 'planning' | 'floors';
export type Text4gRectangularBoundarySource = 'automatic_area_match' | 'explicit_user_request' | 'user_confirmed' | 'ineligible';

export interface ConfirmedText4gBrief {
  version: '1';
  project: {
    category: string;
    type: string;
  ...
  dimensions: {
    area: { value: number; unit: Text4gAreaUnit; source: Text4gValueSource };
    envelope: {
      width: number;
      depth: number;
      unit: Text4gLengthUnit;
      scope: Text4gEnvelopeScope;
      source: Text4gValueSource;
    };
    rectangularBoundary: {
      locked: boolean;
      source: Text4gRectangularBoundarySource;
    };
  };
  rooms: Array<{ id: string; name: string; count: number; details?: string; source: Text4gValueSource }>;
  adjacency: Array<{
    id: string;
    description: string;
  ...
  renderingProfileId: typeof TEXT4G_RENDERING_PROFILE_ID;
  provenance: {
    draftSource: 'design_copilot' | 'legacy_summary';
    confirmed: boolean;
    lastEditedSection?: Text4gBriefSection;
  };
}

export interface Text4gBriefValidation {
  valid: boolean;
  errors: string[];
  warnings: string[];
}

export interface Text4gRectangularBoundaryEligibility {
  eligible: boolean;
  coverageRatio: number;
  mismatchRatio: number;
}

export const getText4gRectangularBoundaryEligibility = (
  brief: Pick<ConfirmedText4gBrief, 'project' | 'dimensions'>,
): Text4gRectangularBoundaryEligibility => {
...
export const text4gHasExplicitRectangularRequest = (text: string): boolean => {
...
export const applyText4gRectangularBoundaryPolicy = (
  brief: ConfirmedText4gBrief,
  explicitRequestText = '',
): ConfirmedText4gBrief => {
  const eligibility = getText4gRectangularBoundaryEligibility(brief);
  const current = brief.dimensions.rectangularBoundary;
  const rectangularBoundary: ConfirmedText4gBrief['dimensions']['rectangularBoundary'] = !eligibility.eligible
    ? { locked: false, source: 'ineligible' }
    : text4gHasExplicitRectangularRequest(explicitRequestText)
...
export const setText4gRectangularBoundaryLock = (
  brief: ConfirmedText4gBrief,
  locked: boolean,
): ConfirmedText4gBrief => {
  const eligibility = getText4gRectangularBoundaryEligibility(brief);
...
const parseRooms = (value: string | null, source: Text4gValueSource): ConfirmedText4gBrief['rooms'] => {
...
const parseAdjacency = (value: string | null, source: Text4gValueSource): ConfirmedText4gBrief['adjacency'] => {
  if (value === null) return [];
  return value
    .split(/[\n\r]+|\s+[•*]\s+/)
    .map(line => line.trim().replace(/^[-*•\s]+/, '').trim())
...
const parseProject = (purposeText: string | null, previous?: ConfirmedText4gBrief) => {
...
export const createEmptyText4gBrief = (): ConfirmedText4gBrief => ({
  version: '1',
  ...
  renderingProfileId: TEXT4G_RENDERING_PROFILE_ID,
  provenance: { draftSource: 'design_copilot', confirmed: false },
});

export const legacySummaryToText4gBrief = (
  summary: string,
  previous?: ConfirmedText4gBrief,
  source: Text4gValueSource = 'model_inferred',
): ConfirmedText4gBrief => {
  ...
  const areaUnit: Text4gAreaUnit = metric ? 'sq_m' : 'sq_ft';
  const lengthUnit: Text4gLengthUnit = areaUnit === 'sq_m' ? 'm' : 'ft';
  ...
  const nextBrief: ConfirmedText4gBrief = {
    version: '1',
  ...
  return applyText4gRectangularBoundaryPolicy(nextBrief, summary);
};

export const updateText4gBriefFromSummary = (
  summary: string,
  previous: ConfirmedText4gBrief,
  editedSection: Text4gBriefSection,
): ConfirmedText4gBrief => {
  const next = legacySummaryToText4gBrief(summary, previous, 'model_inferred');
  ...
  return applyText4gRectangularBoundaryPolicy(next);
};
...
export const text4gBriefToDesignSummary = (brief: ConfirmedText4gBrief): string => {
...
export const validateText4gBrief = (brief: ConfirmedText4gBrief): Text4gBriefValidation => {
  const errors: string[] = [];
...
export const text4gBriefBoundaryMeters = (brief: ConfirmedText4gBrief): Point[] | undefined => {
