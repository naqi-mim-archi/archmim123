...
import { getText4fRectangularBoundaryEligibility } from './text4fBrief';

export interface Text4fAspectRaster {
  width: number;
  height: number;
  data: ArrayLike<number>;
}

export interface Text4fStructuralFrame {
  minX: number;
  maxX: number;
  minY: number;
  maxY: number;
...
export interface Text4fAspectCorrectionPlan {
  applied: boolean;
  targetRatio: number;
  sourceRatio: number;
...
export interface Text4fGeneratedImageAspectResult {
  dataUrl: string;
  applied: boolean;
  targetRatio?: number;
  sourceRatio?: number;
  correctedRatio?: number;
  relativeError?: number;
  reason: 'not-rectangle-locked' | 'frame-not-detected' | Text4fAspectCorrectionPlan['reason'];
}
 ...
 */
export const detectText4fLockedStructuralFrame = (
  raster: Text4fAspectRaster,
): Text4fStructuralFrame | undefined => {
  ...
  let bestFrame: Text4fStructuralFrame | undefined;
  let bestScore = -1;
...
export const planText4fLockedAspectCorrection = (
  frame: Pick<Text4fStructuralFrame, 'ratio'>,
  targetRatio: number,
  tolerance = 0.02,
): Text4fAspectCorrectionPlan => {
...
export const getText4fLockedImageAspectTarget = (
...
const loadImage = (dataUrl: string): Promise<HTMLImageElement> => new Promise((resolve, reject) => {
  ...
  image.onerror = () => reject(new Error('Generated Text 4.0 F image could not be decoded for aspect validation.'));
  image.src = dataUrl;
});
 ...
 */
export const enforceText4fGeneratedImageAspect = async (
  dataUrl: string,
  brief: ConfirmedText4fBrief,
): Promise<Text4fGeneratedImageAspectResult> => {
  const targetRatio = getText4fLockedImageAspectTarget(brief);
  ...
  const frame = detectText4fLockedStructuralFrame(sourceContext.getImageData(0, 0, width, height));
  if (!frame) return { dataUrl, applied: false, targetRatio, reason: 'frame-not-detected' };
  const plan = planText4fLockedAspectCorrection(frame, targetRatio);
  if (!plan.applied) {
    return {
      dataUrl,
      applied: false,
