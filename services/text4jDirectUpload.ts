...
export type Text4jDirectUploadUnitSystem = 'imperial' | 'metric';

export interface Text4jDirectUploadScaleInput {
  unitSystem: Text4jDirectUploadUnitSystem;
  width: string;
  depth: string;
  area: string;
}

export interface Text4jDirectUploadValidation {
  valid: boolean;
  errors: string[];
...
const propertyArea = (value: string, unitSystem: Text4jDirectUploadUnitSystem): number | undefined => {
  const normalized = normalizeMeasurement(value);
  if (!normalized) return undefined;
...
export const validateText4jDirectUploadScale = (
  input: Text4jDirectUploadScaleInput,
): Text4jDirectUploadValidation => {
  const errors: string[] = [];
...
export const prepareText4jDirectUpload = (
  sourceBrief: ConfirmedText4jBrief,
  input: Text4jDirectUploadScaleInput,
): PreparedText4jDirectUpload => {
  const validation = validateText4jDirectUploadScale(input);
