/**
 * Conservative preset resolution for Text 4.0 D digitization.
 *
...
export const TEXT4D_WALL_PRESETS = [
...
export const TEXT4D_DOOR_PRESETS = [
...
export const TEXT4D_WINDOW_PRESETS = [
...
export type Text4dEvidenceStrength = 'strong' | 'medium' | 'weak' | 'none';
export type Text4dResolutionConfidence = 'high' | 'medium' | 'low' | 'unresolved';
export type Text4dResolutionProvenance =
  | 'measured-preset'
  | 'measured-custom'
  | 'role-default'
  | 'semantic-default'
  | 'unresolved';
export type Text4dWallRole = 'exterior' | 'interior' | 'structural-interior' | 'partition' | 'glass';

export interface Text4dMeasuredValues {
  widthM: number | null;
  heightM: number | null;
  thicknessM: number | null;
}

export interface Text4dPresetResolution {
  kind: 'wall' | 'door' | 'window';
  presetId: string | null;
  presetLabel: string | null;
  subtype: string;
  measuredValues: Text4dMeasuredValues;
  resolvedValues: Text4dMeasuredValues;
  confidence: Text4dResolutionConfidence;
  provenance: Text4dResolutionProvenance;
...
export interface ResolveText4dWallInput {
  semanticRole: Text4dWallRole;
  projectClass?: string | null;
  measuredThicknessM?: number | null;
  evidenceStrength?: Text4dEvidenceStrength;
  /** Required for measured-to-preset snapping. No implicit tolerance is applied. */
  toleranceM?: number | null;
}

export interface ResolveText4dDoorInput {
...
export interface ResolveText4dWindowInput {
  measuredWidthM?: number | null;
  measuredHeightM?: number | null;
  detectedSubtype?: string | null;
  evidenceStrength?: Text4dEvidenceStrength;
...
const evidenceConfidence = (
  strength: Text4dEvidenceStrength,
  unresolved = false,
): Text4dResolutionConfidence => {
...
const emptyValues = (): Text4dMeasuredValues => ({ widthM: null, heightM: null, thicknessM: null });
...
const wallCandidates = (role: Text4dWallRole): readonly (typeof TEXT4D_WALL_PRESETS)[number][] => {
  if (role === 'exterior') return TEXT4D_WALL_PRESETS.filter(preset => preset.subtype === 'exterior');
  return TEXT4D_WALL_PRESETS.filter(preset => preset.subtype === role);
};

const semanticWallPresetId = (role: Text4dWallRole, projectClass?: string | null): string => {
...
export const resolveText4dWallPreset = (input: ResolveText4dWallInput): Text4dPresetResolution => {
  ...
  const measuredValues: Text4dMeasuredValues = { ...emptyValues(), thicknessM: measuredThicknessM };
  ...
  const preset = TEXT4D_WALL_PRESETS.find(candidate => candidate.id === presetId)!;
...
export const resolveText4dDoorPreset = (input: ResolveText4dDoorInput): Text4dPresetResolution => {
  ...
  const measuredValues: Text4dMeasuredValues = {
    ...emptyValues(),
  ...
  if (measuredWidthM !== null) {
    const knownSubtype = TEXT4D_DOOR_PRESETS.some(preset => preset.subtype === detectedSubtype);
    const candidates = knownSubtype
      ? TEXT4D_DOOR_PRESETS.filter(preset => preset.subtype === detectedSubtype)
      : detectedSubtype ? [] : [...TEXT4D_DOOR_PRESETS];
  ...
  const rolePreset = rolePresetId
    ? TEXT4D_DOOR_PRESETS.find(candidate => candidate.id === rolePresetId) || null
    : null;
  if (rolePreset) {
    return {
      kind: 'door',
...
export const resolveText4dWindowPreset = (input: ResolveText4dWindowInput): Text4dPresetResolution => {
  const measuredWidthM = positiveMeasurement(input.measuredWidthM);
  const measuredHeightM = positiveMeasurement(input.measuredHeightM);
  const toleranceM = explicitTolerance(input.toleranceM);
  const evidence = input.evidenceStrength || 'none';
  ...
  if (measuredWidthM !== null) {
    const knownSubtype = TEXT4D_WINDOW_PRESETS.some(preset => preset.subtype === detectedSubtype);
    ...
    const candidates = knownSubtype
      ? TEXT4D_WINDOW_PRESETS.filter(preset => preset.subtype === detectedSubtype)
      : detectedSubtype ? [] : TEXT4D_WINDOW_PRESETS.filter(preset => preset.subtype === 'regular');
    const nearest = candidates
      .map(preset => {
