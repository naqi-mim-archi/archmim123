...
import { recognizeFloorplanText4j, type FloorplanTextObservation } from './localFloorplanOcr4j';
import {
  analyzeText4jGeometryMode,
  detectText4jSparseAngularColumns,
  detectText4jSparseAngularRailings,
  detectText4jSparseAngularStairs,
  inspectText4jFreeformGap,
  recoverText4jFaintAngularWindowHosts,
  recoverText4jSupplementalAngularDoorHosts,
  type Text4jFreeformWallGap,
  type Text4jFreeformWallSegment,
  type Text4jGeometryMode,
  type Text4jPixelPoint,
} from './text4jGeometryModes';
import {
  consolidateText4jCurveArcs,
  text4jCurveArcSourceEnvelopeCoherentForTest,
  type Text4jNativeArcRun,
} from './text4jCurveArcs';
import {
  cleanupText4jAngularWalls,
  collapseText4jExteriorAngularChamfers,
} from './text4jAngularCleanup';
...
export interface LocalImageExtractionOptions {
  ...
  /** Text 4.0 J uses its confirmed width and depth as independent, authoritative axes. */
...
interface Text4jHostedAperture {
  pos: number[];
  rotation?: number;
  width?: number;
  measuredWidth?: number;
  evidence?: { confidence?: number; notes?: string[] };
}
 ...
 */
export const text4jHostedAperturesOverlap = (
  first: Text4jHostedAperture,
  second: Text4jHostedAperture,
) => {
...
export const arbitrateText4jHostedOpenings = <T extends Text4jHostedAperture>(
  doors: Text4jHostedAperture[],
  openings: T[],
) => {
  const ranked = openings
    .filter(opening => !doors.some(door => text4jHostedAperturesOverlap(door, opening)))
  ...
  return ranked.filter((opening, index) => !ranked.slice(0, index).some(retained =>
    text4jHostedAperturesOverlap(retained, opening)));
};

interface Text4jOpeningHostWall {
  p1: number[];
  p2: number[];
  wallSource?: string;
  isCurved?: boolean;
...
const text4jHostWallPoint = (wall: Text4jOpeningHostWall, progress: number) => {
...
export const text4jOpeningHasHost = (
  opening: Pick<Text4jHostedAperture, 'pos'>,
  walls: Text4jOpeningHostWall[],
  tolerance = 0.22,
) => walls.some(wall => {
  const curved = wall.wallSource === 'arc' || wall.wallSource === 'ellipse' || wall.isCurved;
  const sampleCount = curved ? 96 : 1;
  let previous = text4jHostWallPoint(wall, 0);
  for (let index = 1; index <= sampleCount; index++) {
    const current = text4jHostWallPoint(wall, index / sampleCount);
    ...
    image.onload = async () => {
      try {
        ...
        if (!textObservations && !options.disableOcr) {
          ...
          const ocrTask = recognizeFloorplanText4j(canvas);
          if (options.onGeometryReady) {
            try {
              ...
              console.warn('[Text 4.0 J] Early raster preview was unavailable; continuing to final digitization.', error);
            }
          }
          try {
            ...
            if (result === timeout) {
              console.warn(`[Text 4.0 J] Local OCR exceeded its ${budgetMs}ms budget; continuing with raster and design-brief signals.`);
              textObservations = [];
              ocrStatus = 'timed-out';
            } else {
            ...
            ocrDurationMs = performance.now() - ocrStartedAt;
            console.warn('[Text 4.0 J] Local OCR was unavailable; continuing with raster and design-brief signals.', error);
            textObservations = [];
...
/**
 * A door and a neighbouring window can be merged into one raster gap before
 * symbol classification. Preserve the non-overlapping glazing span instead
 * of allowing the inferred window to cover the evidenced door.
 */
export const arbitrateText4jHostedWindows = <T extends Text4jHostedAperture>(
  doors: Text4jHostedAperture[],
  windows: T[],
  ...
  const windowWidth = window.measuredWidth || window.width || 0;
  if (windowWidth <= 0) return [];
  let spans: Array<[number, number]> = [[-windowWidth / 2, windowWidth / 2]];
  doors.forEach(door => {
    const doorWidth = door.measuredWidth || door.width || 0;
    if (doorWidth <= 0) return;
    const rawRotationDelta = Math.abs(rotation - (door.rotation || 0)) % 180;
    if (Math.min(rawRotationDelta, 180 - rawRotationDelta) > 12) return;
    const deltaX = door.pos[0] - window.pos[0], deltaY = door.pos[1] - window.pos[1];
    const normalDistance = Math.abs(-deltaX * tangent.y + deltaY * tangent.x);
    if (normalDistance > Math.max(0.08, Math.min(0.16, Math.min(windowWidth, doorWidth) * 0.16))) return;
    const center = deltaX * tangent.x + deltaY * tangent.y;
    const blocked: [number, number] = [center - doorWidth / 2, center + doorWidth / 2];
    spans = spans.flatMap(([start, end]) => {
      if (blocked[1] <= start + 0.01 || blocked[0] >= end - 0.01) return [[start, end]];
      const remainder: Array<[number, number]> = [];
      if (blocked[0] - start >= 0.35) remainder.push([start, Math.min(end, blocked[0])]);
      if (end - blocked[1] >= 0.35) remainder.push([Math.max(start, blocked[1]), end]);
      return remainder;
    });
  });
  if (!spans.length) return [];
  const [start, end] = spans.sort((first, second) => (second[1] - second[0]) - (first[1] - first[0]))[0];
  if (end - start >= windowWidth - 0.01) return [window];
  const offset = (start + end) / 2;
  const retainedWidth = end - start;
  return [{
    ...window,
    pos: [window.pos[0] + tangent.x * offset, window.pos[1] + tangent.y * offset],
    width: retainedWidth,
    measuredWidth: retainedWidth,
    evidence: window.evidence ? {
      ...window.evidence,
      notes: [...(window.evidence.notes || []), 'Window span trimmed around an authoritative raster-evidenced door.'],
    } : window.evidence,
  }];
});

// Paired wall-opening dashes can use a lighter construction pen than door
// leaves. Keep this mask private to dashed-opening recognition so the extra
// tonal range cannot participate in wall, window, or door extraction.
const createDashedOpeningEvidenceMask = (image: RasterImageData) => {
  const pixelCount = image.width * image.height;
  const mask = new Uint8Array(pixelCount);
  for (let pixel = 0; pixel < pixelCount; pixel++) {
    const offset = pixel * 4;
    ...
    const luminance = 0.2126 * image.data[offset] + 0.7152 * image.data[offset + 1] + 0.0722 * image.data[offset + 2];
    const composited = Math.round(255 - alpha * (255 - luminance));
    mask[pixel] = composited <= 235 ? 1 : 0;
  }
  return mask;
};

export const createText4jDashedOpeningEvidenceMaskForTest = createDashedOpeningEvidenceMask;
...
export const resolveText4jMissingHostRecoveryScaleForTest = resolveMissingHostRecoveryScale;
...
const retainMainWallNetwork = (
  ...
  if (mainRoot === undefined) return [];

  const bounds = segmentBounds(segments);
  const sideTolerance = Math.max(2, typicalThickness * 2);
  type EnvelopeSide = 'top' | 'right' | 'bottom' | 'left';
  const componentSides = new Map<number, Set<EnvelopeSide>>();
  const componentAxes = new Map<number, Set<Axis>>();
  segments.forEach((segment, index) => {
    const root = find(index);
    const sides = componentSides.get(root) || new Set<EnvelopeSide>();
    const axes = componentAxes.get(root) || new Set<Axis>();
    axes.add(segment.axis);
    if (segment.axis === 'horizontal') {
      if (Math.abs(segment.line - bounds.minY) <= sideTolerance) sides.add('top');
      if (Math.abs(segment.line - bounds.maxY) <= sideTolerance) sides.add('bottom');
    } else {
      if (Math.abs(segment.line - bounds.minX) <= sideTolerance) sides.add('left');
      if (Math.abs(segment.line - bounds.maxX) <= sideTolerance) sides.add('right');
    }
    componentSides.set(root, sides);
    componentAxes.set(root, axes);
  });

  const mainScore = scores.get(mainRoot) || 0;
  const mainSides = componentSides.get(mainRoot) || new Set<EnvelopeSide>();
  const retainedRoots = new Set<number>([mainRoot]);
  scores.forEach((score, root) => {
    if (root === mainRoot) return;
    const sides = componentSides.get(root) || new Set<EnvelopeSide>();
    const axes = componentAxes.get(root) || new Set<Axis>();
    const sharedEnvelopeSide = Array.from(sides).some(side => mainSides.has(side));
    const substantial = score >= Math.max(0.75, mainScore * 0.12);
    // Multiple windows or doors can split one exterior shell into upper and
    // lower U-shaped components. Retain a substantial orthogonal companion
    // only when it occupies at least two true envelope sides and cooperates
    // with the dominant network on one of those same facade axes.
    if (substantial && axes.size >= 2 && sides.size >= 2 && sharedEnvelopeSide) retainedRoots.add(root);
  });
  return segments.filter((_, index) => retainedRoots.has(find(index)));
};

export const retainText4jCooperatingExteriorWallNetworksForTest = retainMainWallNetwork;
...
const rasterizeNativeText4jArcs = (
  barrier: Uint8Array,
  arcs: Array<{
    center: Text4jPixelPoint;
    radius: number;
    ellipseRadiusX?: number;
    ellipseRadiusY?: number;
    ellipseRotation?: number;
...
const freeformBounds = (segments: Text4jFreeformWallSegment[]): PixelBounds | undefined => {
...
const isFreeformExterior = (
  segment: Pick<Text4jFreeformWallSegment, 'p1' | 'p2' | 'thickness'>,
  outside: Uint8Array,
  width: number,
  height: number,
) => {
...
export const findText4jExplicitHostedOffsetSwingForTest = findExplicitHostedOffsetSwing;
 ...
 */
export const text4jClosedLeafFitsHostForTest = (
  leafLength: number,
  hostThickness: number,
) => leafLength <= hostThickness * 8;

export const text4jSwingQuadrantForTest = (
  axis: PixelGap['axis'],
  isFlipped: boolean,
  facingFlipped: boolean,
) => {
...
const recoverClosedSwingDoorGaps = (
  ...
  symbolSegments.forEach(leaf => {
    ...
    const hosts = hostSegments.filter(host => host.axis === leaf.axis
      ...
      && text4jClosedLeafFitsHostForTest(leafLength, host.thickness)
    ...
    hosts.forEach(host => {
      ...
      const leafNormalOffset = Math.abs(host.line - leaf.line);
      const alignedClosedLeaf = leafNormalOffset <= Math.max(1, typicalThickness * 0.2);
      // A separate near-closed leaf sits on or beyond the physical wall face.
      // Thin fixture/enclosure lines buried inside the structural wall band do
      // not establish a clean door component, even when sanitary curves nearby
      // happen to satisfy the arc sampler.
      if (!alignedClosedLeaf && leafNormalOffset < host.thickness * 0.95) return;
...
export const findText4jPhysicalWallFaceSwingForTest = (
...
export const recoverText4jLongWallEndpointSwingHostsForTest = recoverLongWallEndpointSwingDoorHosts;
...
/**
 * Wall-opening symbols are sometimes drafted as two interrupted parallel
 * strokes. Their repeated breaks distinguish them from continuous glazing
 * rails and from a solid wall band.
 */
const parallelDashedOpeningBandCount = (
  gap: PixelGap,
  mask: Uint8Array,
  width: number,
  height: number,
  ...
  const normalReach = Math.max(3, Math.ceil(gap.thickness * 1.5));
  const dashedOffsets: number[] = [];
  for (let offset = -normalReach; offset <= normalReach; offset++) {
    const samples: boolean[] = [];
    for (let along = from; along <= to; along++) {
      const x = gap.axis === 'horizontal' ? along : Math.round(gap.line + offset);
      const y = gap.axis === 'horizontal' ? Math.round(gap.line + offset) : along;
      samples.push(x >= 0 && y >= 0 && x < width && y < height && !!mask[y * width + x]);
    }
    const support = samples.filter(Boolean).length / Math.max(1, samples.length);
    if (support < 0.28 || support > 0.9) continue;
    let darkRuns = 0;
    let internalLightRuns = 0;
    let index = 0;
    while (index < samples.length) {
      const value = samples[index];
      const start = index;
      while (index < samples.length && samples[index] === value) index++;
      const runLength = index - start;
      if (value && runLength >= 2) darkRuns++;
      if (!value && start > 0 && index < samples.length && runLength >= 2) internalLightRuns++;
    }
    if (darkRuns >= 2 && internalLightRuns >= 1) dashedOffsets.push(offset);
  }
  let bands = 0;
  for (let index = 0; index < dashedOffsets.length;) {
    let end = index;
    while (end + 1 < dashedOffsets.length && dashedOffsets[end + 1] === dashedOffsets[end] + 1) end++;
    bands++;
    index = end + 1;
  }
  return bands;
...
export const text4jParallelDashedOpeningBandCountForTest = parallelDashedOpeningBandCount;

const continuousStructuralWallBandCount = (
  gap: PixelGap,
  mask: Uint8Array,
  width: number,
  height: number,
  ...
  if (to - from < Math.max(6, typicalThickness)) return 0;
  ...
  for (let offset = -normalReach; offset <= normalReach; offset++) {
    ...
    if (supported / Math.max(1, to - from + 1) >= 0.9) supportedOffsets.push(offset);
  }
  const minimumBandWidth = Math.max(2, Math.round(typicalThickness * 0.18));
  let structuralBands = 0;
  for (let index = 0; index < supportedOffsets.length;) {
    let end = index;
    while (end + 1 < supportedOffsets.length && supportedOffsets[end + 1] === supportedOffsets[end] + 1) end++;
    if (end - index + 1 >= minimumBandWidth) structuralBands++;
    index = end + 1;
  }
  return structuralBands;
};

interface Text4jHostedNonDoorRasterEvidence {
  exterior: boolean;
  explicitOpenPassage: boolean;
  dashedOpeningBandCount: number;
  windowFrame: boolean;
  windowFrameBandCount: number;
  continuousStructuralWallBandCount: number;
  wallBandInk: number;
  missingHostWindow: boolean;
}

export const resolveText4jHostedNonDoorRasterKindForTest = (
  evidence: Text4jHostedNonDoorRasterEvidence,
): 'window' | 'opening' | 'wall' | 'none' => {
  if (evidence.explicitOpenPassage || evidence.dashedOpeningBandCount >= 2) return 'opening';
  // Physical wall continuity is authoritative unless independently separated
  // frame bands and low total ink prove that those lines are glazing, not a
  // filled wall. A missing-host fallback alone is never sufficient.
  const separatedFrameEvidence = evidence.windowFrame
    && evidence.windowFrameBandCount >= 2
    && evidence.wallBandInk < (evidence.exterior ? 0.5 : 0.4);
  if (evidence.continuousStructuralWallBandCount > 0 && !separatedFrameEvidence) return 'wall';
  if (separatedFrameEvidence
    || (evidence.exterior && evidence.windowFrame
      && evidence.continuousStructuralWallBandCount === 0
      && evidence.wallBandInk < 0.55)) return 'window';
  return evidence.exterior ? 'none' : 'opening';
};
...
const hasTwoContinuousStructuralWallBands = (
  ...
  return continuousStructuralWallBandCount(gap, mask, width, height, typicalThickness) >= 2;
};
...
const detectGapSymbolEvidence = (
  ...
  if (gap.closedSwingEvidence) {
    ...
    return {
      ...
      windowFrame: Math.max(cleanWindowSupport, originalWindowSupport) >= 0.62,
      windowFrameSupport: Math.max(cleanWindowSupport, originalWindowSupport),
      windowFrameBandCount: Math.max(
  ...
  const cleanDoorSwing = cleanWeakSwing.detected && cleanWeakSwing.arcConfirmed && !cleanWeakSwing.arcOnly
    ? cleanWeakSwing
    : cleanHostedArc.confidence > cleanWeakSwing.confidence
    ? cleanHostedArc
    : cleanWeakSwing;
  ...
  if (gapOverlapsRecognizedText(gap, observations, typicalThickness)) {
    ...
    return {
      ...
      windowFrameBandCount: parallelWindowFrameBandCount(gap, cleanMask, width, height),
    };
  }
  ...
  const originalWindowSupport = parallelWindowFrameSupport(gap, originalMask, width, height);
  const swingCandidates = [originalWeakSwing, originalHostedArc, cleanDoorSwing];
  const explicitLeafAndArc = swingCandidates
    .filter(candidate => candidate.detected && candidate.arcConfirmed && !candidate.arcOnly)
    .sort((first, second) => second.confidence - first.confidence)[0];
  return {
    doorSwing: explicitLeafAndArc || swingCandidates
      .sort((first, second) => second.confidence - first.confidence)[0],
...
export const recoverText4jMissingHostedWindowOrSlidingForTest = recoverMissingHostedWindowOrSliding;
 ...
 */
export const clampText4jRecoveredSwingGapToJambFacesForTest = (
  gap: PixelGap,
  axisWalls: PixelSegment[],
  freeformWalls: Text4jFreeformWallSegment[],
  typicalThickness: number,
) => {
...
export const resolveText4jFramedInsetPixelsForTest = (
  gap: PixelGap,
...
export const recoverText4jMissingFacadeWallRunsForTest = recoverMissingFacadeWallRuns;
...
export const recoverText4jInteriorPartitionsForTest = recoverRasterSupportedInteriorPartitions;

interface Text4jTopologyRecoveryCandidate extends Text4jFreeformWallSegment {
  source: 'axis' | 'freeform';
}

const text4jRecoverySegmentLength = (segment: Pick<Text4jFreeformWallSegment, 'p1' | 'p2'>) =>
  Math.hypot(segment.p2.x - segment.p1.x, segment.p2.y - segment.p1.y);

const text4jNearestPointOnSegment = (
  point: Text4jPixelPoint,
  segment: Pick<Text4jFreeformWallSegment, 'p1' | 'p2'>,
) => {
  const dx = segment.p2.x - segment.p1.x, dy = segment.p2.y - segment.p1.y;
  const denominator = Math.max(1e-6, dx * dx + dy * dy);
  const progress = clamp(
    ((point.x - segment.p1.x) * dx + (point.y - segment.p1.y) * dy) / denominator,
    0,
    1,
  );
...
const text4jRecoverySegmentsOverlap = (
  candidate: Pick<Text4jFreeformWallSegment, 'p1' | 'p2'>,
  retained: Pick<Text4jFreeformWallSegment, 'p1' | 'p2'>,
  typicalThickness: number,
) => {
  const candidateLength = text4jRecoverySegmentLength(candidate);
  const retainedLength = text4jRecoverySegmentLength(retained);
  if (candidateLength <= 1e-6 || retainedLength <= 1e-6) return false;
  const candidateDirection = {
    x: (candidate.p2.x - candidate.p1.x) / candidateLength,
    y: (candidate.p2.y - candidate.p1.y) / candidateLength,
  };
  const retainedDirection = {
    x: (retained.p2.x - retained.p1.x) / retainedLength,
    y: (retained.p2.y - retained.p1.y) / retainedLength,
  };
  if (Math.abs(candidateDirection.x * retainedDirection.x + candidateDirection.y * retainedDirection.y)
    < Math.cos(7 * Math.PI / 180)) return false;
  const lineTolerance = Math.max(2, typicalThickness * 1.15);
  if (text4jNearestPointOnSegment(candidate.p1, retained).distance > lineTolerance
    && text4jNearestPointOnSegment(candidate.p2, retained).distance > lineTolerance) return false;
  const project = (point: Text4jPixelPoint) =>
    (point.x - retained.p1.x) * retainedDirection.x + (point.y - retained.p1.y) * retainedDirection.y;
  const first = project(candidate.p1), second = project(candidate.p2);
  const overlap = Math.max(0, Math.min(Math.max(first, second), retainedLength)
    - Math.max(Math.min(first, second), 0));
  // A retained jamb-side run may cover most of a partition while still
  // omitting the door-host continuation needed to close the room. Treat only
  // near-complete coverage as an already represented recovery candidate.
  return overlap >= candidateLength * 0.94;
};

const text4jRecoveryRasterSupport = (
  segment: Pick<Text4jFreeformWallSegment, 'p1' | 'p2' | 'thickness'>,
  mask: Uint8Array,
  width: number,
  height: number,
  ...
  const length = Math.max(1e-6, Math.hypot(dx, dy));
  const normal = { x: -dy / length, y: dx / length };
  const samples = Math.min(96, Math.max(16, Math.ceil(length / 4)));
  const radius = Math.max(1.5, Math.min(segment.thickness, typicalThickness * 1.5) * 0.48);
  const offsets = [-1, -0.5, 0, 0.5, 1];
  let supported = 0, ink = 0, total = 0;
  for (let step = 1; step < samples; step++) {
    const progress = step / samples;
    const center = {
      x: segment.p1.x + dx * progress,
      y: segment.p1.y + dy * progress,
    };
    let crossInk = 0, crossSamples = 0;
    offsets.forEach(offset => {
      const x = Math.round(center.x + normal.x * radius * offset);
      const y = Math.round(center.y + normal.y * radius * offset);
      if (x < 0 || y < 0 || x >= width || y >= height) return;
      crossSamples++;
      crossInk += mask[y * width + x];
    });
    if (crossInk >= Math.min(2, crossSamples)) supported++;
    ink += crossInk;
    total += crossSamples;
  }
  return {
    supportedRatio: supported / Math.max(1, samples - 1),
    inkRatio: ink / Math.max(1, total),
  };
};

const text4jTagsMayShareOpenPlan = (first: RoomTextTag, second: RoomTextTag) => {
  const a = first.label.toLowerCase(), b = second.label.toLowerCase();
  const commonArea = (label: string) => /living|dining|kitchen/.test(label);
  if (!commonArea(a) || !commonArea(b)) return false;
  if ((/living/.test(a) && /dining/.test(b)) || (/dining/.test(a) && /living/.test(b))) return true;
  return /open.?plan|integrated/.test(a) || /open.?plan|integrated/.test(b);
};

const text4jTagRegionId = (
  flood: FloodResult,
  tag: RoomTextTag,
  width: number,
  height: number,
  ...
  const centerX = clamp(Math.round(tag.x), 0, width - 1);
  const centerY = clamp(Math.round(tag.y), 0, height - 1);
  const direct = flood.labels[centerY * width + centerX];
  ...
  for (let radius = 1; radius <= searchRadius; radius++) {
    for (let x = Math.max(0, centerX - radius); x <= Math.min(width - 1, centerX + radius); x++) {
      for (const y of [centerY - radius, centerY + radius]) {
        ...
        const label = flood.labels[y * width + x];
        if (label) return label;
      }
    }
    for (let y = Math.max(0, centerY - radius + 1); y < Math.min(height, centerY + radius); y++) {
...
const text4jTaggedTopologyUtility = (
  flood: FloodResult,
  roomTextTags: RoomTextTag[],
  width: number,
  height: number,
  ...
  const regionIds = roomTextTags.map(tag => text4jTagRegionId(
    flood, tag, width, height, Math.max(2, Math.round(typicalThickness * 1.5)),
  ));
  const validTags = regionIds.filter(Boolean).length;
  let separatedPairs = 0;
  for (let first = 0; first < roomTextTags.length; first++) {
    for (let second = first + 1; second < roomTextTags.length; second++) {
      if (text4jTagsMayShareOpenPlan(roomTextTags[first], roomTextTags[second])) continue;
      if (regionIds[first] && regionIds[second] && regionIds[first] !== regionIds[second]) separatedPairs++;
    }
  }
  return validTags * 100 + separatedPairs;
};

const text4jRoomTopologyAudit = (
  flood: FloodResult,
  roomTextTags: RoomTextTag[],
  width: number,
  height: number,
  ...
  if (roomTextTags.length < 3) {
    return { expectedGroups: roomTextTags.length, observedRegions: roomTextTags.length, severe: false };
  }
  const parent = roomTextTags.map((_, index) => index);
  ...
  for (let first = 0; first < roomTextTags.length; first++) {
    for (let second = first + 1; second < roomTextTags.length; second++) {
      if (text4jTagsMayShareOpenPlan(roomTextTags[first], roomTextTags[second])) {
        union(first, second);
        continue;
      }
      const firstLabel = roomTextTags[first].label.toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim();
      const secondLabel = roomTextTags[second].label.toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim();
      const nearbyDuplicate = firstLabel === secondLabel
        && Math.hypot(
          roomTextTags[first].x - roomTextTags[second].x,
          roomTextTags[first].y - roomTextTags[second].y,
        ) <= typicalThickness * 7;
      if (nearbyDuplicate) union(first, second);
    }
  }
  const expectedGroups = new Set(roomTextTags.map((_, index) => find(index))).size;
  const observedRegions = new Set(roomTextTags.map(tag => text4jTagRegionId(
    flood, tag, width, height, Math.max(2, Math.round(typicalThickness * 1.5)),
  )).filter(Boolean)).size;
  const deficit = expectedGroups - observedRegions;
  return {
    expectedGroups,
    observedRegions,
    severe: expectedGroups >= 3
      && deficit >= 2
      && observedRegions / Math.max(1, expectedGroups) < 0.75,
  };
};

const recoverTopologyValidatedInteriorPartitions = (
  axisSegments: PixelSegment[],
  freeformSegments: Text4jFreeformWallSegment[],
  axisCandidates: PixelSegment[],
  freeformCandidates: Text4jFreeformWallSegment[],
  mask: Uint8Array,
  width: number,
  height: number,
  typicalThickness: number,
  ...
  const unchanged = { axisSegments, freeformSegments, recoveredCount: 0 };
  if (roomTextTags.length < 2) return unchanged;
  const asFreeform = (segment: PixelSegment): Text4jTopologyRecoveryCandidate => segment.axis === 'horizontal'
    ? {
  ...
  const retained: Text4jTopologyRecoveryCandidate[] = [
    ...axisSegments.map(asFreeform),
    ...freeformSegments.map(segment => ({ ...segment, source: 'freeform' as const })),
  ];
  const minimumLengthPixels = Math.max(
    typicalThickness * 4.5,
    0.55 / Math.max(1e-6, Math.min(pixelScale.x, pixelScale.y)),
  );
  const maximumThickness = typicalThickness * 2.2;
  const rawCandidates: Text4jTopologyRecoveryCandidate[] = [
    ...axisCandidates.map(asFreeform),
    ...freeformCandidates.map(segment => ({ ...segment, source: 'freeform' as const })),
  ].filter(candidate => {
    const length = text4jRecoverySegmentLength(candidate);
    if (length < minimumLengthPixels
      // Match the upstream structural-band threshold. Interior partitions are
      // often half the exterior wall width and can land just below 50% after
      // raster quantization, as in DS14/1401 (12 px versus a 26 px facade).
      || candidate.thickness < typicalThickness * 0.38
      || candidate.thickness > maximumThickness) return false;
    if (retained.some(segment => text4jRecoverySegmentsOverlap(candidate, segment, typicalThickness))) return false;
    const support = text4jRecoveryRasterSupport(candidate, mask, width, height, typicalThickness);
    return support.supportedRatio >= 0.68 && support.inkRatio >= 0.24;
  }).sort((a, b) => text4jRecoverySegmentLength(b) - text4jRecoverySegmentLength(a))
    // Keep the quadratic anchor graph and trial floods bounded on detailed
    // presentation plans. Long structural runs are considered first.
    .slice(0, 180);
  const candidates: Text4jTopologyRecoveryCandidate[] = [];
  rawCandidates.forEach(candidate => {
    if (!candidates.some(existing => text4jRecoverySegmentsOverlap(candidate, existing, typicalThickness))) {
      candidates.push({ ...candidate, p1: { ...candidate.p1 }, p2: { ...candidate.p2 } });
    }
  });
  if (!candidates.length) return unchanged;

  const anchorTolerance = Math.max(3, typicalThickness * 2.2);
  const existingAnchor = (point: Text4jPixelPoint) => retained
    .map(segment => ({ segment, ...text4jNearestPointOnSegment(point, segment) }))
    .filter(anchor => anchor.distance <= anchorTolerance)
    .sort((a, b) => a.distance - b.distance)[0];
  const candidateAnchor = (point: Text4jPixelPoint, ownIndex: number) => candidates
    .flatMap((segment, index) => index === ownIndex
      ? []
      : [{ index, segment, ...text4jNearestPointOnSegment(point, segment) }])
    .filter(anchor => anchor.distance <= anchorTolerance)
    .sort((a, b) => a.distance - b.distance)[0];
  const links = candidates.map(() => new Set<number>());
  const endpointAnchors = candidates.map((candidate, index) => ([candidate.p1, candidate.p2].map(point => {
    const existing = existingAnchor(point);
    if (existing) return { existing, candidate: undefined };
    const linked = candidateAnchor(point, index);
    if (linked) links[index].add(linked.index);
    return { existing: undefined, candidate: linked };
  })));
  links.forEach((connected, index) => connected.forEach(other => links[other].add(index)));
  const separatesIndependentRoomTags = (candidate: Text4jTopologyRecoveryCandidate) => {
    const dx = candidate.p2.x - candidate.p1.x;
    const dy = candidate.p2.y - candidate.p1.y;
    const length = Math.hypot(dx, dy);
    if (length < Math.max(minimumLengthPixels, 1.2 / Math.max(1e-6, Math.min(pixelScale.x, pixelScale.y)))) {
      return false;
    }
    const taggedSides = roomTextTags.flatMap(tag => {
      const relativeX = tag.x - candidate.p1.x;
      const relativeY = tag.y - candidate.p1.y;
      const along = (relativeX * dx + relativeY * dy) / Math.max(1e-6, length * length);
      const signedDistance = (relativeX * -dy + relativeY * dx) / Math.max(1e-6, length);
      if (along < -0.12 || along > 1.12 || Math.abs(signedDistance) < typicalThickness * 1.5) return [];
      return [{ tag, side: Math.sign(signedDistance) }];
    });
    return taggedSides.some(first => taggedSides.some(second =>
      first.side !== second.side && !text4jTagsMayShareOpenPlan(first.tag, second.tag)));
  };
  const componentIds = new Int32Array(candidates.length);
  ...
  for (let start = 0; start < candidates.length; start++) {
    ...
    while (queue.length) {
      ...
      links[index].forEach(other => {
        if (componentIds[other]) return;
        componentIds[other] = id;
        queue.push(other);
      });
    }
  ...
  const viableComponents = components.filter(component => {
    ...
    for (const index of component) {
      for (const anchor of endpointAnchors[index]) {
        if (anchor.existing) retainedAnchors++;
        else if (!anchor.candidate || !componentSet.has(anchor.candidate.index)) freeEndpoints++;
      }
    }
    if (!freeEndpoints) return retainedAnchors >= 2;
    // A genuine room partition may terminate at a door jamb rather than at a
    // second retained wall. Permit exactly one such endpoint only when a long
    // observed run separates independent OCR room labels on opposite sides.
    return freeEndpoints === 1
      && retainedAnchors >= 1
      && component.some(index => separatesIndependentRoomTags(candidates[index]));
  }).map(component => component.map(index => candidates[index]))
    .sort((a, b) =>
      b.reduce((sum, segment) => sum + text4jRecoverySegmentLength(segment), 0)
        - a.reduce((sum, segment) => sum + text4jRecoverySegmentLength(segment), 0))
    .slice(0, 32);
  if (!viableComponents.length) return unchanged;

  const snapCandidates = viableComponents.flat();
  snapCandidates.forEach((candidate, ownIndex) => {
    (['p1', 'p2'] as const).forEach(endpoint => {
      const sourcePoint = candidate[endpoint];
      const anchors = [
        ...retained.map(segment => ({ ...text4jNearestPointOnSegment(sourcePoint, segment), segment })),
        ...snapCandidates.flatMap((segment, index) => index === ownIndex
          ? []
          : [{ ...text4jNearestPointOnSegment(sourcePoint, segment), segment }]),
      ].filter(anchor => anchor.distance <= anchorTolerance)
        .sort((a, b) => a.distance - b.distance);
      const nearest = anchors[0];
      if (!nearest || nearest.distance <= 0.01) return;
      const extension = {
        p1: sourcePoint,
        p2: nearest.point,
        thickness: candidate.thickness,
      };
      const support = text4jRecoveryRasterSupport(extension, mask, width, height, typicalThickness);
      if (nearest.distance <= typicalThickness * 1.2 || support.supportedRatio >= 0.42) {
        candidate[endpoint] = { ...nearest.point };
      }
    });
  });

  const floodWithComponents = (included: typeof viableComponents) => {
    const { barrier } = rasterizeWalls(axisSegments, width, height, typicalThickness);
    rasterizeFreeformWalls(barrier, freeformSegments, width, height);
    rasterizeFreeformWalls(barrier, included.flat(), width, height);
    return floodSpaces(barrier, width, height, pixelScale);
  };
  const baseFlood = floodWithComponents([]);
  const baseUtility = text4jTaggedTopologyUtility(
    baseFlood, roomTextTags, width, height, typicalThickness,
  );
  const fullFlood = floodWithComponents(viableComponents);
  const targetUtility = text4jTaggedTopologyUtility(
    fullFlood, roomTextTags, width, height, typicalThickness,
  );
  const doorTerminatedComponents = viableComponents.filter(component =>
    component.some(separatesIndependentRoomTags));
  if (targetUtility <= baseUtility && !doorTerminatedComponents.length) return unchanged;
  let acceptedComponents = targetUtility > baseUtility
    ? [...viableComponents]
    : [...doorTerminatedComponents];
  const removalOrder = [...viableComponents].sort((a, b) =>
    a.reduce((sum, segment) => sum + text4jRecoverySegmentLength(segment), 0)
      - b.reduce((sum, segment) => sum + text4jRecoverySegmentLength(segment), 0));
  if (targetUtility > baseUtility) {
    removalOrder.forEach(component => {
      const trial = acceptedComponents.filter(candidate => candidate !== component);
      const utility = text4jTaggedTopologyUtility(
        floodWithComponents(trial), roomTextTags, width, height, typicalThickness,
      );
      if (utility >= targetUtility) acceptedComponents = trial;
    });
  }
  const accepted = acceptedComponents.flat();
  if (!accepted.length) return unchanged;
  const recoveredAxis: PixelSegment[] = [];
  const recoveredFreeform: Text4jFreeformWallSegment[] = [];
  accepted.forEach(candidate => {
    const dx = candidate.p2.x - candidate.p1.x, dy = candidate.p2.y - candidate.p1.y;
    if (candidate.source === 'axis' && Math.min(Math.abs(dx), Math.abs(dy)) <= typicalThickness * 0.75) {
      if (Math.abs(dx) >= Math.abs(dy)) recoveredAxis.push({
        axis: 'horizontal',
        line: (candidate.p1.y + candidate.p2.y) / 2,
        start: Math.min(candidate.p1.x, candidate.p2.x),
        end: Math.max(candidate.p1.x, candidate.p2.x),
        thickness: candidate.thickness,
      });
      else recoveredAxis.push({
        axis: 'vertical',
        line: (candidate.p1.x + candidate.p2.x) / 2,
        start: Math.min(candidate.p1.y, candidate.p2.y),
        end: Math.max(candidate.p1.y, candidate.p2.y),
        thickness: candidate.thickness,
      });
      return;
    }
    recoveredFreeform.push({
      p1: { ...candidate.p1 },
      p2: { ...candidate.p2 },
      thickness: candidate.thickness,
      confidence: Math.max(0.72, candidate.confidence),
    });
  });
  return {
    axisSegments: deduplicateParallelSegments([...axisSegments, ...recoveredAxis], typicalThickness),
    freeformSegments: [...freeformSegments, ...recoveredFreeform],
    recoveredCount: recoveredAxis.length + recoveredFreeform.length,
  };
};

export const recoverText4jTopologyValidatedPartitionsForTest = recoverTopologyValidatedInteriorPartitions;

export const text4jInteriorArchitecturalCurveEvidenceForTest = (
  arc: Text4jNativeArcRun,
  sourceSegments: Text4jFreeformWallSegment[],
  curveSourceSegments: Text4jFreeformWallSegment[],
  axisSegments: PixelSegment[],
  width: number,
  height: number,
  typicalThickness: number,
  active: boolean,
  nestCenter?: Text4jPixelPoint,
  ...
  const rotation = arc.ellipseRotation || 0;
  const cosR = Math.cos(rotation), sinR = Math.sin(rotation);
  const parameter = (point: Text4jPixelPoint) => {
  ...
  const curveResidual = (point: Text4jPixelPoint) => {
    const dx = point.x - arc.center.x, dy = point.y - arc.center.y;
    const localX = dx * cosR + dy * sinR;
    const localY = -dx * sinR + dy * cosR;
...
export interface Text4jConcentricInteriorRingEvidence {
  center: Text4jPixelPoint;
  radius: number;
  support: number;
...
export const text4jShouldRetainHybridCurveModeForTest = (
  geometryMode: Text4jGeometryMode,
...
export const text4jOpenShellExteriorCurveFamilyEvidenceForTest = (
  arc: Text4jNativeArcRun,
  sourceSegments: Text4jFreeformWallSegment[],
 ...
 */
export const text4jDominantEllipseGapHasRasterEvidenceForTest = (
 ...
 */
export const recoverText4jConcentricInteriorRingForTest = (
  darkMask: Uint8Array,
  width: number,
  height: number,
  typicalThickness: number,
  ...
  active: boolean,
): Text4jConcentricInteriorRingEvidence | undefined => {
...
/** Pure raster-to-geometry rule engine used by Text 4.0 J. */
export const classifyText4jRasterGeometry = (
  image: RasterImageData,
  ...
  return analyzeText4jGeometryMode(
    darkMask,
    width,
    height,
...
export const extractGeometryFromImageData = (
  ...
  const dashedOpeningEvidenceMask = createDashedOpeningEvidenceMask(image);
  ...
  let geometryModeAnalysis = analyzeText4jGeometryMode(
    darkMask,
    width,
    height,
    estimateTypicalWallThickness(rawSegments),
  ...
  if (textObservations.length && !roomTextTags.length && geometryModeAnalysis.mode !== 'orthogonal') {
    ...
    const unstrippedModeAnalysis = analyzeText4jGeometryMode(
      originalDarkMask,
      width,
      height,
  ...
  let geometryMode: Text4jGeometryMode = geometryModeAnalysis.mode;
  ...
  const faintAngularWindowGaps = new Set<Text4jFreeformWallGap>();
  ...
  // Recover strongly raster-supported facade bands while all structural
  // components are still available. Once component selection has discarded a
  // split upper/lower shell, the former post-selection pass cannot find it.
  const preRetentionFacadeSegments = geometryMode === 'orthogonal'
    ? recoverMissingFacadeWallRuns(
    ...
    : networkConnectedSegments;
  let usableSegments = retainMainWallNetwork(preRetentionFacadeSegments, preliminaryScale, typicalThickness);
  usableSegments = retainTaggedInteriorWallBoxes(
    preRetentionFacadeSegments,
    usableSegments,
    roomTextTags,
    preliminaryScale,
    typicalThickness,
  ...
  if (geometryMode === 'hybrid') {
    const recoveredAngularWindows = recoverText4jFaintAngularWindowHosts(
      originalDarkMask,
      originalDoorEvidenceMask,
      width,
      height,
  ...
  let topologyRecoveredPartitionCount = 0;
  ...
  if (!componentSheetMode && roomTextTags.length >= 2) {
    // Main-network selection intentionally works from longer wall runs, but a
    // real partition can be split or shortened around a hosted door. Rebuild
    // the second-chance pool from pre-retention and merged structural runs;
    // topology recovery still requires raster support, two-ended anchoring,
    // and a measurable improvement to OCR-labelled room enclosure.
    const topologyAxisCandidates = mergeWallsAcrossHostedGaps(
    ...
    const recoveredTopology = recoverTopologyValidatedInteriorPartitions(
      usableSegments,
      freeformSegments,
      topologyAxisCandidates,
      geometryMode === 'orthogonal' ? [] : geometryModeAnalysis.candidateSegments || [],
      originalDarkMask,
      width,
      height,
      typicalThickness,
    ...
    usableSegments = recoveredTopology.axisSegments;
    freeformSegments = recoveredTopology.freeformSegments;
    topologyRecoveredPartitionCount = recoveredTopology.recoveredCount;
  }
  ...
  const frameAxisBounds = geometryMode === 'orthogonal'
    ...
    : wallBounds;
  const frameLabelMargin = Math.max(4, typicalThickness * 2.5);
  const frameValidationTags = roomTextTags.filter(tag =>
    !/balcony|terrace|loggia|porch|deck/i.test(tag.label));
  const roomTagsOutsideSelectedFrame = frameValidationTags.filter(tag =>
    tag.x < frameAxisBounds.minX - frameLabelMargin
    || tag.x > frameAxisBounds.maxX + frameLabelMargin
    || tag.y < frameAxisBounds.minY - frameLabelMargin
    || tag.y > frameAxisBounds.maxY + frameLabelMargin);
  const selectedFrameMissesRoomLabels = roomTagsOutsideSelectedFrame.length >= 2
    || roomTagsOutsideSelectedFrame.length / Math.max(1, frameValidationTags.length) >= 0.34;
  ...
  const ordinaryPixelColumns = componentSheetMode
    ...
    : geometryMode === 'orthogonal'
      ...
      : [];
  const curvilinearColumnSegments: Text4jFreeformWallSegment[] = smoothCurvilinearCandidate
    ? [
      ...freeformSegments,
  ...
  const supplementalAngularStairs = (sparseHybridSymbols || smoothCurvilinearCandidate) && !ordinaryPixelStairs.length
    ? detectText4jSparseAngularStairs(originalDoorEvidenceMask, originalDarkMask, width, height, typicalThickness)
    : [];
  ...
  const detectedAngularRailings = sparseHybridSymbols || smoothCurvilinearCandidate
    ? detectText4jSparseAngularRailings(createCoolGrayRailingMask(image), width, height, typicalThickness)
    : [];
  ...
  if (sparseAngularComponentSheetMode) {
    const recoveredAngularDoors = recoverText4jSupplementalAngularDoorHosts(
      originalDarkMask,
      originalDoorEvidenceMask,
      width,
      height,
  ...
  const hostedGapsForWallMerge = architecturalHostedGaps.map(gap =>
    clampText4jRecoveredSwingGapToJambFacesForTest(
      gap,
      outputWallSegments,
      freeformSegments,
      typicalThickness,
  ...
  const pointSegmentDistance = (point: Text4jPixelPoint, first: PixelPoint, second: PixelPoint) => {
  ...
  const segmentOnFootprintEdge = (segment: Text4jFreeformWallSegment, toleranceFactor = 2.2) => {
  ...
  const segmentHasSeparatedGlazingBands = (segment: Text4jFreeformWallSegment) => {
    const dx = segment.p2.x - segment.p1.x, dy = segment.p2.y - segment.p1.y;
    const length = Math.hypot(dx, dy);
  ...
  const curvedExteriorNeighbourCount = (segment: Text4jFreeformWallSegment, maximumAngleDelta: number) => {
  ...
  const nativeCurveMode: Text4jGeometryMode = text4jShouldRetainHybridCurveModeForTest(
    geometryMode,
  ...
  const interiorArchitecturalCurveEvidence = (
    ...
    return text4jInteriorArchitecturalCurveEvidenceForTest(
      arc,
      sourceSegments,
      curveSourceSegments,
      outputWallSegments,
  ...
  const openShellExteriorCurveEvidence = (
    ...
    return text4jOpenShellExteriorCurveFamilyEvidenceForTest(
      arc,
      sourceSegments,
      outsideSourceCount,
      width,
  ...
  const consolidatedFreeform = consolidateText4jCurveArcs(curveSourceSegments, {
  ...
  const recoverNestedInteriorCurve = () => {
    ...
    if (dominantOuterEllipse) {
      ...
      if (nestedSegments.length >= 4) {
        const nestedConsolidated = consolidateText4jCurveArcs(nestedSegments, {
          mode: 'curved',
          structuralMask: originalDarkMask,
          width,
          height,
        ...
        const acceptedNested = nestedConsolidated.arcs.filter(arc => {
          ...
          const sources = familySourceIndices
            ...
            .filter((segment): segment is Text4jFreeformWallSegment => !!segment);
          return text4jInteriorArchitecturalCurveEvidenceForTest(
            arc,
            sources,
            curveSourceSegments,
        ...
        if (acceptedNested.length) {
          const acceptedSources = new Set<Text4jFreeformWallSegment>();
      ...
      if (!hasNestedFamily && nestedSegments.length >= 4) {
        const ring = recoverText4jConcentricInteriorRingForTest(
          originalDarkMask,
          width,
          height,
          typicalThickness,
  ...
  if (curveEvidenceMode && consolidatedFreeform.arcs.length >= 2) {
    ...
    if (ellipseArcs.length) {
      const ellipseParameter = (ellipse: ConsolidatedArc, point: Text4jPixelPoint) => {
      ...
      const ellipsePointAt = (ellipse: ConsolidatedArc, angle: number): Text4jPixelPoint => {
        const radiusX = ellipse.ellipseRadiusX!;
        const radiusY = ellipse.ellipseRadiusY!;
        const rotation = ellipse.ellipseRotation || 0;
        const cosR = Math.cos(rotation), sinR = Math.sin(rotation);
  ...
  if (curveEvidenceMode && consolidatedFreeform.arcs.length >= 1) {
    ...
    families.forEach(family => {
      ...
      const acceptCurveGap = (
        ...
        const gap: Text4jFreeformWallGap = { p1, p2, thickness: typicalThickness, confidence: 0.82 };
        const evidence = inspectText4jFreeformGap(gap, originalDarkMask, originalDoorEvidenceMask, width, height);
      ...
      const hasStructuralInk = (point: Text4jPixelPoint) => {
    ...
    canonicalEllipseFamilies.forEach(family => {
      ...
      const pointAt = (angle: number): Text4jPixelPoint => {
        const cosR = Math.cos(rotation), sinR = Math.sin(rotation);
        const localX = Math.cos(angle) * radiusX;
        const localY = Math.sin(angle) * radiusY;
        return {
  ...
  if (curveEvidenceMode && consolidatedFreeform.arcs.length) {
    ...
    const curveParameterDistance = (arc: (typeof consolidatedFreeform.arcs)[number], point: Text4jPixelPoint) => {
    ...
    const curvePointAt = (arc: (typeof consolidatedFreeform.arcs)[number], angle: number): Text4jPixelPoint => {
      const radiusX = arc.ellipseRadiusX ?? arc.radius;
      const radiusY = arc.ellipseRadiusY ?? arc.radius;
      const rotation = arc.ellipseRotation || 0;
      const cosR = Math.cos(rotation), sinR = Math.sin(rotation);
    ...
    const curveTangentAt = (arc: (typeof consolidatedFreeform.arcs)[number], angle: number): Text4jPixelPoint => {
      const radiusX = arc.ellipseRadiusX ?? arc.radius;
      const radiusY = arc.ellipseRadiusY ?? arc.radius;
      const rotation = arc.ellipseRotation || 0;
      const cosR = Math.cos(rotation), sinR = Math.sin(rotation);
    ...
    if (nativeCurveMode === 'hybrid') {
      ...
      if (dominantEllipse) {
        ...
        const supported = Array.from({ length: sampleCount }, (_, index) => {
          ...
          return text4jDominantEllipseGapHasRasterEvidenceForTest(
            structuralSupport,
            symbolSupport,
          );
        });
    ...
    if (nativeCurveMode === 'hybrid') {
      const rejectedSources = new Set<Text4jFreeformWallSegment>();
      const coherentArcs = consolidatedFreeform.arcs.filter(arc => {
        const sources = lateCurveFamilySources(arc);
        const coherent = text4jCurveArcSourceEnvelopeCoherentForTest(
          arc,
          sources,
          width,
          height,
    ...
    recoverNestedInteriorCurve();
    rasterizeNativeText4jArcs(barrier, consolidatedFreeform.arcs, width, height, typicalThickness);
  ...
  const cleanedAngularWalls = cleanupText4jAngularWalls(
    consolidatedFreeform.retainedSegments,
    outputWallSegments,
    {
      mode: geometryMode,
  ...
  const curveApertureChord = (segment: Text4jFreeformWallSegment) => {
    ...
    return consolidatedFreeform.arcs.some(arc => {
      const radiusX = arc.ellipseRadiusX ?? arc.radius;
      const radiusY = arc.ellipseRadiusY ?? arc.radius;
      const endpointResidual = (point: Text4jPixelPoint) => Math.abs(Math.hypot(
  ...
  const duplicatesAxisWall = (segment: Text4jFreeformWallSegment) => {
    if (!curveEvidenceMode) return false;
    const dx = segment.p2.x - segment.p1.x, dy = segment.p2.y - segment.p1.y;
  ...
  const freeformStructuralCoreRatio = (segment: Text4jFreeformWallSegment) => {
    const dx = segment.p2.x - segment.p1.x, dy = segment.p2.y - segment.p1.y;
    const length = Math.max(1e-6, Math.hypot(dx, dy));
    const normal = { x: -dy / length, y: dx / length };
  ...
  // Circular plans commonly express room divisions as straight radial
  // spokes. Skeleton/curve consolidation can consume them before export, so
  // inspect the source raster directly. A candidate must contain a long,
  // filled wall-width core; thin leaves, glazing, furniture and text cannot
  // satisfy the transverse samples used here.
  const recoverRasterRadialPartitions = () => {
    if (!curveEvidenceMode || !consolidatedFreeform.arcs.length) return [] as Text4jFreeformWallSegment[];
    const outerArc = [...consolidatedFreeform.arcs].sort((first, second) => {
      const firstRadius = Math.min(first.ellipseRadiusX ?? first.radius, first.ellipseRadiusY ?? first.radius);
      const secondRadius = Math.min(second.ellipseRadiusX ?? second.radius, second.ellipseRadiusY ?? second.radius);
      return secondRadius - firstRadius;
    })[0];
    const outerRadius = Math.min(
      outerArc.ellipseRadiusX ?? outerArc.radius,
      outerArc.ellipseRadiusY ?? outerArc.radius,
    );
    if (outerRadius < Math.min(width, height) * 0.22) return [] as Text4jFreeformWallSegment[];
    const center = outerArc.center;
    // The exterior ring is commonly two to three times heavier than its room
    // partitions. Cap the transverse probe to an evidenced interior-wall
    // scale so that a genuine spoke is not rejected merely because the global
    // thickness estimate came from the outer shell.
    const radialProbeThickness = Math.min(
      typicalThickness,
      Math.max(8, outerRadius * 0.045),
    );
    const minimumRadius = Math.max(radialProbeThickness * 2, outerRadius * 0.1);
    const maximumRadius = outerRadius - Math.max(radialProbeThickness * 0.8, 2);
    const maximumLightBridge = Math.max(1, Math.round(radialProbeThickness * 0.22));
    type RadialRun = { angle: number; start: number; end: number; support: number };
    const detections: RadialRun[] = [];
    for (let degrees = 0; degrees < 360; degrees++) {
      const angle = degrees * Math.PI / 180;
      const direction = { x: Math.cos(angle), y: Math.sin(angle) };
      const normal = { x: -direction.y, y: direction.x };
      let runStart = -1, lastDark = -1, lightBridge = 0, supported = 0, samples = 0;
      let best: RadialRun | undefined;
      const finishRun = () => {
        if (runStart < 0 || lastDark < runStart) return;
        const runLength = lastDark - runStart;
        const support = supported / Math.max(1, samples);
        if (runLength >= Math.max(radialProbeThickness * 3.5, outerRadius * 0.16)
          && support >= 0.72
          && (!best || runLength * support > (best.end - best.start) * best.support)) {
          best = { angle, start: runStart, end: lastDark, support };
        }
      };
      for (let radius = Math.ceil(minimumRadius); radius <= Math.floor(maximumRadius); radius++) {
        let transverseInk = 0, transverseSamples = 0;
        for (const factor of [-0.38, -0.19, 0, 0.19, 0.38]) {
          const x = Math.round(center.x + direction.x * radius + normal.x * radialProbeThickness * factor);
          const y = Math.round(center.y + direction.y * radius + normal.y * radialProbeThickness * factor);
          if (x < 0 || y < 0 || x >= width || y >= height) continue;
          transverseSamples++;
          transverseInk += originalDarkMask[y * width + x];
        }
        const wallCore = transverseSamples >= 3 && transverseInk / transverseSamples >= 0.6;
        if (wallCore) {
          ...
          supported++;
          samples++;
        } else if (runStart >= 0 && lightBridge < maximumLightBridge) {
          lightBridge++;
          samples++;
        } else if (runStart >= 0) {
          finishRun();
    ...
    if (!detections.length) return [] as Text4jFreeformWallSegment[];
    const groups: RadialRun[][] = [];
    detections.forEach(detection => {
      const previous = groups[groups.length - 1];
      if (previous && Math.round(detection.angle * 180 / Math.PI)
        === Math.round(previous[previous.length - 1].angle * 180 / Math.PI) + 1) previous.push(detection);
      else groups.push([detection]);
    });
    if (groups.length > 1
      && Math.round(groups[0][0].angle * 180 / Math.PI) === 0
      && Math.round(groups[groups.length - 1][groups[groups.length - 1].length - 1].angle * 180 / Math.PI) === 359) {
      groups[0] = [...groups.pop()!, ...groups[0]];
    }
    return groups.flatMap(group => {
      if (group.length < 2) return [];
      const best = [...group].sort((first, second) =>
        (second.end - second.start) * second.support - (first.end - first.start) * first.support)[0];
      const direction = { x: Math.cos(best.angle), y: Math.sin(best.angle) };
      return [{
        p1: { x: center.x + direction.x * best.start, y: center.y + direction.y * best.start },
        p2: { x: center.x + direction.x * best.end, y: center.y + direction.y * best.end },
        thickness: Math.min(typicalThickness, radialProbeThickness * 1.15),
        confidence: clamp(0.78 + best.support * 0.16, 0, 0.94),
      }];
    });
  };
  const protectedRadialPartitions = recoverRasterRadialPartitions();
  const supportedCurvilinearStraightWall = (segment: Text4jFreeformWallSegment) => {
  ...
  const ordinaryExportFreeformSegments = collapseText4jExteriorAngularChamfers(
  ...
  const protectedRadialIsCovered = (segment: Text4jFreeformWallSegment) => ordinaryExportFreeformSegments.some(other => {
    const dx = segment.p2.x - segment.p1.x, dy = segment.p2.y - segment.p1.y;
    const otherDx = other.p2.x - other.p1.x, otherDy = other.p2.y - other.p1.y;
    const length = Math.hypot(dx, dy), otherLength = Math.hypot(otherDx, otherDy);
    const alignment = Math.abs((dx * otherDx + dy * otherDy) / Math.max(1e-6, length * otherLength));
    if (alignment < 0.96) return false;
    const midpoint = { x: (segment.p1.x + segment.p2.x) / 2, y: (segment.p1.y + segment.p2.y) / 2 };
    const otherMidpoint = { x: (other.p1.x + other.p2.x) / 2, y: (other.p1.y + other.p2.y) / 2 };
    const normalDistance = Math.abs((midpoint.x - otherMidpoint.x) * -otherDy
      + (midpoint.y - otherMidpoint.y) * otherDx) / Math.max(1e-6, otherLength);
    return normalDistance <= typicalThickness * 1.2
      && Math.hypot(midpoint.x - otherMidpoint.x, midpoint.y - otherMidpoint.y) <= Math.max(length, otherLength) * 0.55;
  });
  const exportFreeformSegments = [
    ...ordinaryExportFreeformSegments,
    ...protectedRadialPartitions.filter(segment =>
      !duplicatesAxisWall(segment) && !protectedRadialIsCovered(segment)),
  ];
  ...
  const isTinyAngularConnector = (segment: (typeof cleanedAngularWalls.freeformSegments)[number]) => {
    ...
    const connectedToLongWall = (point: Text4jPixelPoint) => exportFreeformSegments.some(other => {
      if (other === segment) return false;
  ...
  const pointIsHostedByAxisWall = (point: Text4jPixelPoint) => outputWallSegments.some(axisSegment => {
  ...
  const freeformOnFootprintEdge = (segment: Text4jFreeformWallSegment) => {
        ...
        evidence: {
          ...
          notes: [segment.bridge
            ...
            : segment.cleanup
              ...
              : `${geometryMode[0].toUpperCase()}${geometryMode.slice(1)} wall chord retained by the Text 4.0 J geometry detector.`],
        },
      };
    });
  ...
  const gapRecords = architecturalHostedGaps.map(gap => {
    ...
    const outputGap = clampText4jRecoveredSwingGapToJambFacesForTest(
      gap,
      outputWallSegments,
      freeformSegments,
      typicalThickness,
    ...
    const windowFrameBandCount = Math.max(
      symbolEvidence.windowFrameBandCount || 0,
      parallelWindowFrameBandCount(gap, originalDoorEvidenceMask, width, height),
    );
    const explicitParallelWindowFrame = windowFrame && windowFrameBandCount >= 2;
    ...
    const dashedOpeningBandCount = parallelDashedOpeningBandCount(
      gap,
      dashedOpeningEvidenceMask,
      width,
      height,
    );
    const structuralWallBandCount = continuousStructuralWallBandCount(
      gap,
      originalDarkMask,
      width,
      height,
    ...
    const intactDoubleLineWall = doorSwing.detected
    ...
    const slidingDoor = slidingEvidence.detected
      && !foldingDoor
      && !gap.openPassageEvidence
      && dashedOpeningBandCount < 2
      && (!windowFrame || slidingEvidence.confidence >= 0.88)
      && structuralWallBandCount === 0
      && wallBandInk < 0.58
      && rawWidthMeters >= 0.8
      && rawWidthMeters <= 3.2;
    ...
    const doorStroke = doorSwing.detected
      ...
      && clearSwingSymbol
      && (structuralWallBandCount === 0
        || (doorSwing.closedLeaf && doorSwing.arcConfirmed))
      // Parallel glazing defeats arc-only and near-closed interpretations.
      // A distinct leaf plus matching arc remains authoritative.
       && (!explicitParallelWindowFrame || doorSwing.closedLeaf || (doorSwing.endpointCount >= 2
         ? doorSwing.doubleArcConfirmed
         : doorSwing.arcConfirmed && !doorSwing.arcOnly && !doorSwing.closedLeaf));
    const hostedNonDoorKind = resolveText4jHostedNonDoorRasterKindForTest({
      exterior,
      explicitOpenPassage: !!gap.openPassageEvidence,
      dashedOpeningBandCount,
      windowFrame,
      windowFrameBandCount,
      continuousStructuralWallBandCount: structuralWallBandCount,
      wallBandInk,
      missingHostWindow: gap.missingHostFallback === 'window',
    });
    const authoritativeWindowFrame = hostedNonDoorKind === 'window';
    ...
    const framedInsetPixels = resolveText4jFramedInsetPixelsForTest(
      outputGap,
    ...
    return {
      ...
      foldingDoor,
      foldingEvidence,
      windowFrame: authoritativeWindowFrame,
      wallOpeningEvidence: hostedNonDoorKind === 'opening',
      ...
      intactSolidWall: hostedNonDoorKind === 'wall'
        && !doorStroke && !slidingDoor && !foldingDoor,
  ...
  for (const record of gapRecords) {
    ...
    } else if (!record.wallOpeningEvidence
      && !record.foldingDoor && !effectiveSlidingDoor && !record.doorStroke
  ...
  freeformGaps.forEach((gap: Text4jFreeformWallGap) => {
    ...
    const evidence = inspectText4jFreeformGap(
      gap,
      originalDarkMask,
      originalDoorEvidenceMask,
      width,
    ...
    if (isDoor) {
      ...
      // inspectText4jFreeformGap reports hinge/side in the gap's own p1->p2
  ...
  openings.splice(0, openings.length, ...arbitrateText4jHostedOpenings(doors, openings));
  windows.splice(0, windows.length, ...arbitrateText4jHostedWindows(doors, windows));
  ...
  openings.splice(0, openings.length, ...openings.filter(opening =>
    text4jOpeningHasHost(opening, walls, hostTolerance)));
  ...
  const roomTopologyAudit = text4jRoomTopologyAudit(
    finalFlood,
    roomTextTags,
    width,
    height,
  ...
  if (topologyRecoveredPartitionCount) {
    warnings.push(
      `${topologyRecoveredPartitionCount} raster-supported interior wall segment${topologyRecoveredPartitionCount === 1 ? '' : 's'} recovered because the observed geometry improved OCR-tagged room enclosure topology.`,
    );
  }
  ...
  if (roomTagsOutsideSelectedFrame.length) {
    warnings.push(
      `${roomTagsOutsideSelectedFrame.length} OCR room label${roomTagsOutsideSelectedFrame.length === 1 ? '' : 's'} fell outside the selected wall frame; the local result may be missing a disconnected floorplan component.`,
    );
  }
  if (geometryMode !== 'orthogonal') {
    ...
    warnings.push(
      `${geometryMode[0].toUpperCase()}${geometryMode.slice(1)} geometry detector retained ${representations.join(' and ')}; scaling, OCR, enclosure recovery, and JSON mapping used the common Text 4.0 J pipeline.`,
    );
  }
  ...
  if (roomTopologyAudit.severe) {
    warnings.push(
      `Only ${roomTopologyAudit.observedRegions} enclosed OCR regions were recovered for ${roomTopologyAudit.expectedGroups} independently labelled room groups; import remains locked pending topology review.`,
    );
  }
  ...
  const confidence: NonNullable<GeneratedData['extractionDiagnostics']>['confidence'] =
    selectedFrameMissesRoomLabels || roomTopologyAudit.severe
      ? 'low'
  ...
  const canImport = !selectedFrameMissesRoomLabels && !roomTopologyAudit.severe && (roomSlabEnclosureIncomplete
