...
type LengthUnit = ConfirmedText4fBrief['dimensions']['envelope']['unit'];
...
export interface Text4fRoomCrossDimensionLabel {
  label: string;
...
export const formatText4fRoomCrossDimensions = (
  widthMeters: number,
  depthMeters: number,
  unit: LengthUnit,
): string => unit === 'ft'
...
export const buildText4fRoomCrossDimensionLabels = (
  geometry: GeneratedData,
  unit: LengthUnit,
): Text4fRoomCrossDimensionLabel[] => {
  ...
  return (geometry.rooms || []).flatMap(room => {
    ...
    return [{
      ...
      widthMeters,
      depthMeters,
      dimensionText: formatText4fRoomCrossDimensions(widthMeters, depthMeters, unit),
    }];
  });
};
...
const loadImage = (dataUrl: string): Promise<HTMLImageElement> => new Promise((resolve, reject) => {
  ...
  image.onerror = () => reject(new Error('Generated Text 4.0 F image could not be decoded for room annotation.'));
  image.src = dataUrl;
});
 ...
 */
export const annotateText4fGeneratedImageRooms = async (
  dataUrl: string,
  geometry: GeneratedData,
  ...
  const labels = buildText4fRoomCrossDimensionLabels(geometry, brief.dimensions.envelope.unit);
