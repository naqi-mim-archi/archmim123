...
import { ai } from "./aiClient4g";
import { getText4gConversationProfile } from "./text4gConversationVariation";

export const refineDesignRequirements4g = async (
  chatHistory: { role: 'user' | 'model'; text: string }[],
  variationIndex = 0,
): Promise<{ reply: string; isReady: boolean; summary: string }> => {
  const profile = getText4gConversationProfile(variationIndex);
