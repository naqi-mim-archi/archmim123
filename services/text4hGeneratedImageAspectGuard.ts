...
import { getText4hRectangularBoundaryEligibility } from './text4hBrief';

export interface Text4hAspectRaster {
  width: number;
  height: number;
  data: ArrayLike<number>;
}

export interface Text4hStructuralFrame {
  minX: number;
  maxX: number;
  minY: number;
  maxY: number;
...
export interface Text4hAspectCorrectionPlan {
  applied: boolean;
  targetRatio: number;
  sourceRatio: number;
...
export interface Text4hGeneratedImageAspectResult {
  dataUrl: string;
  applied: boolean;
  targetRatio?: number;
  sourceRatio?: number;
  correctedRatio?: number;
  relativeError?: number;
  reason: 'not-rectangle-locked' | 'frame-not-detected' | Text4hAspectCorrectionPlan['reason'];
}
 ...
 */
export const detectText4hLockedStructuralFrame = (
  raster: Text4hAspectRaster,
): Text4hStructuralFrame | undefined => {
  ...
  let bestFrame: Text4hStructuralFrame | undefined;
  let bestScore = -1;
...
export const planText4hLockedAspectCorrection = (
  frame: Pick<Text4hStructuralFrame, 'ratio'>,
  targetRatio: number,
  tolerance = 0.02,
): Text4hAspectCorrectionPlan => {
...
export const getText4hLockedImageAspectTarget = (
...
const loadImage = (dataUrl: string): Promise<HTMLImageElement> => new Promise((resolve, reject) => {
  ...
  image.onerror = () => reject(new Error('Generated Text 4.0 H image could not be decoded for aspect validation.'));
  image.src = dataUrl;
});
 ...
 */
export const enforceText4hGeneratedImageAspect = async (
  dataUrl: string,
  brief: ConfirmedText4hBrief,
): Promise<Text4hGeneratedImageAspectResult> => {
  const targetRatio = getText4hLockedImageAspectTarget(brief);
  ...
  const frame = detectText4hLockedStructuralFrame(sourceContext.getImageData(0, 0, width, height));
  if (!frame) return { dataUrl, applied: false, targetRatio, reason: 'frame-not-detected' };
  const plan = planText4hLockedAspectCorrection(frame, targetRatio);
  if (!plan.applied) {
    return {
      dataUrl,
      applied: false,
