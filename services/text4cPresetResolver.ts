/**
 * Conservative preset resolution for Text 4.0 C digitization.
 *
...
export const TEXT4C_WALL_PRESETS = [
...
export const TEXT4C_DOOR_PRESETS = [
...
export const TEXT4C_WINDOW_PRESETS = [
...
export type Text4cEvidenceStrength = 'strong' | 'medium' | 'weak' | 'none';
export type Text4cResolutionConfidence = 'high' | 'medium' | 'low' | 'unresolved';
export type Text4cResolutionProvenance =
  | 'measured-preset'
  | 'measured-custom'
  | 'role-default'
  | 'semantic-default'
  | 'unresolved';
export type Text4cWallRole = 'exterior' | 'interior' | 'structural-interior' | 'partition' | 'glass';

export interface Text4cMeasuredValues {
  widthM: number | null;
  heightM: number | null;
  thicknessM: number | null;
}

export interface Text4cPresetResolution {
  kind: 'wall' | 'door' | 'window';
  presetId: string | null;
  presetLabel: string | null;
  subtype: string;
  measuredValues: Text4cMeasuredValues;
  resolvedValues: Text4cMeasuredValues;
  confidence: Text4cResolutionConfidence;
  provenance: Text4cResolutionProvenance;
...
export interface ResolveText4cWallInput {
  semanticRole: Text4cWallRole;
  projectClass?: string | null;
  measuredThicknessM?: number | null;
  evidenceStrength?: Text4cEvidenceStrength;
  /** Required for measured-to-preset snapping. No implicit tolerance is applied. */
  toleranceM?: number | null;
}

export interface ResolveText4cDoorInput {
...
export interface ResolveText4cWindowInput {
  measuredWidthM?: number | null;
  measuredHeightM?: number | null;
  detectedSubtype?: string | null;
  evidenceStrength?: Text4cEvidenceStrength;
...
const evidenceConfidence = (
  strength: Text4cEvidenceStrength,
  unresolved = false,
): Text4cResolutionConfidence => {
...
const emptyValues = (): Text4cMeasuredValues => ({ widthM: null, heightM: null, thicknessM: null });
...
const wallCandidates = (role: Text4cWallRole): readonly (typeof TEXT4C_WALL_PRESETS)[number][] => {
  if (role === 'exterior') return TEXT4C_WALL_PRESETS.filter(preset => preset.subtype === 'exterior');
  return TEXT4C_WALL_PRESETS.filter(preset => preset.subtype === role);
};

const semanticWallPresetId = (role: Text4cWallRole, projectClass?: string | null): string => {
...
export const resolveText4cWallPreset = (input: ResolveText4cWallInput): Text4cPresetResolution => {
  ...
  const measuredValues: Text4cMeasuredValues = { ...emptyValues(), thicknessM: measuredThicknessM };
  ...
  const preset = TEXT4C_WALL_PRESETS.find(candidate => candidate.id === presetId)!;
...
export const resolveText4cDoorPreset = (input: ResolveText4cDoorInput): Text4cPresetResolution => {
  ...
  const measuredValues: Text4cMeasuredValues = {
    ...emptyValues(),
  ...
  if (measuredWidthM !== null) {
    const knownSubtype = TEXT4C_DOOR_PRESETS.some(preset => preset.subtype === detectedSubtype);
    const candidates = knownSubtype
      ? TEXT4C_DOOR_PRESETS.filter(preset => preset.subtype === detectedSubtype)
      : detectedSubtype ? [] : [...TEXT4C_DOOR_PRESETS];
  ...
  const rolePreset = rolePresetId
    ? TEXT4C_DOOR_PRESETS.find(candidate => candidate.id === rolePresetId) || null
    : null;
  if (rolePreset) {
    return {
      kind: 'door',
...
export const resolveText4cWindowPreset = (input: ResolveText4cWindowInput): Text4cPresetResolution => {
  const measuredWidthM = positiveMeasurement(input.measuredWidthM);
  const measuredHeightM = positiveMeasurement(input.measuredHeightM);
  const toleranceM = explicitTolerance(input.toleranceM);
  const evidence = input.evidenceStrength || 'none';
  ...
  if (measuredWidthM !== null) {
    const knownSubtype = TEXT4C_WINDOW_PRESETS.some(preset => preset.subtype === detectedSubtype);
    ...
    const candidates = knownSubtype
      ? TEXT4C_WINDOW_PRESETS.filter(preset => preset.subtype === detectedSubtype)
      : detectedSubtype ? [] : TEXT4C_WINDOW_PRESETS.filter(preset => preset.subtype === 'regular');
    const nearest = candidates
      .map(preset => {
