...
export type Text4gMasterFloorplanSourceKind = 'generated' | 'uploaded';
export type Text4gMasterThinkingLevel = 'minimal' | 'low' | 'medium' | 'high';

export interface Text4gMasterFloorplanRequest {
  imageBase64: string;
...
export interface Text4gMasterGraphValidation {
  valid: boolean;
  errors: string[];
...
export const buildText4gMasterFloorplanPrompt = (
  ...
  return `You are the Text 4.0 G master floorplan geometry transcriber.
...
export const validateText4gMasterFloorplanGraph = (rawData: any): Text4gMasterGraphValidation => {
  const errors: string[] = [];
...
const transformMasterGraph = (
  rawData: any,
  request: Pick<Text4gMasterFloorplanRequest, 'requestedBoundary' | 'requestedExtentsMeters'>,
...
const validWalls = (data: any): NonNullable<GeneratedData['walls']> => (Array.isArray(data?.walls) ? data.walls : [])
  .map((wall: any) => {
    ...
    const normalized: NonNullable<GeneratedData['walls']>[number] = {
      ...
      provenance: 'observed',
      evidence: { source: 'raster', confidence: asNumber(wall.confidence, curveType === 'line' ? 0.82 : 0.88), notes: ['Text 4.0 G Gemini geometry graph.'] },
    };
...
export const normalizeText4gMasterFloorplanData = (
  rawData: any,
  request: Pick<Text4gMasterFloorplanRequest, 'brief' | 'imageBase64' | 'sourceKind' | 'requestedBoundary' | 'requestedExtentsMeters'>,
): GeneratedData => {
  const graphValidation = validateText4gMasterFloorplanGraph(rawData);
    ...
    extractionDiagnostics: {
      ...
      warnings: [
        request.sourceKind === 'generated'
          ? 'Generated raster was transcribed through the independent Text 4.0 G Gemini geometry graph.'
          : 'Uploaded raster was transcribed through the independent Text 4.0 G Gemini geometry graph.',
...
export const isText4gMasterFloorplanDataUsable = (data: GeneratedData): boolean =>
  (data.walls?.length || 0) >= 2 && data.extractionDiagnostics?.canImport === true;

export async function transcribeText4gMasterFloorplanData(
  request: Text4gMasterFloorplanRequest,
): Promise<GeneratedData> {
  const { data, mimeType } = splitText4gDataUrl(request.imageBase64);
  const prompt = buildText4gMasterFloorplanPrompt(
    request.brief,
    request.sourceKind,
    request.requestedBoundary,
    request.requestedExtentsMeters,
  );
  const response = await fetch('/api/text4g/master-geometry', {
    method: 'POST',
  ...
  if (!response.ok) {
    ...
    throw new Error(`Text 4.0 G master floorplan data API error (${response.status}): ${errorText}`);
  }
  ...
  return normalizeText4gMasterFloorplanData(geometry, request);
}
