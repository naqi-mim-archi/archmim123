...
export interface Text4gAxisWallSegment {
...
export interface Text4gCleanAngularWallSegment extends Text4gFreeformWallSegment {
...
interface ExteriorAngularChamferCleanupOptions {
  ...
  isExterior: (segment: Pick<Text4gFreeformWallSegment, 'p1' | 'p2' | 'thickness'>) => boolean;
}
...
interface CleanupOptions {
  ...
  height: number;
  typicalThickness: number;
  protectedGaps?: Text4gFreeformWallGap[];
}
...
const distance = (a: Text4gPixelPoint, b: Text4gPixelPoint) =>
...
const isAxisAligned = (segment: Pick<Text4gFreeformWallSegment, 'p1' | 'p2'>) => {
...
const nearestEndpoint = (point: Text4gPixelPoint, segment: Text4gFreeformWallSegment) => {
...
const alignCanonicalAngularFamilies = (
  sourceSegments: Text4gCleanAngularWallSegment[],
...
const absorbAxisAlignedFreeformBridges = (
  sourceFreeform: Text4gCleanAngularWallSegment[],
  sourceAxis: Text4gAxisWallSegment[],
  typicalThickness: number,
) => {
  const axis = sourceAxis.map(segment => ({ ...segment }));
  const retained: Text4gCleanAngularWallSegment[] = [];
...
const asGenericSegments = (
  freeform: Text4gCleanAngularWallSegment[],
  axis: Text4gAxisWallSegment[],
): GenericSegment[] => [
  ...
  while (changed) {
    ...
    for (let first = 0; first < segments.length; first++) {
      for (let second = first + 1; second < segments.length; second++) {
        ...
        let farthest: [Text4gPixelPoint, Text4gPixelPoint] = [allPoints[0], allPoints[1]];
    ...
    let endpoints: [Text4gPixelPoint, Text4gPixelPoint] = [points[0], points[1]];
    let span = 0;
    ...
    const merged: Text4gCleanAngularWallSegment = {
      p1: { ...endpoints[0] },
      p2: { ...endpoints[1] },
 ...
 */
export const collapseText4gExteriorAngularChamfers = (
  ...
  const segments = sourceSegments.map(segment => ({
    ...
    p1: { ...segment.p1 },
    p2: { ...segment.p2 },
  } as Text4gCleanAngularWallSegment));
...
const collapseMicroConnectors = (
  ...
  while (changed) {
    ...
    for (const connector of connectors) {
      ...
      let repair: { first: GenericSegment; second: GenericSegment; point: Text4gPixelPoint; score: number } | undefined;
 ...
 */
export const cleanupText4gAngularWalls = (
  freeformSegments: Text4gFreeformWallSegment[],
  axisSegments: Text4gAxisWallSegment[],
  options: CleanupOptions,
) => {
