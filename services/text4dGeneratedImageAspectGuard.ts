...
import { getText4dRectangularBoundaryEligibility } from './text4dBrief';

export interface Text4dAspectRaster {
  width: number;
  height: number;
  data: ArrayLike<number>;
}

export interface Text4dStructuralFrame {
  minX: number;
  maxX: number;
  minY: number;
  maxY: number;
...
export interface Text4dAspectCorrectionPlan {
  applied: boolean;
  targetRatio: number;
  sourceRatio: number;
...
export interface Text4dGeneratedImageAspectResult {
  dataUrl: string;
  applied: boolean;
  targetRatio?: number;
  sourceRatio?: number;
  correctedRatio?: number;
  relativeError?: number;
  reason: 'not-rectangle-locked' | 'frame-not-detected' | Text4dAspectCorrectionPlan['reason'];
}
 ...
 */
export const detectText4dLockedStructuralFrame = (
  raster: Text4dAspectRaster,
): Text4dStructuralFrame | undefined => {
  ...
  let bestFrame: Text4dStructuralFrame | undefined;
  let bestScore = -1;
...
export const planText4dLockedAspectCorrection = (
  frame: Pick<Text4dStructuralFrame, 'ratio'>,
  targetRatio: number,
  tolerance = 0.02,
): Text4dAspectCorrectionPlan => {
...
export const getText4dLockedImageAspectTarget = (
...
const loadImage = (dataUrl: string): Promise<HTMLImageElement> => new Promise((resolve, reject) => {
  ...
  image.onerror = () => reject(new Error('Generated Text 4.0 D image could not be decoded for aspect validation.'));
  image.src = dataUrl;
});
 ...
 */
export const enforceText4dGeneratedImageAspect = async (
  dataUrl: string,
  brief: ConfirmedText4dBrief,
): Promise<Text4dGeneratedImageAspectResult> => {
  const targetRatio = getText4dLockedImageAspectTarget(brief);
  ...
  const frame = detectText4dLockedStructuralFrame(sourceContext.getImageData(0, 0, width, height));
  if (!frame) return { dataUrl, applied: false, targetRatio, reason: 'frame-not-detected' };
  const plan = planText4dLockedAspectCorrection(frame, targetRatio);
  if (!plan.applied) {
    return {
      dataUrl,
      applied: false,
