import type {
  Text4gFreeformWallGap,
  Text4gFreeformWallSegment,
  Text4gGeometryMode,
  Text4gPixelPoint,
} from './text4gGeometryModes';

export interface Text4gNativeArcRun {
  sourceIndices: number[];
  p1: Text4gPixelPoint;
  p2: Text4gPixelPoint;
  controlPoint: Text4gPixelPoint;
...
interface CurveArcOptions {
  ...
  exteriorEvidence?: (segment: Text4gFreeformWallSegment) => boolean;
  /** A non-facade curve may survive only when its source is a strict raster glazing band. */
  interiorWindowEvidence?: (segment: Text4gFreeformWallSegment) => boolean;
  /**
  ...
  hostedGaps?: Text4gFreeformWallGap[];
}
...
interface CurveFamilyModel {
  ...
  pointAt: (angle: number) => Text4gPixelPoint;
  residual: (point: Text4gPixelPoint) => number;
  tangentSupport: (segment: Text4gFreeformWallSegment) => number;
}
...
const leastSquaresCircle = (points: Text4gPixelPoint[]): CircleFit | undefined => {
...
const circleThroughThreePoints = (
  start: Text4gPixelPoint,
  middle: Text4gPixelPoint,
  end: Text4gPixelPoint,
) => {
  const determinant = 2 * (
 ...
 */
export const text4gCurveArcSourceEnvelopeCoherentForTest = (
  arc: Text4gNativeArcRun,
  sourceSegments: Text4gFreeformWallSegment[],
  width: number,
  height: number,
...
const rasterSupport = (
  fit: { center: Text4gPixelPoint; radius: number; startAngle: number; endAngle: number; counterclockwise: boolean },
  mask: Uint8Array,
  width: number,
  height: number,
  thickness: number,
...
const fitAxisEllipse = (points: Text4gPixelPoint[]): CurveFamilyModel | undefined => {
  ...
  const basis = (point: Text4gPixelPoint) => {
  ...
  const residual = (point: Text4gPixelPoint) => {
  ...
  const tangentSupport = (segment: Text4gFreeformWallSegment) => {
    const midpoint = {
...
const curveParameter = (model: CurveFamilyModel, point: Text4gPixelPoint) =>
...
const fitSeparatedCurveFamilies = (
  ...
  for (let family = 0; family < 4 && remaining.size >= 4; family++) {
    ...
    if (!candidate) {
      ...
      const segmentDistance = (first: Text4gFreeformWallSegment, second: Text4gFreeformWallSegment) => Math.min(
...
const buildRuns = (segments: Text4gFreeformWallSegment[], typicalThickness: number): OrderedRun[] => {
  const eligible = segments.map((segment, index) => ({ segment, index })).filter(({ segment }) =>
    !segment.bridge
...
export const consolidateText4gCurveArcs = (
  segments: Text4gFreeformWallSegment[],
  options: CurveArcOptions,
): { arcs: Text4gNativeArcRun[]; retainedSegments: Text4gFreeformWallSegment[] } => {
  ...
  const localSamples = (arc: Text4gNativeArcRun) => [0, 0.25, 0.5, 0.75, 1]
    .map(t => pointOnArc(arc.center, arc.radius, arc.startAngle, arc.endAngle, arc.counterclockwise, t));
  const familyResidual = (family: Text4gNativeArcRun, point: Text4gPixelPoint) => {
  ...
  const sameEllipseFamily = (first: Text4gNativeArcRun, second: Text4gNativeArcRun) =>
  ...
  const familySources = (arc: Text4gNativeArcRun) => {
    ...
    return indices.map(index => segments[index]).filter((segment): segment is Text4gFreeformWallSegment => !!segment);
  };
  const exteriorRatio = (arc: Text4gNativeArcRun) => {
  ...
  const dominantExteriorFamily = options.mode === 'hybrid' && options.exteriorEvidence
    ...
    : undefined;
  const fullyInsideDominantExterior = (arc: Text4gNativeArcRun) => {
  ...
  const curveCoversSegment = (segment: Text4gFreeformWallSegment, arc: Text4gNativeArcRun) => {
