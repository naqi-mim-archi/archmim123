...
export interface Text4jAxisWallSegment {
...
export interface Text4jCleanAngularWallSegment extends Text4jFreeformWallSegment {
...
interface ExteriorAngularChamferCleanupOptions {
  ...
  isExterior: (segment: Pick<Text4jFreeformWallSegment, 'p1' | 'p2' | 'thickness'>) => boolean;
}
...
interface CleanupOptions {
  ...
  height: number;
  typicalThickness: number;
  protectedGaps?: Text4jFreeformWallGap[];
}
...
const distance = (a: Text4jPixelPoint, b: Text4jPixelPoint) =>
...
const isAxisAligned = (segment: Pick<Text4jFreeformWallSegment, 'p1' | 'p2'>) => {
...
const nearestEndpoint = (point: Text4jPixelPoint, segment: Text4jFreeformWallSegment) => {
...
const alignCanonicalAngularFamilies = (
  sourceSegments: Text4jCleanAngularWallSegment[],
...
const absorbAxisAlignedFreeformBridges = (
  sourceFreeform: Text4jCleanAngularWallSegment[],
  sourceAxis: Text4jAxisWallSegment[],
  typicalThickness: number,
) => {
  const axis = sourceAxis.map(segment => ({ ...segment }));
  const retained: Text4jCleanAngularWallSegment[] = [];
...
const asGenericSegments = (
  freeform: Text4jCleanAngularWallSegment[],
  axis: Text4jAxisWallSegment[],
): GenericSegment[] => [
  ...
  while (changed) {
    ...
    for (let first = 0; first < segments.length; first++) {
      for (let second = first + 1; second < segments.length; second++) {
        ...
        let farthest: [Text4jPixelPoint, Text4jPixelPoint] = [allPoints[0], allPoints[1]];
    ...
    let endpoints: [Text4jPixelPoint, Text4jPixelPoint] = [points[0], points[1]];
    let span = 0;
    ...
    const merged: Text4jCleanAngularWallSegment = {
      p1: { ...endpoints[0] },
      p2: { ...endpoints[1] },
 ...
 */
export const collapseText4jExteriorAngularChamfers = (
  ...
  const segments = sourceSegments.map(segment => ({
    ...
    p1: { ...segment.p1 },
    p2: { ...segment.p2 },
  } as Text4jCleanAngularWallSegment));
...
const collapseMicroConnectors = (
  ...
  while (changed) {
    ...
    for (const connector of connectors) {
      ...
      let repair: { first: GenericSegment; second: GenericSegment; point: Text4jPixelPoint; score: number } | undefined;
 ...
 */
export const cleanupText4jAngularWalls = (
  freeformSegments: Text4jFreeformWallSegment[],
  axisSegments: Text4jAxisWallSegment[],
  options: CleanupOptions,
) => {
