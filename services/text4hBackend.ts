...
} from './text4hImageConfig';
import { MediaResolution, ThinkingLevel, Type } from "@google/genai";
import { validateText4hMasterFloorplanGraph } from './text4hMasterFloorplanData';
...
export const warmText4hVertexAuth = async () => {
...
export const routeText4hApiRequest = async (
  request: ApiRequest,
  ...
  try {
    ...
    if (url.startsWith('/api/text4h/image-redraw')) {
      try {
        ...
        const validMeasurement = (value: unknown) => typeof value === 'number' && Number.isFinite(value) && value > 0;
        const scaleParts = [
        ...
        const redrawPrompt = scaleParts.length > 0
          ? `${TEXT4H_AUTOSCAN_REDRAW_PROMPT}\n\nConfirmed user scale marking: ${scaleParts.join('; ')}. Use these values as scale references while reproducing the attached plan exactly; do not invent a missing dimension or alter the architecture.`
          : TEXT4H_AUTOSCAN_REDRAW_PROMPT;
