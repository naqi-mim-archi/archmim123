import type {
  Text4jFreeformWallGap,
  Text4jFreeformWallSegment,
  Text4jGeometryMode,
  Text4jPixelPoint,
} from './text4jGeometryModes';

export interface Text4jNativeArcRun {
  sourceIndices: number[];
  p1: Text4jPixelPoint;
  p2: Text4jPixelPoint;
  controlPoint: Text4jPixelPoint;
...
interface CurveArcOptions {
  ...
  exteriorEvidence?: (segment: Text4jFreeformWallSegment) => boolean;
  /** A non-facade curve may survive only when its source is a strict raster glazing band. */
  interiorWindowEvidence?: (segment: Text4jFreeformWallSegment) => boolean;
  /**
  ...
  hostedGaps?: Text4jFreeformWallGap[];
}
...
interface CurveFamilyModel {
  ...
  pointAt: (angle: number) => Text4jPixelPoint;
  residual: (point: Text4jPixelPoint) => number;
  tangentSupport: (segment: Text4jFreeformWallSegment) => number;
}
...
const leastSquaresCircle = (points: Text4jPixelPoint[]): CircleFit | undefined => {
...
const circleThroughThreePoints = (
  start: Text4jPixelPoint,
  middle: Text4jPixelPoint,
  end: Text4jPixelPoint,
) => {
  const determinant = 2 * (
 ...
 */
export const text4jCurveArcSourceEnvelopeCoherentForTest = (
  arc: Text4jNativeArcRun,
  sourceSegments: Text4jFreeformWallSegment[],
  width: number,
  height: number,
...
const rasterSupport = (
  fit: { center: Text4jPixelPoint; radius: number; startAngle: number; endAngle: number; counterclockwise: boolean },
  mask: Uint8Array,
  width: number,
  height: number,
  thickness: number,
...
const fitAxisEllipse = (points: Text4jPixelPoint[]): CurveFamilyModel | undefined => {
  ...
  const basis = (point: Text4jPixelPoint) => {
  ...
  const residual = (point: Text4jPixelPoint) => {
  ...
  const tangentSupport = (segment: Text4jFreeformWallSegment) => {
    const midpoint = {
...
const curveParameter = (model: CurveFamilyModel, point: Text4jPixelPoint) =>
...
const fitSeparatedCurveFamilies = (
  ...
  for (let family = 0; family < 4 && remaining.size >= 4; family++) {
    ...
    if (!candidate) {
      ...
      const segmentDistance = (first: Text4jFreeformWallSegment, second: Text4jFreeformWallSegment) => Math.min(
...
const buildRuns = (segments: Text4jFreeformWallSegment[], typicalThickness: number): OrderedRun[] => {
  const eligible = segments.map((segment, index) => ({ segment, index })).filter(({ segment }) =>
    !segment.bridge
...
export const consolidateText4jCurveArcs = (
  segments: Text4jFreeformWallSegment[],
  options: CurveArcOptions,
): { arcs: Text4jNativeArcRun[]; retainedSegments: Text4jFreeformWallSegment[] } => {
  ...
  const localSamples = (arc: Text4jNativeArcRun) => [0, 0.25, 0.5, 0.75, 1]
    .map(t => pointOnArc(arc.center, arc.radius, arc.startAngle, arc.endAngle, arc.counterclockwise, t));
  const familyResidual = (family: Text4jNativeArcRun, point: Text4jPixelPoint) => {
  ...
  const sameEllipseFamily = (first: Text4jNativeArcRun, second: Text4jNativeArcRun) =>
  ...
  const familySources = (arc: Text4jNativeArcRun) => {
    ...
    return indices.map(index => segments[index]).filter((segment): segment is Text4jFreeformWallSegment => !!segment);
  };
  const exteriorRatio = (arc: Text4jNativeArcRun) => {
  ...
  const dominantExteriorFamily = options.mode === 'hybrid' && options.exteriorEvidence
    ...
    : undefined;
  const fullyInsideDominantExterior = (arc: Text4jNativeArcRun) => {
  ...
  const curveCoversSegment = (segment: Text4jFreeformWallSegment, arc: Text4jNativeArcRun) => {
