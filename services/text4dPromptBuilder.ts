import {
  type ConfirmedText4dBrief,
  getText4dRectangularBoundaryEligibility,
  TEXT4D_RENDERING_PROFILE_ID,
} from './text4dBrief';
...
const TEXT4D_IMAGE_CANVAS_PIXELS = 1024;
const TEXT4D_FOOTPRINT_MAX_SPAN_PERCENT = 72;
...
const roomText = (brief: ConfirmedText4dBrief): string => brief.rooms
  .map(room => `${room.count} x ${clean(room.name)}${room.details ? ` (${clean(room.details)})` : ''}`)
  .join('; ');

const relevantPlanningRules = (brief: ConfirmedText4dBrief): string[] => {
...
export const buildText4dImagePrompt = (brief: ConfirmedText4dBrief): string => {
  ...
  const rectangularEligibility = getText4dRectangularBoundaryEligibility(brief);
  ...
  const frameWidth = TEXT4D_FOOTPRINT_MAX_SPAN_PERCENT * brief.dimensions.envelope.width / longestSide;
  const frameDepth = TEXT4D_FOOTPRINT_MAX_SPAN_PERCENT * brief.dimensions.envelope.depth / longestSide;
  const frameWidthPixels = Math.round(TEXT4D_IMAGE_CANVAS_PIXELS * frameWidth / 100);
  const frameDepthPixels = Math.round(TEXT4D_IMAGE_CANVAS_PIXELS * frameDepth / 100);
  const frameLeftPixels = Math.round((TEXT4D_IMAGE_CANVAS_PIXELS - frameWidthPixels) / 2);
  const frameTopPixels = Math.round((TEXT4D_IMAGE_CANVAS_PIXELS - frameDepthPixels) / 2);
