...
export const TEXT4H_RENDERING_PROFILE_ID = 'text4h-digitizable-floorplan-v1' as const;

export type Text4hAreaUnit = 'sq_ft' | 'sq_m';
export type Text4hLengthUnit = 'ft' | 'm';
export type Text4hValueSource = 'user_confirmed' | 'model_inferred' | 'calculated' | 'application_default';
export type Text4hEnvelopeScope = 'building_footprint' | 'enclosed_plan_envelope';
export type Text4hBriefSection = 'project' | 'dimensions' | 'rooms' | 'adjacency' | 'planning' | 'floors';
export type Text4hRectangularBoundarySource = 'automatic_area_match' | 'explicit_user_request' | 'user_confirmed' | 'ineligible';

export interface ConfirmedText4hBrief {
  version: '1';
  project: {
    category: string;
    type: string;
  ...
  dimensions: {
    area: { value: number; unit: Text4hAreaUnit; source: Text4hValueSource };
    envelope: {
      width: number;
      depth: number;
      unit: Text4hLengthUnit;
      scope: Text4hEnvelopeScope;
      source: Text4hValueSource;
    };
    rectangularBoundary: {
      locked: boolean;
      source: Text4hRectangularBoundarySource;
    };
  };
  rooms: Array<{ id: string; name: string; count: number; details?: string; source: Text4hValueSource }>;
  adjacency: Array<{
    id: string;
    description: string;
  ...
  renderingProfileId: typeof TEXT4H_RENDERING_PROFILE_ID;
  provenance: {
    draftSource: 'design_copilot' | 'legacy_summary';
    confirmed: boolean;
    lastEditedSection?: Text4hBriefSection;
  };
}

export interface Text4hBriefValidation {
  valid: boolean;
  errors: string[];
  warnings: string[];
}

export interface Text4hRectangularBoundaryEligibility {
  eligible: boolean;
  coverageRatio: number;
  mismatchRatio: number;
}

export const getText4hRectangularBoundaryEligibility = (
  brief: Pick<ConfirmedText4hBrief, 'project' | 'dimensions'>,
): Text4hRectangularBoundaryEligibility => {
...
export const text4hHasExplicitRectangularRequest = (text: string): boolean => {
...
export const applyText4hRectangularBoundaryPolicy = (
  brief: ConfirmedText4hBrief,
  explicitRequestText = '',
): ConfirmedText4hBrief => {
  const eligibility = getText4hRectangularBoundaryEligibility(brief);
  const current = brief.dimensions.rectangularBoundary;
  const rectangularBoundary: ConfirmedText4hBrief['dimensions']['rectangularBoundary'] = !eligibility.eligible
    ? { locked: false, source: 'ineligible' }
    : text4hHasExplicitRectangularRequest(explicitRequestText)
...
export const setText4hRectangularBoundaryLock = (
  brief: ConfirmedText4hBrief,
  locked: boolean,
): ConfirmedText4hBrief => {
  const eligibility = getText4hRectangularBoundaryEligibility(brief);
...
const parseRooms = (value: string | null, source: Text4hValueSource): ConfirmedText4hBrief['rooms'] => {
...
const parseAdjacency = (value: string | null, source: Text4hValueSource): ConfirmedText4hBrief['adjacency'] => {
  if (value === null) return [];
  return value
    .split(/[\n\r]+|\s+[•*]\s+/)
    .map(line => line.trim().replace(/^[-*•\s]+/, '').trim())
...
const parseProject = (purposeText: string | null, previous?: ConfirmedText4hBrief) => {
...
export const createEmptyText4hBrief = (): ConfirmedText4hBrief => ({
  version: '1',
  ...
  renderingProfileId: TEXT4H_RENDERING_PROFILE_ID,
  provenance: { draftSource: 'design_copilot', confirmed: false },
});

export const legacySummaryToText4hBrief = (
  summary: string,
  previous?: ConfirmedText4hBrief,
  source: Text4hValueSource = 'model_inferred',
): ConfirmedText4hBrief => {
  ...
  const areaUnit: Text4hAreaUnit = metric ? 'sq_m' : 'sq_ft';
  const lengthUnit: Text4hLengthUnit = areaUnit === 'sq_m' ? 'm' : 'ft';
  ...
  const nextBrief: ConfirmedText4hBrief = {
    version: '1',
  ...
  return applyText4hRectangularBoundaryPolicy(nextBrief, summary);
};

export const updateText4hBriefFromSummary = (
  summary: string,
  previous: ConfirmedText4hBrief,
  editedSection: Text4hBriefSection,
): ConfirmedText4hBrief => {
  const next = legacySummaryToText4hBrief(summary, previous, 'model_inferred');
  ...
  return applyText4hRectangularBoundaryPolicy(next);
};
...
export const text4hBriefToDesignSummary = (brief: ConfirmedText4hBrief): string => {
...
export const validateText4hBrief = (brief: ConfirmedText4hBrief): Text4hBriefValidation => {
  const errors: string[] = [];
...
export const text4hBriefBoundaryMeters = (brief: ConfirmedText4hBrief): Point[] | undefined => {
